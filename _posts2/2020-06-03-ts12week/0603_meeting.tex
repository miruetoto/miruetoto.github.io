...
\ck (규빈) 랜덤워크는 사실을 알고 있는사람이라면? 역시 안산다. 랜덤워크에 대한 지식이 없는 사람이라면 이 주식을 안산사람을 세상한심하게 볼것이다. 하지만 이주식을 $t=1500$까지 관측한다면? 결과는 아래와 같다.

\begin{figure}[h]
\includegraphics[width=1\textwidth]{fig2.png}
\end{figure}

\ck (규빈) 랜덤워크에 대한 올바른 지식이 있는사람이라면 재산을 지킬 수 있다. 

\ck (규빈) 만약에 회귀분석과 같은 느낌을 가지는 주식을 관찰했다면? 올바르게 배운사람이라면 
