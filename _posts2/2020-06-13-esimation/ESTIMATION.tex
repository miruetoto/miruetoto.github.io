...
\ex $X_1,\dots,X_n \overset{iid}{\sim} N(20,\sigma^2)$ 이라고 하자. 여기에서 $\mu$를 추정한다고 하자. 사람3이 $\hat{\mu}=20$ 이라고 주장한다고 치자. (이분은 그러니까 데이터를 안보고 추론하시는 분이다.)
...
\note MVUE를 UMVUE라고 부르기도 하는데 이는 다음과 같은 이유에서 그렇다. 

\rdash 
\ck 아래와 같은 스토리를 가진 문제가 많다. 
...
\four 그렇다면 우리가 구했던 MME가 MVUE라는 인증을 받게 된다. 
...
\ck UMVUE를 구하기 위해서는 충분통계량과 완비통계량을 먼저 설명해야한다. 

\ck 추정량과 통계량의 차이는 무엇인가? 추정량은 모수의 추론을 위해 만들어진 특별한 통계량이라고 해석하면 된다. 따라서 통계량은 추정량을 만드는 재료라고 볼 수 있다. 

\ck 좋은 추정량을 만들기 위해서는 MME, MLE와 같은 좋은 추정법을 개발하는 방법도 있다. 하지만 맛있는 음식은 좋은 재료에서 나오듯이 좋은 추정량을 만들기 위해서 좋은 통계량을 먼저 골라보자고 생각할 수 있다. 

\ck 그렇다면 어떠한 통계량이 모수 $\theta$를 추정하기에 적절한 재료가 될까? 일단 충분성과 완비성을 가진 통계량이 모수 $\theta$를 추정하기에 좋은 통계량이라고 외우자. 

\ck 이때 모수 $\theta$를 추론하기에 충분한 통계량을 충분통계량 (sufficient statistics) 이라고 하고 모수 $\theta$를 추론하기에 완전한 통계량을 완비통계량 (complete statistics) 라고 한다. 충분성과 완비성을 모두 갖추면 완비충분통계량 (complete sucfficient statistics) 이라고 하고 약자로 CSS라고 쓴다. 

\ck 따라서 우리의 전략은 아래와 같다. 

\one 데이터를 손질하여 완비충분통계량을 만든다. 

\two 손질된 완비충분통계량을 다시 요리하여 모수에 대한 좋은 추정량을 만든다. 

\ck 완비충분통계량은 기본적으로 모수를 추론하기에 좋은 통계량이다. 특히 완비충분통계량으로 추정량을 만든다면 UMVUE를 쉽게 만들 수 있다는 장점이 있다. 

\rdash 

$\newline$

\noindent\textbf{Step1. 충분성통계량을 구한다.}
