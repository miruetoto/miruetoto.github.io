...
\ck 좋은 추정량을 판단하는 기준: (1) 불편성 (2) 효율성 (3) 최소분산성 (4) 일치성 (5) 충분성 
...
\ck 장점: 직관적이고 쉽다. WLLN에 의해서 일치성을 보이기 쉽다. 
\note 일치성: $\hat{\theta} \overset{p}{\to} \theta ~ as~  n \to \infty.$ 

\ck 단점: 유일하지 않을 수 있다. 효율이 좋지 않은 경우가 많다. 
...
\ck 전략: 어차피 $p \in (0,1)$이므로 모든 $p$에 대하여 아래의 값을 조사하고 이것을 최소화하는 $p$를 구하자. 
\[
...
\hat{p}^{MME}=\bar{x}
\]
가 된다. 따라서 베르누이 분포의 경우 적률추정법과 최대가능도 추정법은 같다. 
