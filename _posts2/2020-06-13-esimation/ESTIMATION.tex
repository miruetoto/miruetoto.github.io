...
\four 그렇다면 우리가 구했던 MME가 MVUE로 업그레이드 된다. 

\note 쉽게 생각하면 MLE, MME는 추정을 하는 방법? 혹은 원리? 를 나타낸다. 이것이 좋은 추정량이라는 증거는 없다. 
\note MVUE는 좋은 추정량임을 인증하는 마크라고 생각하면 된다. 

\subsubsection{MVUE를 구하는 방법?}

\ck 최소분산불편추정량을 구하기 위해서는 충분통계량을 먼저 설명해야한다. 
...
\ck 그렇다면 어떠한 통계량이 모수 $\theta$를 추정하기에 적절한 재료가 될까? 데이터를 정리하여 모수를 추정하는데 불필요한 것은 버리고 모수를 추론할때 필요한 정보만 남기고 싶다. 즉 (1) 압축성 (2) 충분성을 만족하여야 한다. 이러한 통계량을 충분통계량이라고 한다. 

\ck 따라서 우리의 전략은 아래와 같다. 

\one 데이터를 손질하여 충분통계량을 만든다. 

\two 손질된 충분통계량으로 모수를 추론한다. 

\ck 충분통계량은 기본적으로 모수를 추론하기에 좋은 통계량이다. 특히 충분통계량으로 추정량을 만든다면 UMVUE를 쉽게 만들 수 있다는 장점이 있다. 
...
\ck $p$를 추론하는데 총 불량품수만 기억하면 되지 불량품들이 나온 순서 즉 개개의 $x_i$의 값은 알 필요가 없다. 즉 각 실험결과에 대하여 모든 관측치를 일일히 기록할 필요가 없고 아래만 기록하면 된다. 
\[
Y=\sum_{i=1}^{4}X_i 
\]
...
\note 여기에서 $X_1,X_2,X_3,X_4 \overset{iid}{\sim} Bernoulli(p)$ 이므로 $Y \sim b(4,p)$ 이다. 따라서 
\[
