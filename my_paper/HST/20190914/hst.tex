...
Let ${\cal G}=(V,{\bf E},{\bf W})$ be the weighted graph with $|V|=n$ and $f$ be the graph function defined on ${\cal G}$. For fixed $i$, let ${\cal N}_{v_i}$ be set of linked values with $v_i$. Let $b$ be the amount of falling snow and $\tau$ be the number of falling snows. Let $\{\ell_0,\ell_1,\dots ,\ell_{\tau}\}$ as \emph{trace of snow} where $\ell_0 \sim \mbox{Multinomial}($ is result of random selection from $V$ with probability $\frac{1}{n}$ and $\ell^{(\tau)}$ is defined
\[
v^{(\ell)}=\begin{cases}
