...
Let ${\cal G}=(V,{\bf E},{\bf W})$ be the weighted graph with $|V|=n$ and $f$ be the graph function defined on ${\cal G}$. Let ${\cal N}_{v}$ be set of linked vertex with $v$ and $b$ be the amount of falling snow and $\tau$ be the number of falling snows. Define $\{\zeta_0,\zeta_1,\dots ,\zeta_{\tau}\}$ as \emph{trace of snow} where $\zeta_0 \sim \mbox{Multinomial}(1/n\dots,1/n)$ and $\ell^{(\tau)}$ is defined
\[
v^{(\ell)}=\begin{cases}
