...
\[
p(u,v)=
\mbox{Prob}(\tilde v^{\ell}=v|\tilde v^{\ell-1}=u)=w(\tilde v^{\ell},\tilde v^{\ell-1}), ~ \ell=1,2,\dots,\tau
\]
with arbitary vertex $\tilde{v}^0$. Define $h^{\ell}(v)$, $\xi^{\ell}(v)$, and ${\cal U}^{\ell}$ as $\ell$-th \emph{snowyground}, $\ell$-th \emph{snow}, and $\ell$-th {downstream area}, respectively, i.e., 
\[
...
=\begin{cases}
\frac{\sum_{u\in V}w(u,v)}{\sum_{v\in V}\sum_{u\in V}w(u,v)}  & {\cal U}^{\ell-1}= \emptyset \\ 
