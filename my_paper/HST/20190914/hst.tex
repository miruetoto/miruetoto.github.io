...
Let $X:{\cal V} \to V$ be a random variable and $v_1,v_2,\dots,v_n$ be the $i.i.d.$ realization of $X$. Note that $\forall v \in V : ~ X^{-1}(v) \in \sigma(X)$ where $\sigma(X)$ is smallest $\sigma$-field which makes $X$ as random variable. Thus $X^{-1}(v_i)$ is always well defined. Let $V:=\{v_i:i=1,2,\dots,n\}$ and ${\bf E}$ be the $n \times n$ matrix whose $(i,j)$-th element is defined by 
$$E_{ij}=\begin{cases} 1 & (\nu_i,\nu_j)\in {\cal E} \\ 0 & (\nu_i,\nu_j) \notin {\cal E}.\end{cases}$$ Furthermore, let's define another $n\times n$ matrix ${\bf W}$, where each element $W_{ij}$ represents the similarity between two node $\nu_i$ and $\nu_j$ (or $v_i$ and $v_j$). Define $(V,{\bf E},{\bf W})$ triple as weighted graph ${\cal G}$. The graph function $f$ is real valued function such that 
\[
f : V \to \mathbb{R}.
\]
...
Let ${\cal G}=(V,{\bf E},{\bf W})$ be the weighted graph with $|V|=n$ and $f$ be the graph function defined on ${\cal G}$. For fixed $i$, let ${\cal N}_{v_i}$ be set of linked values with $v_i$. Let $b$ be the amount of falling snow and $\tau$ be the number of falling snows. Let $\{v^{(0)},v^{(1)},\dots,v^{(\tau)}\}$ as \emph{trace of snow} where $v^{(0)}$ is result of random selection from $V$ with probability $\frac{1}{n}$ and $v^{(\ell)}$ is defined
\[
v^{(\ell)}=\begin{cases}
