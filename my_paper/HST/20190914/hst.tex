...
\begin{dfn}
Let $V:=\{v_i:i=1,2,\dots,n\}$ be the given index set or realization of $X$. Let ${\boldsymbol f}$ be the graph signal on $(V,E)$. Let ${\cal H}({\boldsymbol f};\tau)$ be heavy-snow transform for ${\boldsymbol f}$. The \emph{snow distance} between $v_i$ and $v_j$ is defined by 
\[
...
\begin{thm} 
Let $V:=\{v_i:i=1,2,\dots,n\}$ be the given index set or realization of $X$. Define graph signal $f(v) \in {\cal M}$ and its heavy-snow transform ${\cal H}({\boldsymbol f};\tau)$. Let ${\cal S}^{(\tau)}:=\{{\boldsymbol h}^{(\tau)}(v): v\in V\}$. 
\begin{itemize}
