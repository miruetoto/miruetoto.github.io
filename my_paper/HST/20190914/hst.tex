...
Let ${\cal G}=(V,{\bf E},{\bf W})$ be the weighted graph with $|V|=n$ and $f$ be the graph function defined on ${\cal G}$. Let ${\cal N}_{v}$ be set of linked vertex with $v$ and $b$ be the amount of falling snow and $\tau$ be the number of falling snows. Define $\{\zeta^{(0)},\zeta^{(1)},\dots ,\zeta^{(\tau)}\}$ as \emph{trace of snow} where $\zeta_0 \sim \mbox{Multinomial}(1/n\dots,1/n)$ and $\ell^{(\tau)}$ is defined
\[
v^*_{\ell+1}=\begin{cases}
\mbox{ sample from } {\cal U}_{v^*_{\ell}} \mbox{ with probability } \frac{w(v^*_{\ell},v^*_{\ell+1})}{\sum_{v^*_{\ell+1} \in V} w(v^*_{\ell},v^*_{\ell+1})}  & {\cal U}_{v^*_{\ell-1}}\neq \emptyset \\ 
