...
\vskip 5mm
\noindent {\it Keywords}: Graph function; Similarity; Distance; Multiscale method. 
...
\item (flows or blocked) After accumulation, snow can move to one of ${\cal N}_{u}$ which is neighborhood of $u$. Of course, if ${\cal N}_{u}=\emptyset$, the snow will no longer flow. Then snow can always flow from $u$ to $v \in {\cal N}_{u}$ in the case of ${\cal N}_{u}\neq \emptyset$? The answer is NO since snow cannot flow high. In otherwords, ${\cal N}_{v}\neq \emptyset$ does not necessarily mean that snow can flow. Thus, we should compare value of $f(u)+b$ and $f(v)$ to check flowbility of snow, i.e., we should check ${\cal U}_{u}:={\cal N}_{u} \cap \{v: f(v) \leq f(u)+b\}$ is emptyset or not.. 
\begin{itemize}
\item (flow) In the case of ${\cal U}_{u}\neq\emptyset$, which is an unblocked situation, snow (which stays in $u$) can flow to $v$ that is randomly sampled element from ${\cal U}_{u}$ with probability $\frac{w(u,v)}{\sum_{v \in {\cal U}_{u} }w(u,v) }$. 
\item (blocked) If ${\cal U}_{u}=\emptyset$, which is a blocked situation, snow can't flow anymore. In this cases, the next node is randomly selected from $V$ with probability $\frac{1}{n}$. 
\end{itemize}
\item (next iteration) In the cases of `flows', go back to step 1 and repeat above procedure with assuming that snow falls at $v$ (which is element of ${\cal U}_{u}$) as much as $\gamma b$. In here $\gamma \in (0,1)$ is parameter adjusting viscosity of snow. (If you choose very small $\gamma$, shape of snow accumulation will be (c) in Figure 2.) For `blocked' situation, go back to step 1 with assuming that snow falls at $u^*$ (which is randomly selected node in $V$ with probability $\frac{1}{n}$) as much as $b$.
  
\end{enumerate}

The following is the formal definition of heavy-snow transform.
\begin{dfn}\label{snow:df_hst}
Let ${\cal G}=(V,{\bf E},{\bf W})$ be the weighted graph with $|V|=n$ and $f$ be the graph function defined on ${\cal G}$. Let ${\cal N}_{v}$ be set of linked vertex with $v$ and $b$ be the amount of falling snow and $\tau$ be the number of falling snows. Define $\{v^*_0,v^*_1,\dots ,v^*_{\tau}\}$ as \emph{trace of snow} where $v^*_0$ is sample from $V$ with probability $\frac{1}{n}$ and $v^*_{\ell}$ is defined
\[
v^*_{\ell+1}=\begin{cases}
\mbox{ sample from } {\cal U}_{v^*_{\ell}} \mbox{ with probability } \frac{w(v^*_{\ell},v^*_{\ell+1})}{\sum_{v\in {\cal U}_{v^*_{\ell}}} w(v^*_{\ell},v)}  & {\cal U}_{v^*_{\ell-1}}\neq \emptyset \\ 
\mbox{ sample from } V \mbox{ with probability  } \frac{1}{n}  & {\cal U}_{v^*_{\ell-1}}= \emptyset 
\end{cases}
\] Let $\boldsymbol f$ be ${\boldsymbol f}=\{f(v_i): i=1,\dots,n \}$.
...
\begin{dfn}
Let $V:=\{v_i:i=1,2,\dots,n\}$ be the given index set or realization of $X$. Let ${\boldsymbol f}$ be the graph function on $(V,E)$. Let ${\cal H}({\boldsymbol f};\tau)$ be heavy-snow transform for ${\boldsymbol f}$. The \emph{snow distance} between $v_i$ and $v_j$ is defined by 
\[
\rho^{(\tau)}(v_i , v_j) := \|{\boldsymbol h}^{(\tau)}_i- {\boldsymbol h}^{(\tau)}_j\|_2.
\]
where ${\boldsymbol h}^{(\tau)}_i=\big(h^{(0)}(v_i),\dots,h^{(\tau)}(v_i)\big)$. 
\end{dfn}

\begin{thm} 
Let $V:=\{v_i:i=1,2,\dots,n\}$ be the given index set or realization of $X$. Define graph function $f(v) \in {\cal M}$ and its heavy-snow transform ${\cal H}({\boldsymbol f};\tau)$. Let ${\cal S}^{(\tau)}:=\{{\boldsymbol h}^{(\tau)}(v): v\in V\}$. 
\begin{itemize}
	\item $\rho^{(0)}$ is metric in ${\cal M}$ as $n\to \infty$. 
	\item Further $\rho^{(\infty)}$ is metric in ${\cal V}$ as $n\to \infty$.
\end{itemize}
\end{thm}
\begin{proof}
\textbf{Claim 1.} 
