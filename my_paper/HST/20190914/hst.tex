...
=\begin{cases}
\frac{w(v,u)}{\sum_{v\in {\cal U}_u}w(v,u)}  & v \in {\cal U}_u\\ 
\frac{d(v)}{\sum_{i=1}^{n} d(v_i)} & v \notin {\cal U}_u
\end{cases}
\]
where 
Define $h^{\ell}(v)$, $\xi^{\ell}(v)$, and ${\cal U}^{\ell}$ as $\ell$-th \emph{snowyground}, $\ell$-th \emph{snow}, and $\ell$-th {downstream area}, respectively, i.e., 
\[
