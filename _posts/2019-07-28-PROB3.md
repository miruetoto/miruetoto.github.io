...
- ***Thm 2.2.6*** 에서 ***i.i.d*** 조건을 추가하면 **($\star$)** 는 **($\tilde\star$)** 를 임플라이 한다. 그리고 $b_n$을 적절하게 선택하면 (정확히 말하면 $b_n$의 오더가 $o(n)$ 보다 크도록) **($\star$)** 은 **($\star\star$)** 를 임플라이한다. 

- 적절한 $b_n$을 선택하였을 경우 **($\star$)** 가 **($\star\star$)** 를 함을 좀 더 자세히 살펴보자. 조건 **($\star$)** 에 ***Lemma 2.2.8*** 을 사용하면 아래와 같은 식을 얻을 수 있다. (듀렛교재참고) 
\begin{align}
