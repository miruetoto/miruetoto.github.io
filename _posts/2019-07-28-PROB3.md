...
- 이때 $\mu(n)\leq n$ 을 항상 만족하는 성질이 있음을 관찰하자. 따라서 ***Thm 2.2.7*** 은 평균이 유한한 경우 **WLLN** 이 성립하고 평균이 무한인 경우는 $n$으로 평균을 추정한다는 의미라고 해석할 수 있다. 즉 ***Thm 2.2.7*** 은 평균이 유한하지 않을때도 어느정도 합리적인 결과를 주는  **WLLN** 이라고 해석할 수 있다. 실제로 ***Thm 2.2.7*** 은 유한평균을 가진다는 조건이 있을때 ***Thm 2.2.9*** 로 바뀐다. 
