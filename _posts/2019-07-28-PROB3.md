...
- ***Thm 2.2.7*** 을 증명하기 위해서는 ***Lemma 2.2.8.*** 과 함께 ***Thm 2.2.6*** 을 증명해야 한다. ***Thm 2.2.6*** 은 아래의 장점이 있는 정리이다. <br/>
**(1)** $X_1,\dots,X_n$ 이 독립이지만 같은 분포를 따르지는 않는 경우에도 아래와 같은 **WLLN** 이 성립함을 증명가능하다. 
\begin{align}
\frac{X_1+\dots+X_n}{n} \to \frac{\mu_1(n)+\dots \mu_n(n)}{n}
\end{align}
**(2)** 

좀더 사기적인 퍼포먼스를 보여주는데 가령 아래와 같은 경우에서도 적용가능한 **WLLN** 이다. ***Thm 2.2.7*** 역시 ***Thm 2.2.9*** 에 비교하면 장점을 가지고 있는데 이것은 확률변수들이 유한평균값을 가지지 않을 경우도 적용가능하다는 장점이 있다. ***Thm 2.2.6*** 은 
