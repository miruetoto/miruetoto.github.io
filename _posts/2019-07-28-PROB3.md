...
\begin{align}
\frac{w_{n,1}X_1+\dots+w_{n,n}X_n}{\sum_{k=1}^n w_{n,k} } \to \frac{\mu_{w_{n,1}X_1} (\sum_{k=1}^n w_{n,k})+\dots + w_{n,n} \mu_n(\sum_{k=1}^n w_{n,k})}{\sum_{i=1}^n w_{n,k} } 
\end{align}
우리가 흔히 아는 **WLLN** 의 경우 $w_{n,1},\dots,w_{n,n}=1$ 인 경우이다. <br/>
