...
\end{align}
반대로 위 조건은 유한평균값을 가짐을 임플라이 하지는 못한다. 하지만 위 조건은 확률변수가 유한평균값을 가진다는 조건보다 정말 말 그대로 $\epsilon$ 만큼만 약한 조건이다. 구체적으로는 위의 조건이 만족하면 모든 $\epsilon>0$ 에 대하여 아래를 임플라이 할 수 있다. (증명은 이따가 소개할 Lemma 2.2.8. 을 이용해서 할 수 있다) 
\begin{align}
