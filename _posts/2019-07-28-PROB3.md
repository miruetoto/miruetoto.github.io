...
- ***Thm 2.2.6*** 과 ***Thm 2.2.7*** 는 확률변수의 $\frac{S_n}{b_n}$ 이 수렴하는 조건을 설명할 뿐이지 기대값으로 수렴하는 조건을 설명하는것은 아니다. 기대값으로 수렴함을 증명한것은 ***Thm 2.2.9*** 이다. 다만 $b_n$을 엄청 빠르게 무한대로 보낸다면 $\frac{S_n}{b_n}$ 을 거의 기대값에 가까운 값으로 수렴시킬 수 있다. 하지만 그래봤자 $b_n$이 $S_n$보다 압도적으로 너무 크면 $\frac{S_n}{b_n}$ 의 수렴값은 항상 $0$이 되어서 informative 한 결과가 나오지는 않는다. 
...
- ***i.i.d*** 경우에 한정하면 조건 **($\star$)** 는 조건 **($\tilde\star$)** 를 임플라이 한다. 그리고 $b_n$을 적절하게 선택하면 (정확히 말하면 $b_n$의 오더가 $o(n)$ 보다 크면 무조건 성립한다.) 조건 **($\star$)** 은 조건 **($\star\star$)** 를 임플라이한다. 

- 적절한 $b_n$을 선택하였을 경우 **($\star$)** 가 **($\star\star$)** 를 함을 다시 설명하여 보자. 조건 **($\star$)** 에 ***Lemma 2.2.8*** 을 사용하면 아래와 같은 식을 얻을 수 있다. (듀렛교재참고) 
\begin{align}
\sum_{k=1}^{n}E \bigg(\frac{X_{n,k}^2 1_{\|X_{n,k}\|\leq b_n} }{b_n^2} \bigg) 
\leq \int_{0}^{n/b_n} 2\frac{b_n^2}{n}\tau P\big( \|X_{n,1}\| \leq \frac{b_n^2\tau}{n} \big)
\end{align}
