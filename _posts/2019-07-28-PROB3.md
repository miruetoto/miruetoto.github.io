...
\end{align}
여기에서 $\mu_{n,k}:=E \big( w_ {n,k} X_k 1_ {\|w_{n,k}X_k\| \leq \sum_{k=1}^n w_{n,k} }\big)$ 이다. 우리가 흔히 아는 **WLLN** 의 경우 $w_{n,1},\dots,w_{n,n}=1$ 인 경우이다. <br/>
...
\end{align}
여기에서는 $\mu_{n,k}(b_n):=E\big(f_{n,k}(X_k) 1_ {\|f_{n,k}(X_k)\|\leq b_n } \big)$ 와 같이 정의한다. <br/>
**(4)** 그런데 $f_{n,k}(X_k):=X_{n,k}$ 로 간단하게 표현할 수 있다. 교재에서는 이러한 수열을 ***triangular arrays*** 라고 표현한다. 이러한 수열에 대하여 일반적인 **WLLN** 은 아래와 같이 쓸 수 있다. 
\begin{align}
