...
- 위의 조건 **($\tilde\star$), ($\star\star$)** 는 Chung의 교재스타일로 정리한것이다. Durret의 노테이션은 아래와 같다. 
**($\tilde\star$)** $\sum_{k=1}^{n} P\big(\|X_{n,k}\|>b_n\big) \to 0 $ <br/>
**($\star\star$)** $\sum_{k=1}^{n} E\big(\frac{X_{n,k}^2 1_{\|X_{n,k}\| \leq b_n} }{b_n^2}\big) \to 0$ <br/>
엄밀하게 말하면 동일한 $P$로 쓸수 없고 $P_k$로 써야한다. $E$역시 마찬가지이다. 하지만 간결함을 위해서 듀렛책은 이렇게 표현한것 같다. 
