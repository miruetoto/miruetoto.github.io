...
- 위의 모델을 좀 더 개신시킨것은 basis를 $[1,\cos x_i, \sin x_i, \cos 2x_i, \sin 2x_i, \dots, ]$ 과 같이 확장해본 것이다. 이때 각각의 basis는 오소고날하다. 

- 몇몇 사람들은 위의 2가지 방법은 각각 어떠한 가정을 내포한다. 가령 ''$y_i$는 $x_i$의 선형결합으로 만들어질 수 있다.'' 라든가 하는식으로 말이다. 즉 
\begin{align}
y_i=f(x_i)+\epsilon
\end{align}
와 같은 모델에서 $f$가 어떠한 형태를 가질것인지 미리 알고있다고 생각한다. 이처럼 $f$가 어떤 모양인지 미리 알고 접근하는 방법을 파라메트릭 모델이라고 한다. 

- $f$에 대한 사전정보가 없는 경우도 있는데 
