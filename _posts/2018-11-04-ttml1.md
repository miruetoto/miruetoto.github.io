...
- 통계학에서는 보통 디자인매트릭스를 $n \times p$와 같이 설계한다. 
- 선형차원축소의 행렬 ${\bf T}$를 사용하여 $z_i=x_i{\bf T}$와 같은 방식으로 쓸 수 있음. 보통 ${\bf T}$는 세로로 긴 행렬이 된다. 행렬이 세로로 긴 이후는 $z_i$의 차원이 $x_i$보다 작아야 하기 때문임. 
