...
따라서 2차원인 경우는 기저를 어떻게 잡아야 할까? 보통 아래와 같이 2가지 경우가 있다. 
  - $x$가 1차원때 basis를 적당히 확장하여 2차원 basis를 만드는 경우 
  - 2차원 basis를 바로 만드는 경우 
이때 1차원 basis를 적당히 확장하는 경우는 아래와 같은 예가 있을 수 있다. 
  - (multiplicative model) 1차원 basis가 $[1,x,x^2]$ 이라고 하면 각각 첫번째 변수 $x_1$ 
  - (additive model) 
