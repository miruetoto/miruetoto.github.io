...
- 지금까지의 논의를 잘 생각해보면 이것이 가능할 것 같다. 결국 <br/><br/>
**$X$를 확률변수라고 하자!!!**<br/><br/>
...
- 확률변수는 $\Omega \to \mathbb{R}$인 함수라고 생각할 수 있다. (물론 단순히 함수는 아니고 조금 특별한 성질을 추가로 가진다.) 확률변수는 realization이 스칼라인데 만약에 realization이 벡터라면 확률벡터가 된다. 만약에 realization이 길이 $m$의 벡터라면 이라면 따라서 확률벡터 ${\boldsymbol X}$는 ${\boldsymbol X}:\Omega \to \mathbb{R}^m$ 으로 정의할 수 있다. 
