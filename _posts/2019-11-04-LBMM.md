...
- $F_v:\mathbb{N} \to [0,1]$를 $V$의 distribution function이라고 한다. 따라서 $F_v(v)=\mu_v((-\infty,v])=P_v(\\{\nu:V(\nu)\leq v\\})$ 가 성립한다.

- 확률변수 $V$에 대한 probability density function 을 $f_v: {\cal M} \to \mathbb{R}^+$ 라고 정의하자. 이는 $F_v(v)$의 라돈니코딤 도함수로 정의된다. 벨킨의 논문에서는 이것을 $P:{\cal M} \to \mathbb{R}^+$ 로 표현하였다. 
