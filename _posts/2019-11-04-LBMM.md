...
에 관심을 가지자. 아래가 성립한다. 
\begin{align}
\Bigg \|\int_{\xi \in {\cal B} } e^{-\\|v -\xi \\|^2/4h}f(\xi))\mu(d\xi) - \int_{\xi \in {\cal B} } e^{-\\|v -\xi \\|^2/4h}f(\xi))\mu(d\xi) \Bigg \| \leq \mu({\cal M}-{\cal B}) \sup_{\xi \in {\cal M} }\big(\|f(\xi)\| \big)e^{-d^2/4t}
\end{align}
여기에서 $d=\inf_{\xi \in {\cal B} } \\|v-\xi\\|^2$ 
