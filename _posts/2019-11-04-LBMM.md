...
\end{bmatrix}
\end{align}
이고 $\beta=(\beta_0,\beta_1)'$ 이다. 이제 $H_0$를 검증하는 것은 $\beta_0=0$ 임을 검증하는 것과 동치가 되고 이는 위의 회귀모형에서 $\beta_0$가 유의한지 아닌지 검증하는것과 동치이다. 이를 위해서는 SSE가 카이제곱분포를 따르는지를 체크하면 되는데 SSE는 아래와 같이 표현가능하다. 
\begin{align}
{\bf f}'({\bf D}-{\bf W}){\bf f}
\end{align}
