...
- 부피를 재는 메져를 $\mu$ 라고 정의하자. 즉 ${\cal M}$ 에 존재하는 임의의 sub-manifold ${\cal M}^* $ 에 대하여 아래가 성립한다. 
\begin{align}
\mbox{vol}((\cal M}^* )= \int_{v \in (\cal M}^* } d\mu(v)
\end{align}
일반적으로 $\mu$ 는 canonical uniform measure 라고 정의한다. 다만 나중에는 $\mu$를 매니폴드 ${\cal M}$ 를 서포트로 가지는 probability measure 로 정의할 수도 있다. 
