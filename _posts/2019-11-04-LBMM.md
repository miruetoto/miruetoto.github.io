...
- 증명을해보자. 여기에서 
\begin{align}
\frac{1}{n} {\cal L}f(v) = \frac{1}{t_n (4\pi t_n)^{\kappa/2}} \sum_{j=1}^{n}e^{-\\|v-v_j\\|^2/4t_n}(f(v)-f(v_j))
\end{align}
와 같이 쓸 수 있다. 아래가 성립한다. 
\begin{align}
\mathbb{E} \frac{1}{n} {\cal L}f(v) = \frac{1}{t_n (4\pi t_n)^{\kappa/2}} \sum_{j=1}^{n}e^{-\\|v-v_j\\|^2/4t_n}(f(v)-f(v_j))
\end{align}
