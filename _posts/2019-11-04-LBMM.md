...
\end{align}
를 고려하였다. 참고로 $\kappa$-dimensional 정규분포의 앞에 붙은 factor는 
\begin{align}
\frac{1}{(2\pi)^{\kappa/2}|\Sigma|} 
\end{align}
이다. 여기에서 
\begin{align}
\frac{1}{t_n (4\pi t_n)^{\kappa/2}} {\cal L}f(v) = \frac{1}{t_n (4\pi t_n)^{\kappa/2}} \sum_{j=1}^{n}e^{-\\|v-v_j\\|^2/4t_n}(f(v)-f(v_j))
\end{align}
와 같이 쓸 수 있다. 
