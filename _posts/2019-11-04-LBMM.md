...
**(1)** hst를 사용해서 무한번 눈을 쌓으면 그래프를 매니폴드 위로 옮기는 변환을 수행하는 셈이 된다. 이 변환은 합리적이다. 합리적이라는 말은 전통적으로 그래프이론에서 두 노드간의 거리를 재는 메져와 snow-dist가 isometric 하다는 의미이다. 최소한 레귤러 그래프에서는 최단거리, snowdist, 저항거리, 확산거리가 모두 비례관계에 있음을 증명할 수 있다. <br/>
**(2)** 일반적인 유클리드 공간에 존재하는 점들 $f_1,\dost,f_n$의 거리를 측정하여 하나의 매트릭스 ${\bf W}$ 로 저장하였다고 하자. 신호 ${\bf f}$ 의 거리를 ${\bf W}$ 라고 하고 그 위에 눈을 한번 쌓자. 눈을 한번 쌓은뒤의 거리를 ${\tilde \bf W}$ 라고 하자. 이때 ${\bf W}$에서 ${\tilde \bf W}$로 가는 호모토픽한 함수가 존재한다. (그래서 ${\tilde \bf W}$ 는 ${\bf W}$와 동치관계에 있다) <br/>
**(3)** 구조가 없는 자료
