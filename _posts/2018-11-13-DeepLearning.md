...
- 제너레이터 관점에서 보면 $D(G((z_i))$값들이 커야지 학습이 잘 된 것이다. 즉 $1-D(G(z_i))$값들이 작을수록 학습이 잘 되었다고 볼 수 있다. 따라서 제너레이터는 아래의 함수를 최소화 하고 싶어한다. 
\begin{align}
\frac{1}{n}\sum_{i=1}^{n} \log \left( 1- D(G(z_i)) \right) 
\end{align}

- 
반면에 디스크리미네이터는 $D(x_i)$의 값들이 작을 수록 학습이 잘 되었다고 볼 수 있다. 
