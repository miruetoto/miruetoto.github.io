...
- 제너레이터 관점에서 보면 $D_{\bf \theta_d}(G_{\bf \theta g}(z_i))$값들이 커야지 학습이 잘 된 것이다. 즉 제너레이터는 아래식을 만족하는 $\bf \theta_d$를 찾고 싶어한다. 
\begin{align}
\underset{\bf \theta_d}{\operatorname{argmin}} D_{\bf \theta_d}(G_{\bf \theta g}(z_i))
\end{align}

$1-D(G(z_i))$값들이 작을수록 학습이 잘 되었다고 셍각할수도 있다. 따라서 제너레이터는 고정된 함수 $D^{(0)}$에 대하여 아래의 함수를 최소화하고 싶어한다. 
\begin{align}
