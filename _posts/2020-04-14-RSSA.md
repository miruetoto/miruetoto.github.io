...
\end{align}
is not in ${\cal M}_ {L,K}^{(H)}$. The components of ${\bf X}$, i.e., $\hat{\bf X}_ 1, \dots , \hat{\bf X}_ m$, are reside in ${\cal M}_ {L,K}$. 

- Thus we need to introduce something new transform which maps element of ${\cal M}_ {L,K}$ to ${\cal M}_ {L,K}^{(H)}$. To do this, they introduce matrix $\Pi^{(H)}$. 
...
- Denote $\Pi^{(H)}:{\cal M}_ {L,K} \to {\cal M}_ {L,K}^{(H)}$ the orthogonal projection on ${\cal M}_ {L,K}^{(H)}$ in Frobenius norm. Transper ${\bf Y}$ from ${\cal M}_ {L,K}$ to ${\cal M}_ {L,K}^{(H)}$ by $\Pi^{(H)}$, i.e., calculate $\Pi^{(H)}{\bf Y}$. Thus I interprete $\Pi^{(H)}$ as hatmatrix which project 

Note that $\Pi^{(H)}{\bf Y}$ also has Hankel like structure just as ${\cal T}(\mathbb{Y})$. 