...
- Thus basic process of reconstruction is as follows: (1) for all $k=1,\dots, m$, get ${\bf \tilde X}_ k = \Pi^(H){\bf \hat X}_ k$, (2) apply inverse transform of imbedding in $\bf{\tilde X_k}$, i.e., get $\tilde{\mathbb X}_ k={\cal T}^{-1} (\bf{\tilde X_k})$. 
