...
- **(proof)** 증명은 그렇게 어렵지 않다. **우선 언급된 첫번째 방법과 두번째 방법이 같음을 보이자.** 첫번째 방법으로 구한 $\tilde{\beta_2}$는 아래와 같이 표현가능하다. 
\begin{align}
\tilde{\beta_2}=(X_2'(I-H_1)X_2)^{-1}X_2'(I-H_1)^2{\bf y}
\end{align}
이때 $(I-H_1)^2{\bf y}$이 성립하므로 **언급된 첫번째 방법과 두번째 방법은 같은 방법이다.** 이제 $\tilde{\beta_2}=\hat{\beta_2}$임을 보이자. 즉 $\hat{\beta_2}$는 아래임을 보이자. 
\begin{align}
