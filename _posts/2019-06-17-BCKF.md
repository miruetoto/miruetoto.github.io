...
참고로 $\bf X_2'X_1(X_1'X_1)^{-1}$ 은 과정 (2) 에서 구해지는 coef 에서 transpose 를 취한 것이다. 좌변과 우변을 각각 계산하면 다음을 얻는다. 
\begin{align}
(LHS):
...
\end{align}
위의 계산에서 **$\bf X_1$과 과정 (2) 에서의 잔차 $\bf e_{X_2|X_1}$이 직교한다는 이용하면 즉 $\bf X_1'(I-H_1)X_2 =0$ 를 이용** 하여야 한다. 아무튼 위의 계산을 거치면 주어진 매트릭스를 아래와 같이 정리가능하다. 
\begin{align}
