...
\begin{align}
\omega \in \big\\{\frac{2 \pi k}{\zeta}, k \in \mathbb{Z}\big\\}
\end{align}
혹은 
\begin{align}
\omega \in \big\\{\frac{2 \pi k}{\xi}, k \in \mathbb{Z}\big\\}
\end{align}
에서만 존재한다는 점이다. 또한 이산신호의 경우 $x_i$의 주기가 $\xi$이면 $x(\omega)$의 주기역시 $\xi$ 라는점 역시 주목할만한 부분이다. 
