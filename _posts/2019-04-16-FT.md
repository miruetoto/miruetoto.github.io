...
\end{equation}
주목할것은 주기가 $\zeta$ 혹은 $\xi$ 인 함수의 주파수 응답은 오로지 $\big\\{\frac{2 \pi k}{\zeta}, k~\in \mathbb{Z} \big\\}$ 혹은 $\big\\{\frac{2 \pi k}{\xi}, k~\in \mathbb{Z} \big\\}$에서만 존재한다는 점이다. 또한 이산신호의 경우 $\big\\{\frac{2 \pi k}{\xi}, k~\in \mathbb{Z} \big\\}$에서 존재하는 값 주기가 $\xi$ 라는 것 역시 주목할만한 부분이다. 
