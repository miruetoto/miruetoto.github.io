...
\begin{align}
\left\\{\frac{2 \pi k}{\zeta}, k~\in \mathbb{Z} \right\\}
\end{align}
혹은 
\begin{align}
\left\\{\frac{2 \pi k}{\xi}, k~\in \mathbb{Z} \right\\}
\end{align}
에서만 존재한다는 점이다. 또한 이산신호의 경우 
\begin{align}
\left\\{\frac{2 \pi k}{\xi}, k~\in \mathbb{Z} \right\\}
\end{align}$에서 존재하는 값 주기가 $\xi$ 라는 것 역시 주목할만한 부분이다. 
