...
- 아래는 Liu(2018)에 수록된 그림이다. 이 그림은 $\xi$에 따라 알고리즘이 적절한 $(\alpha,w)$를 빠르게 찾을수도 느리게 찾을수도 있음을 보여준다. 초기값은 $(\alpha,w)=(2,-2)$이고 최적해는 $(1,1)$에 위치한다. 서로 다른 $\xi$에 따라 최적해에 접근하는 경로가 제각각이다. 이 경우 $\xi=0.5$일 경우(초록색실선) 가장 잘 수렴하는듯 보인다.
