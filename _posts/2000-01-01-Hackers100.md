...
```
이 맞는 문장도 아니니, 이 경우는 정보를 연결하기 까다롭다. 이는 본디 두 정보가 `대등`하지 않고 한 정보가 다른정보에 `종속`되어있기 때문이다. 여기에서는 `수영을 한다`라는 문장이 그녀의 취미를 설명하는 문장에 종속되어있다. 참고로 아래의 두 문장은 두 정보가 대등한 경우이다. 
```
I love you. You love me.
```
