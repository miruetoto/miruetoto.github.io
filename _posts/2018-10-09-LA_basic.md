...
\begin{align}
{\bf A}{\bf B}=\Big[A[,1],\dots A[,p]\Big] \begin{pmatrix} B[1,] \\\\ \dots \\\\ B[p,]\end{pmatrix} = \sum_{j=1}^{p} A[,j]B[j,].
\end{align}
...
\begin{align}
{\bf A}{\bf B}=\begin{pmatrix}A[1,] \\\\ \dots \\\\ A[n,]\end{pmatrix} {\bf B} = \begin{pmatrix}A[1,]{\bf B} \\\\ \dots \\\\ A[n,] {\bf B}\end{pmatrix} .
\end{align}
이처럼 ( )로 묶인 행렬표현에서는 뒤쪽에 행렬들을 계속 곱해도 상관없다. <br/><br/>
**(4)** 행렬곱을 아래과 같이 "matrix $\times$ cols"의 꼴로 표현할 수도 있다. 
\begin{align}
{\bf A}{\bf B}={\bf A} \Big[B[,1] \dots,B[,q]\Big] = \Big[{\bf A}B[,1],\dots,{\bf A}B[,q]\Big]. 
\end{align}
이처럼 [ ]로 묶인 행렬표현에서는 앞쪽의 행렬들을 계속 곱해도 상관없다. <br/><br/>
