...
와 같은 기호로 표시한다. 대각행렬의 경우 임의의 $n \times p$ 행렬 ${\bf C}$와 $p \times q$ 행렬 ${\bf E}$에 대하여 아래의 식이 성립함을 기억하면 편리하다. (좀 헷갈릴 수 있는데 그냥 외우는게 속편함) 
**(1)** ${\bf A}{\bf D}=cbind({\bf A}_ 1, \dots, {\bf A}_ p)diag(\lambda_1,\dots,\lambda_p)=cbind(\lambda_1{\bf A}_ 1,\dots,\lambda_p {\bf A}_ p)$.  
