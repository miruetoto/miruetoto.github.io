...
\end{align}
와 같이 세로로 긴 대각행렬을 다루는 경우가 많다. 물론 ${\bf D}_ {2 \times 3}=rbind(c(\lambda_1,0,0),c(0,\lambda_2,0))$와 같이 가로로 긴 대각행렬을 정의할 수도 있다. 아무튼 두 경우 모두 편의상 $diag(\lambda_1,\lambda_2)$로 표현가능하다. 그리고 이 경우에는 행렬곱을 $min(n,m)$까지만 전개하면된다. 예를들면 아래와 같은 식으로 말이다. 
\begin{align}
