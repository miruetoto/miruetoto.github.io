...
\begin{align}
{\bf V}=cbind(\bf v_1, v_2, \dots, v_n)
\end{align}
즉 따라서 정리하면 아래와 같은 표현 모두 가능하다. 
\begin{align}
{\bf X}_{n\times p}=cbind(X_1, X_2, \dots, X_p)=cbind(\bf x_1, x_2, \dots, x_p)
\end{align}
\begin{align}
{\bf V}_{n\times n}=cbind(V_1, V_2, \dots, V_n)=cbind(\bf v_1, v_2, \dots, v_n)
\end{align}
다만 가급적이면 $cbind(\bf x_1, x_2, \dots, x_p)$이라든가 $cbind(V_1, V_2, \dots, V_n)$이런 표현은 어색하니 지양하자. 
