...
- **(이인석교수님 스타일의 정의)** 임의의 행렬 ${\bf X}_ {n \times p}$, ${\bf T}_ {p \times q}$에 대하여 행렬곱 ${\bf X}{\bf T}$의 각 원소를 아래와 같이 정의한다. 
\begin{align}
({\bf X}{\bf T})=\left( \sum_{k=1}^{p} x_{ik}t_{kj} \right) ,\textup{ for } i=1,2,\dots,n \textup{ and } j=1,2,\dots,q 
\end{align}
