...
- 우리의 목적은 어떠한 ***estimator*** 가 가지는 성질을 규명하기 위함이다. 특히 극한적 성질을 규명할 일이 많은데 이때 estimator 를 만들어 내는 $X_1,\dots,X_n$ 의 극한적 성질을 다루기 보다 $F_n$의 극한적 성질을 다루는 것이 더 편리하다. (글리벤코-칸텔리 정리와 같은것을 쓸 수 있기 때문) 그래서 우리가 관심있는 estimator를 아래와 같은 ***functional*** 의 형태로 표현한다. 
\begin{align}
T:{\cal T} \to \mathbb{R} 
\end{align}
여기에서 $\cal T$는 ***random element*** 들이 정의되는 공간 $(\Omega,{\cal F})$ 에서 정의가능한 모든 ***singed measure*** 들의 집합 혹은 그것의 ***convex subset*** 이다. (응용통계 강의노트, 헴펠 P.82) 
