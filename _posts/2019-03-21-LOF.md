...
\end{align}
여기에서 $p^{(2)}$가 $o_1$인지 $o_{-1}$인지 아니면 둘다 인지 신경쓰지 않도록 하자. $p^{(3)}$역시 마찬가지이다. 논문에서는 특정 datapoint $p^{(2)}$, $p^{(3)}$과 같은 것은 정의하지도 않았다. 대신에 그들과의 거리 즉 $\\|p-p^{(2)}\\|$와 $\\|p-p^{(3)}\\|$를 $2-dist(p)$ 혹은 $3-dist(p)$와 같은 방식으로 정의했다. 
