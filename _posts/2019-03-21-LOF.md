...
(1) $k$를 고정한다. <br/>
(2) 점 $p$를 중심으로 점선원을 펼치고 점 $p$의 1차이웃을 구한다. 이 이웃들을 $q_1,\dots,q_k$라고 하자. <br/>
(3) $q_1,\dots,q_k$를 중심으로 다시 점섬원을 펼친다. 이때 점선원들의 반지름은 각각 다를것이다. <br/>
(4) 각각의 $q_1,\dots,q_k$를 기준으로 하여 $p$와의 RD를 구하여 이 값들을 평균낸다. <br/> 
