...
- **(Definition 3)** 여기에서는 $k-distance(p)$라는 것을 정의한다. (아니진짜 기호를 왜 이따위로 정의하는거야?? $k$ **마이너스** $distance(p)$ 같잖아!!)  $k-distance(p)$의 의미는 **"점 $p$"와 "$p$로부터 $k$번째로 가까운 점"과의 거리**를 의미한다. $p$와 가장 가까운 점은 자기자신이므로 $k=1$이면 $k-distance(p)=0$이다. 즉 모든 경우에 클리어하게 아래가 정의된다. 
\begin{align}
