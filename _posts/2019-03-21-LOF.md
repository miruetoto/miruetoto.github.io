...
\end{align}
여기에서 ${\bf x}^{(k)}$는 훈련표본 $\\{ {\bf x}_ i\\}_ {i=1}^{n}$중에서 ${\bf x}$와 $k$번째로 가까운 표본을 나타낸다. $k$가 작으면 일반적인 유클리드 거리처럼 동작한다. $k$가 크면 대부분의 자료들이 똑같이 떨어져있다고 생각하게 된다. 극단적으로 $k$가 무한대라면 모든자료들이 똑같은 거리만큼 떨어져있다고 생각한다. 즉 여기에서 $k$는 multiscale 파라메터라고 해석할 수 있다. 
