...
- 아래 그림에서 점선으로 된 원은 중심이 $o$이고 반경이 $4.distance(o)$이다. 이 점선원의 내부에 위치한 점들은 점 $o$와의 $reach.dist_{k}$가 w점선원의 반지름값 즉 $4.distance(o)$의 값과 같다. 그리고 이 점선원의 외부에 위치한 점들과 $o$와의 $reach.dist_{k}$는 그냥 유클리드 거리와 같다. 
