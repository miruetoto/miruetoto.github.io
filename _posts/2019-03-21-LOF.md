...
- 논문의 Fig 1 에서 $o_2$는 아웃라이어로 판단하고 클러스터 $C_1$에 존재하는 다른 모든점들은 아웃라이어가 아니라고 판단하는 것이 우리의 목표이다. (즉 $o \in C_1$ 이면 $LOF_k(o) \approx 1$이 성립하게끔 하는것) 우리가 정의한 LOF가 과연 이러한 성질을 만족할까? Lemma1은 $C_1$에 존재하는 모든 점들은 아웃라이어가 아니라는 결론을 주는 레마이다. 
