...
\end{align}
여기에서 ${\bf y}$는 $n\times 4$ 인 행렬이고 ${\bf X}$는 $n \times 8T$ 인 행렬이며 $\bf \beta$는 $8T \times 4$ 행렬이다. 오차항 $\epsilon$은 $n \times 4$행렬이다. 따라서 $n$개의 관측치를 모으기만 하면 쉽게 $\beta$를 구할수 있다. 여기에서 $\beta$가 아래와 같이 구성되어있다는 것을 주의하자. 
\begin{align}
\beta=\begin{bmatrix}
A_1 \\\\ \\\\ 
\dots \\\\ \\\\ 
