...
- 가장 흔하게 분산이 변하는 경우는 평균이 커짐에 따라서 분산도 제곱으로 같이 커지는 경우이다. 즉 아래와 같은 경우이다. 
\begin{align}
V(Z_t)=c\mu_t^2
\end{align}
여기에서 $c$는 양수라고 가정하자. 
...
- 결론: $V(Z_t)=c\mu_t^2$꼴의 형태의 시계열인 경우 
\begin{align}
V\big(\log(Z_t)\big)\approx \mbox{const}
\end{align}
가 된다. 
