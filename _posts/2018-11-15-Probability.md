...
X \in {\cal F}
\end{align}
인지를 따지기만 하면된다. 따라서 앞으로 논의하게 될 수렴등 확률변수가 가지는 일반적인 성질을 따질때는 $P$는 고려하지 않는다. 

- 사실 $P$는 $X$가 가지는 분포를 결정하여 주므로 $P$가 정의되면 부수적으로 $X$가 가지는 여러가지 성질들을 추가적으로 **assume**할 수 있다. 예를들면 $E|X|<\infty$라든가 하는 식으로 말이다. 
