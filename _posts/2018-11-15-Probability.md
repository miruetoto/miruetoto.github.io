...
- 요런 느낌의 정의를 확률론에서는 많이 가지고 있다. 가령 예를들면 시그마필드 ${\cal F}$는 전체집합 $\Omega$ 없이는 정의 될 수 없다. 
...
- 보렐셋이라는 것은 $\mathbb{R}$에서 크기를 잴 수 있는 집합을 의미한다. 좀 더 일반적으로 **"어떠한 시그마필드의 메저러블셋"**은 (반복하여 말하지만 확률변수 $X$가 홀로 정의될 수 없듯이 메저러블셋도 시그마필드없이는 홀로 정의될 수 없음. 그리고 시그마필드는 전체집합 $\Omega$없이는 홀로 정의될 수 없음) 
