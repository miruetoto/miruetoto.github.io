...
- $X \in {\cal F}$라고 하자. $\sigma(X)$는 $X$를 ${\cal F}$-메저러블 하게 만드는 가장 작은 시그마 필드이다. 예를들면 
\begin{align}
X(\\{1\\})=X(\\{3\\})=X(\\{5\\})=1 
\end{align}
\begin{align}
...
와 같은 경우에서 $X$는 분명히 
\begin{align}
{\cal F}_1=\\{\emptyset, \\{1,3,5\\}, \\{2,4,6\\},\Omega \\}.
\end{align}
에 대하여 메저러블 하다. 즉 $X \in {\cal F}_1$이다. 하지만 $X$는 
\begin{align}
{\cal F}_2=2^{\Omega}
\end{align}
에 대하여서도 메저러블 하다. 따라서 $X \in {\cal F}_2$이기도 하다. 
