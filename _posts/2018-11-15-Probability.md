...
- 다시 $X$로 관심을 돌려보자. $X$는 기본적으로 $\Omega$와 $\mathbb{R}$을 연결하는 맵핑이다. 이것이 $(\Omega,{\cal F})$와 $(\mathbb{R},{\cal R})$을 연결하는 맵핑, 즉 메저러블 맵핑이 되려면 어떻게 되어야 할까? 해답은 매우 간단하다. $\Omega$의 부분집합중에서 $\cal F$에 속하는 집합에 대해서만 맵핑값을 정의하면 된다. 예를들어 $\Omega=\\{1,2,3,4,5,6\\}$이라고 하자. 즉 주사위를 던져서 나오는 눈들의 집합이 $\Omega$이다. $\Omega$에 대한 시그마필드를 아래와 같이 정의하자. 
\begin{align}
{\cal F}=\\{\emptyset, \\{1,3,5\\}, \\{2,4,6\\},\Omega \\}.
\end{align}
즉 $\cal F$의 원소는 1) 공집합 2) 전체집합 그리고 3) 주사위를 던져서 짝수가 나오는 경우를 모은 집합 4) 주사위를 던져서 홀수가 나오는 경우를 모은 집합이다. $X$가 단지 $\Omega$에서 $\mathbb{R}$의 맵핑이 아니라 $(\Omega,{\cal F})$에서 $(\mathbb{R},{\cal R})$으로 맵핑이라는 것은 



즉 $\cal F$에서 정의된 어떠한 확률변수 $X$는 $\cal F$에 포함된 모든 근원사건들을 **모순없이** 연결하는 느낌을 준다. 
