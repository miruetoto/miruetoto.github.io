...
- 확률변수는 사실 메저러블-맵핑중에서 매우 특수한 경우인데 이것은 메저러블-스페이스 $(S,{\cal S})=(\mathbb{R},{\cal R})$로 고정되어있기 때문이다. 여기에서 $\cal R$은 보렐클라스이다. 아마 나는 이 포스팅을 읽을때마다 보렐클라스가 무엇인지 다시 헷갈리기 시작할것인데 (나 자신에 대한 끝없는 불신..) 그래서 정리했다. $\cal R$ 보렐클라스란 $\mathbb{R}$의 모든 오픈셋을 포함하는 시그마필드라는 것이다. 좀 더 엄밀하게 써보자. 집합 $O$를 $\mathbb{R}$에 속하는 임의의 오픈셋이라고 하자. 즉 <br/><br/>
*** $O \subset \mathbb{R}$ and $O$ is open set*** <br/><br/>
이라고 하자. 이러한 집합 $O$들을 모아놓은 클라스를 $\cal O$를 정의하자. 즉 
\begin{align}
{\cal O}=\\{O: O \subset \mathbb{R} ~ and ~ O~ is ~ open set \\}
\end{align}
