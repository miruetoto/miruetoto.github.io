...
- 확률변수는 사실 메저러블-맵핑중에서 매우 특수한 경우인데 이것은 메저러블-스페이스 $(S,{\cal S})=(\mathbb{R},{\cal R})$로 고정되어있기 때문이다. 여기에서 $\cal R$은 보렐클라스이다. 아마 나는 이 포스팅을 읽을때마다 보렐클라스가 무엇인지 다시 헷갈리기 시작할것인데 (나 자신에 대한 끝없는 불신..)
