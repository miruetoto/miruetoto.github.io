...
라고 말하는것이 올바른 정의라는 것이다. 요런 느낌의 정의를 확률론에서는 많이 가지고 있다. 가령 예를들면 시그마필드 ${\cal F}$는 홀로 정의될 수 없고 전체집합 $\Omega$가 존재해야지 정의된다. 그리고 메저러블셋은 시그마필드 $\cal F$가 있어야지 정의될 수 있다. 즉 아래와 같이 말한다. 
...
- 보렐셋 즉 $\cal R$-메저러블한 집합은 $\mathbb{R}$에서 크기 혹은 길이를 잴 수 있는 집합을 의미한다. 가령 예를 들면 $[0,1]$의 길이를 1이라고 하고 $\emptyset$의 길이를 0이라고 하자. 그러면 $[0,0.5]$와 같은 집합의 길이는 $0.5$, $\\{0\\}$와 같은 집합의 길이는 $0$, 유리수들의 집합의 길이는 $0$, 그리고 $[0,1]$사이에 포함된 무리수들의 집합을 길이로 재면 $1$과 같은식으로 정의할 수 있다(이러한 방식으로 길이를 재는 메저를 **르벡메저**라고 함). 참고로 르벡메저는 $\cal R$에 속한 임의의 집합에 대하여서도 이러한식으로 **모순없이** 길이를 정의할 수 있다. 여기에서 모순없다는 것은 1) 

에 길이를 잴 수 없다는 말은 

- 좀 더 일반적으로 **"어떠한 시그마필드 $\cal F$의 메저러블셋 $A$"**은 (반복하여 말하지만 확률변수 $X$가 홀로 정의될 수 없듯이 메저러블셋도 시그마필드없이는 홀로 정의될 수 없음) $\cal F$에서 젤 큰 집합 $\Omega$의 크기보다 작고 $\empty$의 크기보다 큰 어떠한 크기값으로 "잘"잴 수 있는 집합들이다. 이 논리를 그대로 이용하면 보렐셋이 아니라는것은 보렐클라스 $\cal R$에 대하여 
