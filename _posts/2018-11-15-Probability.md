...
- 보렐셋 즉 $\cal R$-메저러블한 집합은 $\mathbb{R}$에서 크기 혹은 길이를 잴 수 있는 집합을 의미한다. 가령 예를 들면 $[0,1]$의 길이를 1이라고 하고 $\emptyset$의 길이를 0이라고 하자. 그러면 $[0,0.5]$와 같은 집합의 길이는 $0.5$, $\\{0\\}$와 같은 집합의 길이는 $0$, 유리수들의 집합의 길이는 $0$, $[0,1]$사이에 포함된 무리수들의 집합을 길이로 재면 $1$과 같은식으로 정의할 수 있다(이러한 방식으로 길이를 재는 메저를 ***르벡메저***라고 함). 참고로 르벡메저는 $\cal R$에 속한 임의의 집합에 대하여서도 이러한식으로 모순없이 길이를 정의할 수 있다. 
