...
\end{align}
와 같은 느낌으로 기억하면 된다. 왜 이렇게 가정하는것일까? 결국 우리가 관측한 자료 $x_1,\dots,x_n \| \mu,\sigma^2$ 의 분산은 $\sigma^2$ 자체의 값과 $\mu$의 분산 $V(\mu\|\sigma^2)$의 값에 영향을 같이 받게 된다. 예를 들어서 관측된자료 $x_1,\dots,x_n$ 이 있다고 치자. 표준편차 $s^2$을 계산해보니 1에 가까웠다고 하자. 하지만 이것은 아래의 경우에 모두 가능한 상황이다. 
\begin{align}
\begin{cases}
V(\mu\|\sigma^2)=1 \\\\ \\
x_1,\dots,x_n \| \mu, \sigma^2=0 
\end{cases}, \quad 
\begin{cases}
V(\mu\|\sigma^2)=0 \\\\ \\
x_1,\dots,x_n \| \mu, \sigma^2=1 
\end{cases}.
\end{align}
극단적이긴 했지만 모든 케이스들은 이러한 두 양근단의 사이일 것이다. 즉 적당한 값의 $V(\mu\|\sigma^2)$ 와 $\sigma^2$ 으로 이루어졌을 것인데 이것을 결정하기위해서 
\begin{aling}
\frac{\nu V(\mu\|\sigma^2}{\sigma^2}:=\sum_{i=1}^{\nu}Z_i^2
\end{align}
와 같은 가정이 필요하다. 이때 $\nu$의 역할은 무엇일까? 어차피 자료의 분산을 해석할때 $V(\mu\|\sigma^2)$와 $\sigma^2$의 가중합으로 표현할텐데 이때 둘사이의 가중치를 조정하는 역할을 한다. 
