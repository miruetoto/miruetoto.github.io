...
\end{align}
뭐 저렇게만 보면 $\mbox{likelihood} \times \mbox{prior}$ 가 엄청난 의미를 가지고 있는것 같은데 그냥 $({\boldsymbol x},\theta)$의 ***joint pdf*** 란 의미다. 여기에서 ${\boldsymbol x}=(x_1,\dost,x_n)'$ 이다. 따라서 아래는 그냥 다 같은소리다. 
...
\end{align}
이 식에 따르면 사후분포는 다음의 과정으로 구할 수 있다. **(1)** join를($=\mbox{likelihood} \times \mbox{prior}$)를 쓰고 **(2)** $\{x_1,\dots,x_n\}$ 에 관련된텀은 모두 무시하고 $\lambda$와 관련된텀만 남긴뒤 **(3)** 간추린모양을 보고 ***pdf*** 를 역추론해 때려맞춘다. 위의 예제로 치면 아래와 같이 joint를 쓴뒤 
\begin{align}
f(x_1,\dots,x_n,\lambda)= \left( \prod_{i=1}^{n} \frac{\lambda^{x_i} e^{-\lambda} }{x_i !} \right) \frac{\lambda^{\alpha-1}e^{-\lambda/\beta} }{\Gamma(\alpha)\beta^{\alpha} }
\end{align}
정리해서 아래와 같이 **간추린형태** 를 만들고 
\begin{align}
f(x_1,\dots,x_n,\lambda)= \lambda^{\sum x_i} e^{-n\lambda} \frac{\lambda^{\alpha-1}e^{-\lambda/\beta} }{\Gamma(\alpha)\beta^{\alpha} }
\end{align}
sdf
