...
\end{cases}
\end{align}
둘다 다소 극단적이긴 했지만 모든 케이스들은 이러한 두 양근단의 사이일 것이다. 다시 말하면 $X_1,\dots,X_n \| \mu,\sigma^2$의 평균을 모델링하는 것과 달리 분산을 모델링하는 일은 $V(\mu\|\sigma^2)$와 $\sigma^2$ 모두 고려해야해서 ***identifiable*** 하지 않다. 그럼 어떻게 결정을 해야할까? 실제모델은 저 두 경우 중 사이값일텐데 그 중 무엇일까? 한 가지 방법은 
\begin{align}
...
\end{cases}
\end{align}
여기에서 $\kappa$는 표본이 많아질수록 점점 커저야하는 수이다. 따라서 마치 $n$과 같은 개념으로 사용할 수 있다. 

- 실제로 $\kappa$는 거의 $n$과 비슷하게 생각해도 된다. 그 예로 모수 $\mu$의 포스테리어는 아래와 같이 계산가능하다. 
\begin{align}
...
\begin{align}
\frac{\mu\|x_1,\dots,x_n,\sigma^2-E(\mu\|x_1,\dots,x_n,\sigma^2)}{\sqrt{\sigma^2/(n+\kappa)} }
\end{align}
