...
\mathbb{E}_ {\lambda}(\lambda-\hat\lambda)^2
\end{cases}
\end{align}
위의식은 predictor에 대한 MSE의 정의이고 아래식은 estimator에 대한 MSE의 정의이다. 다소 헷갈리지만 그냥 **회귀식정의**와 **수통식정의** 2개 있다고 생각하는 것이 속 편하다. 우선 우리는 회귀식정의에는 관심이 없다. 


- 수통식정의의 황당한 점은 빈도주의자와 베이지안이 생각하는 정의가 다르다는 것이다. 우선 빈도주의자들은 MSE를 아래와 같이 정의한다. 
\begin{align}
\mathbb{E}_ {\lambda}(\lambda-\hat\lambda)^2:=\int (\lambda-\hat{\lambda})^2 L(\lambda) dx_1,\dots dx_n =\int (\lambda-\hat{\lambda})^2 f(x_1,\dots,x_n ; \lambda) dx_1,\dots dx_n 
\end{align}
베이지안은 MSE를 아래와 같이 생각한다. 
\begin{align}
\mathbb{E}_ {\lambda}(\lambda-\hat\lambda)^2:=\int \int (\lambda-\hat{\lambda})^2 f(\lambda,x_1,\dots,x_n) dx_1,\dots dx_n d\lambda
\end{align}

- 아래의 수통식정의를 자세히 살펴보자.   
\begin{align}
\mathbb{E}_ {\lambda}(\lambda-\hat\lambda)^2:=\int (\lambda-\hat{\lambda})^2 f(x_1,\dots,x_n ; \lambda) dx
\end{align}
이걸 자세히보면 지독하게도 빈도주의적 관점임을 볼 수 있다. 베이지안 이었다면 우도를 $f(x_1,\dots,x_n \| \lambda)$ 처럼 썻을 것이다. 즉 MSE 자체가 베이지안들이 쓰는 용어가 아니다. 실제로 베이지안들은 MSE라는 표현을 쓰지 않는다. (베이즈리스크라고 말함..) 만약에 베이지안들이 MSE라는 단어를 썼다면 이는 회귀식 MSE를 뜻하는 것일 확률이 높다. 말이 나온김에 하는 말인데 $\mathbb{E}_{\lambda}$ 역시 베이지안들이 즐겨서 쓰는 노테이션은 아니다. 이것은 확률로 우도함수를 사용한 평균을 의미한다. 

- 수통식으로 정의한 MSE는 빈도주의자들에게 위험함수라고 불린다. 그리고 $(\lambda-\hat{\lambda})^2$ 를 손실함수라고 한다. 따라서 빈도주의자들에게는 위험함수, 즉 수통식 MSE는 손실함수의 평균으로 기억된다. 즉 빈도주의자들에게는 아래의 식이 머리속에 있다. 
\begin{align}
\mbox{MSE}=\mbox{risk}=\mbox{expected loss}
\end{align}
그리고 

- 반대로 베이지안역시 MSE개념 자체가 희박하다. 

- 이게 진짜 괴로운게 비통계학자중 머신러닝 계열만 주구장창 다룬사람들은 수통식MSE 개념자체가 없다. 그래서 빈도주의자들이든 베이지안이든 
(이게 진짜 괴로운게 머신러닝계열에서는 )
