...
\end{align}
와 같은 느낌으로 기억하면 된다. 왜 이렇게 가정하는것일까? 결국 우리가 관측한 자료 $x_1,\dots,x_n \| \mu,\sigma^2$ 의 분산은 $\sigma^2$ 자체의 값과 $\mu$의 분산 $V(\mu\|\sigma^2)$의 값에 영향을 같이 받게 된다. 예를 들어서 관측된자료 $x_1,\dots,x_n$ 이 있다고 치자. 표준편차 $s^2$을 계산해보니 1에 가까웠다고 하자. 하지만 이것은 $V(\mu\|\sigma^2)=1$, $x_1,\dots,x_n \| \mu, \sigma^2=0$ 이어도 가능하고, $V(\mu\|\sigma^2)=0$, $x_1,\dots,x_n \| \mu, \sigma^2=1$ 이어도 가능
