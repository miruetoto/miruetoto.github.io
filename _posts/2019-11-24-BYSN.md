...
\end{align}
와 같은 느낌으로 기억하면 된다. 왜 이렇게 가정하는것일까? 결국 우리가 관측한 자료 $x_1,\dots,x_n \| \mu,\sigma^2$ 의 분산은 $\sigma^2$ 자체의 값과 $\mu$의 분산 $V(\mu\|\sigma^2)$의 값에 영향을 같이 받게  된다. 예를 들어서 관측된자료 $x_1,\dots,x_n$ 이 있다고 치자. 



베이즈정리를 쓰면 아래가 성립한다. 
\begin{align}
f(\mu,\sigma^2 \|x_1,\dots,x_n\) \propto f(x_1,\dots,x_n\|\mu,\simga^2) f(\mu,\sigma^2) 
\end{align}
이중에서 $f(\mu,\sigma^2)=f(\mu\|\sigma^2)f(\sigma^2)$ 가 성립하므로 위의 식은 아래와 같이 된다. 
\begin{align}
f(\mu,\sigma^2 \|x_1,\dots,x_n\) \propto f(x_1,\dots,x_n\|\mu,\simga^2) f(\mu\|\sigma^2)f(\sigma^2)
\end{align}
