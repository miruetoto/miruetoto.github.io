...
\begin{align}
\mathbb{E}(\lambda-\hat\lambda)^2:=\int \dots \int (\lambda-\hat{\lambda})^2 L(\lambda) dx_1,\dots dx_n =\int \dots \int (\lambda-\hat{\lambda})^2 f(x_1,\dots,x_n ; \lambda) dx_1,\dots dx_n 
\end{align}
...
\end{align}
즉 빈도주의자들은 우도함수에 대한 평균을 취하고 베이지안은 조인트에 대하여 평균을 취한다. 그리고 빈도주의자와 베이지안 모두 이를 리스크라고 부른다. 

- 베이지안들은 리스크 이외에 베이지리스크라는 것을 새롭게 정의한다. 이것은 아래와 같이 정의한다. 
\begin{align}
\mbox{Bayes risk}:=\int \dots \int \int (\lambda-\hat{\lambda})^2 f(\lambda \|x_1,\dots,x_n)d\lambda
\end{align}
