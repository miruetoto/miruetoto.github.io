...
\end{align}
이때 $\lambda^{\sum x_i} e^{-n\lambda} \lambda^{\alpha-1}e^{-\lambda/\beta}= \lambda^{\sum x_i\alpha -1} e^{-\lambda\big(n+\frac{1}{\beta}\big)}$ 가 되는데 이는 모수가 $\big(\sum x_i+\alpha,\big(n+\frac{1}{\beta}\big)^{-1} \big) $  인 감마분포가 된다. 따라서 사후분포는 모수가 $\big(\sum x_i+\alpha,\big(n+\frac{1}{\beta}\big)^{-1} \big) $ 인 감마분포이다. 
