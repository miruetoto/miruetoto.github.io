...
<center>
	...
	|:----------|:------------------:|:----------------------:|
	|$X_1,\dots,X_n \|\lambda$ in \mbox{Poisson}(\lambda)$ | $\lambda \sim \Gamma(\alpha,\beta)$ | $\lambda\| x_1,\dots,x_n \sim \Gamma\big(\sum x_i+\alpha,\big(n+\frac{1}{\beta}\big)^{-1} \big) |
</center>
...
\begin{align}
\mathbb{E}(\lambda-\hat\lambda)^2:=\int \Bigg\\{ \int \dots \int (\lambda-\hat{\lambda})^2 f(\lambda,x_1,\dots,x_n) dx_1\dots dx_n \Bigg\\} d\lambda
\end{align}
