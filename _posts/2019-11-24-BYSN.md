...
\mathbb{E}_{\lambda}(\lambda-\hat\lambda)^2:=\int (\lambda-\hat{\lambda})^2 L(\lambda) dx=\int (\lambda-\hat{\lambda})^2 f(x_1,\dots,x_n ; \lambda) dx
\end{cases}
\end{align}
...
\end{align}
이걸 자세히보면 지독하게도 빈도주의적 관점임을 볼 수 있다. 베이지안 이었다면 우도를 $f(x_1,\dots,x_n \| \lambda)$ 처럼 썻을 것이다. 즉 MSE 자체가 베이지안들이 쓰는 용어가 아니다. 실제로 베이지안들은 MSE라는 표현을 쓰지 않는다. (베이즈리스크라고 말함..) 만약에 베이지안들이 MSE라는 단어를 썼다면 이는 회귀식 MSE를 뜻하는 것일 확률이 높다.
