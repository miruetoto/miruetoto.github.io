...
\end{align}
이제 $f(x_1,\dots,x_n)$은 $f(x_1,\dots,x_n,\lambda)$을 적분해서 얻을 수 있다. 즉
\begin{align}
...
\begin{align}
f(x_1,\dots,x_n)&=\int f(x_1,\dots,x_n,\lambda) d\lambda  \\\\ \\
&= \int \left( \prod_{i=1}^{n} \frac{\lambda^{x_i} e^{-\lambda} }{x_i !} \right) \frac{\lambda^{\alpha-1}e^{-\lambda/\beta} }{\Gamma(\alpha)\beta^{\alpha} } d\lambda \\\\ \\
&= asdf
\end{align}
