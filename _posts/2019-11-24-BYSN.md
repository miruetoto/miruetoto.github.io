...
\end{cases}
\end{align}
둘다 다소 극단적이긴 했지만 모든 케이스들은 이러한 두 양근단의 사이일 것이다. 다시 말하면 $X_1,\dots,X_n \| \mu,\sigma^2$의 평균을 모델링하는 것과 달리 분산을 모델링하는 일은 $V(\mu\|\sigma^2)$와 $\sigma^2$ 모두 고려해야해서 ***identifiable*** 하지 않다. 그럼 어떻게 결정을 해야할까? 실제모델은 저 두경우중 사이값일텐데 그 중 무엇일까? 바로 이것을 결정하는 식이 아래식이다. 
\begin{align}
\frac{\nu V(\mu\|\sigma^2)}{\sigma^2} \sim \chi^2(\nu) 
\end{align}
