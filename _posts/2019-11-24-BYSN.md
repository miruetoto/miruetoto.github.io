...
\end{cases}
\end{align}
카이제곱분포를 따르는 변수를 기억하는것도 좀 힘든데 대충 
\begin{align}
\frac{\nu V(\mu\|\sigma^2}{\sigma^2}:=\sum_{i=1}^{\nu}Z_i^2
\end{align}
와 같은 느낌으로 기억하면 된다. 왜 이렇게 가정하는것일까? 결국 우리가 관측한 자료 $x_1,\dots,x_n \| \mu,\sigma^2$ 의 분산은 $\sigma^2$ 자체의 값과 $\mu$의 분산 $V(\mu\|\sigma^2)$의 값에 영향을 같이 받게 된다. 예를 들어서 관측된자료 $x_1,\dots,x_n$ 이 있다고 치자. 계산해보니까 $\bar{x}=2$이고 표준편차 $s^2=1$ 이었다고 하자. 하지만 이것은 아래의 경우에 모두 가능한 상황이다. 
\begin{align}
\begin{cases}
\mu \sim N(2,1) \\\\ \\
...
\begin{cases}
\mu \sim N(2,0) \\\\ \\
X_1,\dots,X_n \sim N(2,1)
\end{cases}.
\end{align}
둘다 다소 극단적이긴 했지만 모든 케이스들은 이러한 두 양근단의 사이일 것이다. $x_1,\dots,x_n \| \mu,\sigma^2$ 의 평균과 달리 분산은 $(\mu\|\sigma^2)$와 $\sigma^2$ 모두에 영향을 받는다. 그럼 어떻게 결정을 해야할까? 실제모델은 저 두경우중 사이값일텐데 그 중 무엇일까? 이것을 결정하는 식이 아래식이다. 
\begin{alig}
\frac{\nu V(\mu\|\sigma^2}{\sigma^2}:=\sum_{i=1}^{\nu}Z_i^2
\end{align}
