...
- 빈도주의자와 베이지안 모두 추정량은 MSE(=expectation of loss=risk)를 최소화하면 된다는 마인드를 가지고 있다. 그런데 빈도주의자의 관점에서는 
\begin{align}
...
| 수식 | 빈도주의자 | 베이지안 | 베이지안을 설명하는 빈도주의자 | 
|:----|:-----:|:------:|:--------------------:|
$\lambda-\hat{\lambda}$|loss|loss|loss|
$\int \dots \int (\lambda-\hat{\lambda})^2 f(x_1,\dots,x_n ; \lambda) dx_1\dots dx_n $ | expected loss, MSE, risk | . | . | 
$\int \dots \int \int (\lambda-\hat{\lambda})^2 f(\lambda,x_1,\dots,x_n) dx_1\dots dx_n d\lambda $ |. | expected loss, MSE  | expected risk  
$\int (\lambda-\hat{\lambda})^2 f(\lambda \|x_1,\dots,x_n)d\lambda$ | . | Bayes risk | Bayes risk | 

- 이제부터는 비켈책을 보면서 정리할 것이다. 

- 로스를 $l(\lambda,\hat\lambda)$ 으로 정의하자. 여기에서 $\hat\lambda$ 은 일반적으로 아래를 모두 지칭할때가 많다. 
\begin{align}
\end{align}
$X_1,\dots,X_n$의 함수로 표현되는 통계학자의 선택이다. 따라서 이것을 $\delta(X_1,\dots,X_n)$와 같이 쓰기도 한다. 
