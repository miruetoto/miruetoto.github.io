...
\end{align}
이때 $\propto$ 의 의미는 원래 비례라는 의미이다. 즉 $f(y) \propto g(y)$ 라는 의미는 적당한 상수 $k$ 가 존재하여 $f(y)=k \times g(y)$ 가 성립한다는 의미이다. 이때 $k$는 진짜 $0.2$ 같은 상수란 의미가 아니라 $y$와 상관없는 어떤 수식이라는 의미이다. 따라서 여기에서는 아래와 같이 쓸 수 있다. 
\begin{align}
f(\lambda | x_1,\dots,x_n) \propto f(x_1,\dots,x_n | \lambda) f(\lambda) 
\Longleftrightarrow f(\lambda | x_1,\dots,x_n) = \mbox{const with regard to $\theta$} \times f(x_1,\dots,x_n | \lambda) f(\lambda) 
\end{align}
