...
|$X_1,\dots,X_n \| \mu, \sigma^2  \sim N(\mu,\sigma^2)$ | $\frac{\nu V(\mu\|\sigma^2)}{\sigma^2} \sim  \chi^2(\nu)$ <br/> $\mu \| \sigma^2 \sim N \big(E(\mu\|\sigma^2), V(\mu\|\sigma^2)\big)$ | $\frac{\nu V(\mu\|\sigma^2)}{\sigma^2}\|x_1,\dots,x_n \sim  \chi^2(n+\nu)$ <br/> $\mu \| \sigma^2, x_1,\dots,x_n \sim N\Big(\frac{\sum x_i +\frac{\sigma^2E(\mu\|\sigma^2)}{V(\mu\|\sigma^2)}  }{n+\frac{\sigma^2}{V(\mu\|\sigma^2)} } , \frac{\sigma^2}{n+\frac{\sigma^2}{V(\mu\|\sigma^2)} }\Big)$ <br/> $\frac{\mu-E(\mu\|\sigma^2,x_1,\dots,x_n)}{asd} \sim t(\nu+n)$
...
\end{cases}
\end{align}
왜 이렇게 가정하는것일까? 결국 우리가 관측한 자료 $x_1,\dots,x_n \| \mu,\sigma^2$ 의 분산은 $\sigma^2$ 자체의 값과 $\mu$의 분산 $V(\mu\|\sigma^2)$의 값에 영향을 같이 받게 된다. 예를 들어서 관측된자료 $x_1,\dots,x_n$ 이 있다고 치자. 계산해보니까 $\bar{x}=2$이고 표준편차 $s^2=1$ 이었다고 하자. 하지만 이것은 아래의 경우에 모두 가능한 상황이다. 
\begin{align}
\begin{cases}
\mu \sim N(2,1) \\\\ \\
...
\end{cases}
\end{align}
둘다 다소 극단적이긴 했지만 모든 케이스들은 이러한 두 양근단의 사이일 것이다. 다시 말하면 $X_1,\dots,X_n \| \mu,\sigma^2$의 평균을 모델링하는 것과 달리 분산을 모델링하는 일은 $V(\mu\|\sigma^2)$와 $\sigma^2$ 모두 고려해야해서 ***identifiable*** 하지 않다. 그럼 어떻게 결정을 해야할까? 실제모델은 저 두 경우중 사이값일텐데 그 중 무엇일까? 바로 이것을 결정하는 식이 아래식이다. 
\begin{align}
\frac{\nu V(\mu\|\sigma^2)}{\sigma^2} \sim \chi^2(\nu) 
\end{align}
그럼 이때 $\nu$의 역할은 무엇일까? 이는 $V(\mu\|\sigma^2)$와 $\sigma^2$사이의 가중치를 조정하는 역할을 한다. 

- 김우철 교수님의 수리통계학 책에서는 아래와 같은 기호를 도입한다. 
\begin{align}
\kappa:=\frac{\sigma^2}{V(\mu\|\sigma^2)}
\end{align}
따라서 이 기호를 사용하면 아래와 같이 표현할 수 있다. 
\begin{align}
\begin{cases}
\frac{\nu}{\kappa} \sim  \chi^2(\nu) \\\\ \\
...
\end{cases}
\end{align}
여기에서 $\kappa$는 표본이 많아질수록 점점 커저야하는 수이다. 따라서 마치 $n$과 같은 개념으로 사용할 수 있다. 이는 실제로도 그러한것 같다. 모수 $\mu$의 포스테리어는 아래와 같이 계산가능하다. 
\begin{align}
\mu \| \sigma^2, x_1,\dots,x_n \sim N\Big(\frac{n\bar{x}+\kappa E(\mu\|\sigma^2)}{n+\kappa} ,\frac{\sigma^2}{n+\kappa}\Big)
\end{align}
이때 마치 $\kappa$는 샘플수와 프라이어 사이에 적절한 웨이트를 조정하는 역할을 한다. 이런 역할은 아래의 포스테리어에서도 확인가능하다. 
\begin{align}
\frac{\nu V(\mu\|\sigma^2) + (n-1)s^2+(n^{-1}+\kappa^{-1})^{-1}\big(\bar{x}-E(\mu\|\sigma^2)\big)^2 }{\sigma^2} \bigg\| x_1,\dots,x_n \sim \chi^2(\nu+n)
\end{align}
위의 식은 (1) prior variance 즉 $V(\mu\|\sigma^2)$ 로만 분산을 구한것과 (2) 샘플 variance 즉 $x_1,\dots,x_n$ 으로만 분산을 계산한것 (3) 프라이어와 표본의 오차를 보정하는 텀으로 혼합되어 있다. 이중에서 $\kappa$는 (3)을 계산하는 가중치를 조절한다. 

- 여기까지 과정을 거치면 아래와 같이 쓸 수 있다. 
\begin{align}
\frac{\mu\|x_1,\dots,x_n,\sigma^2-E}{\sigma/\sqrt{n+\kappa} }
\end{align}


- 베이즈정리를 쓰면 아래가 성립한다. 
\begin{align}
f(\mu,\sigma^2 \|x_1,\dots,x_n\) \propto f(x_1,\dots,x_n\|\mu,\sigma^2) f(\mu,\sigma^2) 
\end{align}
이중에서 $f(\mu,\sigma^2)=f(\mu\|\sigma^2)f(\sigma^2)$ 가 성립하므로 위의 식은 아래와 같이 된다. 
\begin{align}
f(\mu,\sigma^2 \|x_1,\dots,x_n\) \propto f(x_1,\dots,x_n\|\mu,\sigma^2) f(\mu\|\sigma^2)f(\sigma^2)
\end{align}
우리는 $\sigma^2$이 아니라 $\sigma^{-2}$의 분포를 알고 있으므로 아래와 같이 바꾼다. 
\begin{align}
f(\mu,\sigma^{-2} \|x_1,\dots,x_n\) \propto f(x_1,\dots,x_n\|\mu,\sigma^{-2}) f(\mu\|\sigma^{-2})f(\sigma^{-2})
\end{align}
