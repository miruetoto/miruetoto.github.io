...
- 회귀식 MSE는 잔차 즉 $e_i=y_i-\hat{y}_ i$의 average이다. 수통식 MSE는 손실함수 $(\lambda-\hat{\lambda})^2$의 mean이다. 여기에서 미치겠는것은 머신러닝쪽에서는 회귀식 MSE 그 자체를 손실함수라고 표현하기도 한다는 점이다. 

- 수통식정의의 황당한 점은 빈도주의자와 베이지안이 생각하는 정의가 다르다는 것이다. 우선 빈도주의자들은 MSE를 아래와 같이 정의한다. 
\begin{align}
\mathbb{E}_ {\lambda}(\lambda-\hat\lambda)^2:=\int \dots \int (\lambda-\hat{\lambda})^2 L(\lambda) dx_1,\dots dx_n =\int \dots \int (\lambda-\hat{\lambda})^2 f(x_1,\dots,x_n ; \lambda) dx_1,\dots dx_n 
\end{align}
베이지안은 MSE를 아래와 같이 생각한다. 
\begin{align}
\mathbb{E}_ {\lambda}(\lambda-\hat\lambda)^2:=\int \dots \int \int (\lambda-\hat{\lambda})^2 f(\lambda,x_1,\dots,x_n) dx_1,\dots dx_n d\lambda
\end{align}
즉 빈도주의자들은 우도함수에 대하여 평균을 취하고 베이지안은 그리고 둘다 MSE를 risk라고 부른다. 이때 베이지안들이 부르는 risk를 베이즈리스크라고 한다. 즉 두 학파의 마인드는 아래와 같다.
\begin{align}

\end{align}

즉 MSE는 빈도주의자들에게 위험함수라고 불린다. 그리고 $(\lambda-\hat{\lambda})^2$ 를 손실함수라고 한다. 따라서 빈도주의자들에게는 위험함수, 즉 수통식 MSE는 손실함수의 평균으로 기억된다. 즉 빈도주의자들에게는 아래의 식이 머리속에 있다. 
\begin{align}
