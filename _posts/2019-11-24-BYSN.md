...
계산해보면 아래와 같이 정리된다. 
\begin{align}
f(\lambda \| x_1,\dots,x_n)=\frac{\lambda^{\sum x_i \alpha-1} e^{-\lambda\big(n+\frac{1}{\beta}\big)} }{\Gamma(\sum x_i+\alpha)\big(\big(n+\frac{1}{\beta}\big)^{-1} \big)^{\sum x_i +\alpha} }
\end{align}
따라서 $\lambda$의 사후분포는 모수가 $\big(\sum x_i+\alpha,\big(n+\frac{1}{\beta}\big)^{-1} \big) $ 인 감마분포를 따른다.  asdf
