...
- ${\cal T}$ 가 $X$ 의 subset 으로 이루어진 collection 이라고 하자. ${\cal T}$ 가 ***uncountable union*** 에 닫혀있고 ***finite intersection*** 에 닫혀있다면 ${\cal T}$ 를 $X$ 의 topology 라고 한다. 그리고 $(X,{\cal T})$ 를 ***topological space*** 라고 한다. 
...
\end{align}
참고로 위와 같은 ball 들을 모은 collection 을 ${\cal B}$라고 하자. 그리고 ${\cal B}$의 임의의 원소를 언카운터블-유니온하여 얻을 수 있는 집합들의 모임을 ${\cal T}$라고 하자. 그러면 (1) ${\cal T}$ 가 $X$ 의 토폴로지임을 보이고 (2) ${\cal B}$의 모든 원소가 오픈셋임을 보인다면 ${\cal B}$는 ${\cal T}$의 base가 된다고 주장할 수 있다. 그런데 (2)는 (1)이 성립하면 자동으로 성립하므로 (1)만 보이면 된다. ${\cal T}$가 언카운터블-유니온에 닫혀있음 ***associative laws*** 에 의해서 성립한다. 이제 ${\cal T}$가 파이나이트-인터섹션에 닫혀있음을 보이자. ${\cal T}$의 임의의 두 원소는 각각 ${\cal B}$의 언카운터블-유니온으로 표현가능하다. 가령 예를들어 임의의 $T,S \in {\cal T}$ 가 아래와 같이 표현되었다고 치자. 
\begin{align}
...
\end{align}
따라서 $B_t \cap B_s$ 가 오픈셋이면 $T \cap S$ 가 오픈셋이 되는 구조라 (2)가 증명된다. 이제 우리가 할일은 $B_t\cap B_s$가 오픈셋임을 보이는 것이다. 이것은 $B_t \cap B_s$ 의 모든점에 **나이테정리**를 써서 증명하면 된다. 즉 $B_t\cap B_s$의 모든 점은 해당점을 포함하면서 $B_t\cap B_s$보다는 작은 볼이 존재한다. 
