...
\end{align}
보는것처럼 $\tilde{\cal B}$는 ${\cal O}$의 base가 아니다. 하지만 $\pi(\tilde{\cal B})$ 는 $(a,b)$를 포함하고 있기에 ${\cal O}$ 의 base가 된다. 여기에서 $\pi(\tilde{\cal B})$ 는 ${\cal B}$ 에 의해서 생성된 가장 작은 ***$\pi$-system*** 이다. 참고로 $\pi$-시스템은 모든 원소가 ***finite intersection*** 에 닫혀있는 collection 을 의미한다. 
