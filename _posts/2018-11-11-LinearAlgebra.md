...
- 가끔씩 ${\bf D}$를 정사각행렬이 아니라 사각행렬로 ${\bf D}_ {n \times m}$로 정의할때도 있다. 가령 예를 들어 ${\bf D}_ {2 \times 3}=rbind(c(\lambda_1,0,0),c(0,\lambda_2,0))$ 혹은 ${\bf D}_ {3\times 2}=rbind(c(\lambda_1,0),c(0,\lambda_2),c(0,0))$과 같이 정의하는 식이다. 위의 두 경우 모두 편의상 $diag(1,2)$로 표현가능하다. 그리고 이 경우에는 $min(n,m)$까지만 전개하면된다. 즉 아래와 같은 식이다. <br/><br/>
  ...
  - ${\bf D}_ {2 \times 3} rbind({\bf U}'_ 1, {\bf U}'_ 2)= rbind(\lambda_1{\bf U}'_ 1, \lambda_2{\bf U}'_ 2)$. 
