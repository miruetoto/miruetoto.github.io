...
- 이제부터는 그냥 머리 비우고 단순계산만 하면된다. 먼저 **E-step**을 수행하자. 
\begin{align}
\quad Q(\boldsymbol{\theta}\|\boldsymbol{\theta}^{(k)}):=\mathbb{E}{\boldsymbol\ell}^* \big({\bf W}\|{\bf z} ,\lambda_1^{(k)},\lambda_2^{(k)},p^{(k)}\big) \\\\ 
&=\mathbb{E} \left[ \sum_{i=1}^{n}\left( W_i\log\frac{pe^{-\lambda_1} \lambda_1^{z_i}}{z_i !}+(1-W_i)\log\frac{(1-p)e^{-\lambda_2} \lambda_2^{z_i}}{z_i !} \right)  ~ ~ \Bigg|~ ~ {\bf Z=z} \right]  \\\\ \\\\
&=\sum_{i=1}^{n}\mathbb{E} \left[\left( W_i\log\frac{pe^{-\lambda_1} \lambda_1^{z_i}}{z_i !}+(1-W_i)\log\frac{(1-p)e^{-\lambda_2} \lambda_2^{z_i}}{z_i !} \right)  ~ ~ \Bigg|~ ~ Z_i=z_i \right]  \\\\ \\\\
&=\sum_{i=1}^{n}\mathbb{E} \left[\left( W_i\log\bigcirc +(1-W_i)\log\bigtriangleup \right)   ~ ~ \Bigg| ~ ~ Z_i=z_i  \right]  \\\\ \\\\
&=\sum_{i=1}^{n}\mathbb{E} \left[\left( W_i\log\frac{\bigcirc}{\bigtriangleup}+\log\bigtriangleup \right)  ~ ~ \Bigg| ~ ~ Z_i=z_i \right] \\\\ \\\\
&=\sum_{i=1}^{n}\left( \mathbb{E}(W_i|Z_i=z_i)\log\frac{\bigcirc}{\bigtriangleup}+\log\bigtriangleup \right)
\end{align}
여기에서 $\mathbb{E}(W_i|Z_i=z_i)$는 아래와 같이 구한다. ~~(그냥 때려맞추면 됨)~~
\begin{align}
