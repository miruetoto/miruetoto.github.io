...
- **미싱-프라블럼** 이란 우리는 사실 ${\bf Y}^{com}=({\bf Y}^{obs},{\bf Y}^{mis})$를 관측하고 싶었는데 어떠한 이유로 ${\bf Y}^{obs}$만 관측한 상황을 의미한다. 여기에서 ${\bf Y}^{obs}=c(Y^{obs}_ 1,\dots,Y^{obs}_ n)$ 이고 ${\bf Y}^{mis}=c(Y^{mis}_ 1,\dots, Y^{mis}_ n)$ 이다. 이때 보통 ${\bf Y}^{obs}$와 ${\bf Y}^{mis}$ 의 각 관측치는 독립이라고 가정하는것 같다. 즉 $Y^{obs}_ 1,\dots,Y^{obs}_ n$은 서로 독립이고 $Y^{mis}_ 1,\dots, Y^{mis}_ n$도 서로 독립이라고 가정한다. 

- 일반적으로 항상 
\begin{align}
{\bf Y}^{com}=({\bf Y}^{obs},{\bf Y}^{mis})
\end{align}
의 관계가 성립하는 것은 아니다. 랭은 $t({\bf Y}^{com})={\bf Y}^{obs}$ 를 만족하는 some function $t$가 있다고만 하였는데 이게 가장 general form 인듯 하다. 따라서 우리가 두는 셋팅 ${\bf Y}^{com}=({\bf Y}^{obs},{\bf Y}^{mis})$ 는 $t$ 가 ***simple projection*** 인 특수한 경우라 볼 수 있다. 
