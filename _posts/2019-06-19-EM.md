...
&=\mathbb{E} \Bigg[ \sum_{i=1}^{n}\Bigg(W_i\|z_i \log\big(\mbox{blah-blah}\big)+(1-W_i\|z_i)\log\big(\mbox{yadi-yadi}\big) \Bigg)\Bigg]
\end{align}
여기에서 **블라블라**는 
\begin{align}
...
\end{align}
이다. 둘다 "***funtion of*** $\boldsymbol{\theta}^{(k)}$" 이므로 $M$-step 에서는 그냥 상수취급된다. 

- 정작 $E$-step 에서 중요한 계산은 $\mathbb{E}(W_i|z_i)$ 이다. 이는 아래와 같이 구한다. ~~(그냥 때려맞추면 됨)~~
\begin{align}
