...
- 그런데 골치 아픈 것은 우리가 관측한 자료가 사실 $\bf (z,w)$가 아니라 $\bf z$ 라는 것이다. 그래서 $L(\lambda_1,\lambda_2,p ~\|~ {\bf Z}={\bf z},{\bf W}={\bf w})$ 는 우도함수가 아니라 **랜덤변수** 가 된다. 즉 우도함수라고 믿고 구해왔던 것이 아래와 같은 모양이 되어버린다. 
\begin{align}
\prod_{i=1}^{n}\left(\frac{pe^{-\lambda_1} \lambda_1^{z_i}}{z_i !} \right)^{W_i} \left(\frac{(1-p)e^{-\lambda_2} \lambda_2^{z_i}}{z_i !} \right)^{1-W_i}
\end{align}
위의 식은 $W_i$가 포함되어있으므로 랜덤이 되기 때문이다. 따라서 이를 최적화할 수는 없다. 
