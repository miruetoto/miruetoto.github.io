...
\begin{align}
dist({\bf x},{\bf x}^* )=-\\|{\bf x}-{\bf x}^* \\|_ {2}^{2}
\end{align}
where $\\|{\bf x}-{\bf x}^* \\|$ is the squared Euclidean distance.

자료간의 **dissimilarity**란 말 그대로 자료들이 얼마나 비슷하지 않은가를 나타내는 개념이다. 두 자료 ${\bf x}$와 ${\bf x}^* $사이의 거리를 측정하는 가장 간단한 measure로는 
