...
### 서론 
- 이 논문에서는 두 자료 ${\bf x}$와 ${\bf x}^* $사이의 **dissimilarity**를 정의하기 위한 새로운 measure를 정의한다. 자료간의 **dissimilarity**란 말 그대로 자료들이 얼마나 비슷하지 않은가를 나타내는 개념이다. 두 자료 ${\bf x}$와 ${\bf x}^* $사이의 거리를 측정하는 가장 간단한 measure로는 
