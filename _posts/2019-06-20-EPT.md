...
- 이번 예제에서는 미국의 전기생산량을 기록한 electricity 자료를 분석하겠다. 이 자료는 1973년 1월부터 2005년 12월까지 달별로 기록하였다. 자료는 R package TSA에서 얻을 수 있다. 분석하고자 하는 자료는 그림 4의 (a)에 나타나 있다. 자료는 보는것 처럼 뚜렷한 추세(trend)와 계절성(seasonality)을 가지고 있다. 주목할만한 점은 시간에 따라서 trend 가 증가하듯이 시간에 따라서 seasonal effect의 출렁임 (fluciation) 또한 증가한다는 것이다. 

- 보통 이러한 유형의 자료를 다루는 방법은 적당한 변환 (예를들면 로그변환) 을 통하여 시계열의 분산을 안정화 시키고 계절성을 제외하는 것이다. 우리는 electricity 의 seasonality 를 periodic component 와 amplitude modulating component 의 곱으로 이루어 졌다고 해석하였다. 따라서 우리는 이 예제에서 (1) amplitude modulating component 를 추론하여 자료의 출렁임을 안정화 시키고 (2) seasonality 를 제거하는 순으로 자료를 분해할 것이다. 
