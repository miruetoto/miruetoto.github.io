...
\end{align}
또한 이 필터는 적절한 $\tau$를 선택하였을 경우 임의의 seasonality 를 가지는 신호를 cancellable 할 수 있다. 

- 따라서 $f(t)-{\cal L}\\{f(t)\\}$ 를 계산하면 $f(t)$ 에서 abrupt change 를 효과적으로 제거할 수 있다. Figure 6의 (b)는 $f(t)$와 $L(t):={\cal L}\\{f(t)\\} $를 나타낸 것이다. 보는것처럼 $L(t)$는 급격한 변화가 일어나는 지점에서 $f(t)$의 변화를 매우 sharp 하게 찾아낸다. 만약에 $f(t)$에 trend term이 섞여 있지 않다면 ${\cal L}\\{f(t)\\}$를 활용하여 "periodic component + abrup chagne" 가 혼합된 자료를 완벽하게 분해할 수 있다. 
