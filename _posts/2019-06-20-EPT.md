...
- 아래는 EMD를 활용하여 $X_t$를 분해한 결과이다. 자료의 trend 는 잘 추출하지만 seasonality 와 seasonality 의 분산을 변화하게 만드는 term 에 대한 해석이 unclear하다. 
...
- EPT를 활용한 분해의 장점은 분해방법이 additive model 에 국한되지 않는다는 것이다. 특히 위와 같은 신호 $X_t$는 additive model 보다 multiplicative model 이 더 적절하다고 해석될 수 있다. (Metcalfe, A. V., & Cowpertwait, P. S. (2009). Introductory time series with R. Springer-Verlag New York. 의 Ch 1.5 를 참고) 제안된 방법은 additive model 과 multiplicative model 사이를 오가며 적절한 분해방법을 선택할 수 있다는 장점이 있다. 
