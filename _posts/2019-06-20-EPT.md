...
(4) 적절한 $\tau$를 선택해야만 한다. <br/>
반면에 EMD와 VMD와 같은 방법은 target signal $X_t$가 IMF의 합으로 구성되어 있다고 가정하므로 (1)의 단점이 없다. 또한 분해하는 방법이 고정되어 있으므로 (EMD는 채거름과정, VMD는 퓨리에 basis 기반의 optimization) (2)의 단점도 없다. 또한 (3)과 같이 필터를 설계할때마다 이론적 성질을 살펴볼 필요도 없다. 또한 EMD는 어떠한 튜닝 파라메터도 필요로 하지 않기 때문에 (4)의 단점도 없다. (VMD는 튜닝파라메터를 조정해야 한다.) 종합하면 EMD와 VMD는 사용자가 분석하기 매우 편리하며 적용되는 신호의 class도 넓어 매우 큰 **범용성**을 가지는 툴이라고 볼 수 있다. 반면에 EPT는 범용성보다 **특수성** 에 초점을 맞추는 분해방법이다. EPT는 사용자가 개입하여 설계해야 하는 부분이 많지만 이것이 오히려 $X_t$를 분해함에 있어 극도의 **자유도** 를 보장한다. EMD와 VMD와 같은 방법이 cover하는 신호의 class가 매우 넓은 것은 사실이지만 분명히 세상에는 IMF들의 조합 혹은 퓨리에 베이시스들의 조합만으로 해석하기에는 자연스럽지 않은 다양한 신호가 있다. EPT는 분석하고자 하는 신호 $X_t$에 따라 필터를 바꾸기 때문에 까다로운 신호들을 분석함에 있어 극도의 **유연성** 을 가진다. 다음예제
