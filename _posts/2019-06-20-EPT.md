...
- 연속신호 $X(t)$를 sampling period $T$ 로 sampling 하여 아래와 같은 이산신호 $\bf X$ 를 얻었다고 하자. 
\begin{align}
{\bf X}:=\\{X_i: X_i=X(iT) \\} 
\end{align}
$i$ 와 $\tau \in \mathbb{N}$ 에 대하여 아래와 같은 집합 ${\cal N}_ {\tau}\big({\bf X}\big)$ 를 정의하자. 
\begin{align}
${\cal N}_ {\tau}\big({\bf X}\big):=\\{ \\}$
\end{align}
