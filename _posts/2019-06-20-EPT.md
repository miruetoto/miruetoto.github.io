...
- 이번에는 한국의 3도시 서울, 대구, 부산에서 2003년 9월1일부터 30일까지 hourly 관측된 solar radiation data 를 분석해보도록 하겠다. 자료는 기상청으로 부터 얻었다. 우선 서울, 대구, 부산 중 대구와 부산은 지형적으로 매우 가까운 위치에 있다는 것을 유의하자. 그리고 2003년 9월은 한반도를 비롯한 동북아시아 지역에 태풍(매미)이 발생한 지역이며 그중 대구와 부산은 태풍 매미의 피해가 큰 한반도 지역이었으며 서울은 상대적으로 태풍 매미의 피해를 받지 않은 지역이다. 따라서 서울과 달리 대구와 부산은 solar radation 을 비롯한 기후적인 특징이 매우 유사할 것으로 쉽게 추론할 수 있으며 실제로도 그러하다. 

- 다만 서울, 대구, 부산에서 관측된 solar radation data 간의 correlation 을 계산하여 보면 예상과는 다르게 세 지역의 시계열이 모두 유사하게 측정된다. (표1 참고) 그 이유는 solar radation 자료가 가지는 강력한 주기성 때문이다. 세 지역의 solar radation data 는 그림 XX에 나타나 있다. 보는것처럼 (1) 매우 강력한 주기성을 가지고 있으며 (2) $0$의 값을 다수 포함하고 있다는 특징이 있다. 이는 solar radiation 이 햇빛의 세기에 비례하며, 밤에는 햇빛이 없다는 점을 생각하면 매우 당연한 현상이다. 

- 문제는 (1)에 의한 유사성이 너무 강하기 때문에 서울, 대구, 부산 사이에 존재하는 기후적인 유사성이 숨겨진다는 점이다. 그림 XX 에서 보여지는 solar radiation 는 <br/>
"periodic component $\times$ amplitude modulated component" <br/>
꼴로 해석가능한 신호인데, 이중에서 periodic component 는 단순히 낮과 밤의 반복으로 생기는 daily effect일 뿐이다. 우리가 관심이 있는 것은 "amplitude modulated component" 간의 유사성이다. 

- 우리는 "amplitude modulated component" 를 추출하기 위해서 이전예제와 동일하게 max $\to$ mean 필터를 사용하였다. 이러한 필터는 로그변환과는 다르게 $0$의 값이 다수 포함된 (2) 와 같은 특징을 가지는 신호에서도 잘 적용할 수 있다. 또한 로그변환에 의한 전통적인 멀티플리케이트 모델과는 다르게 "amplitude modulated component" 자체를 추출할 수 있다는 장점이 있어  "amplitude modulated component" 간의 유사성을 직접 측정할 수 있다. 

- 그림 XX 는 제안된 방법으로 추출한 upper envelope of solar radiation data 이며 표 XX는 upper envelope of solar radiation data 간의 correlation 이다. 보는것처럼 대구-부산과의 유사성이 서울-대구, 혹은 서울-부산 간 유사성보다 뚜렷해졌음을 확인할 수 있다. 
