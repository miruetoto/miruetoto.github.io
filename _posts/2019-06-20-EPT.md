...
- 이 예제에서 여러 기후자료 중 solar radiation data 만을 뽑아서 분석하는 이유는 solar radiation data 가 앞의 Example 1,2 에서 분석하였던 자료와 유사하게 "periodic component $\times$ amplitude modulated component" 꼴로 표현가능한 모형이기 때문이다. 그림 XX는 서울, 대구, 부산 각각의 지역에서 solar radiation 자료 이다. 보는것 처럼 (1) 매우 뚜렷한 주기성분을 가지고 있으며 (2) $0$의 값을 다수 포함하고 있다는 특징을 가지고 있다. 이는 solar radiation 은 햇빛의 세기에 비례하고 따라서 밤에는 그 크기가 $0$ 이므로 매우 당연한 현상이다. 

- solar radiation data 의
