...
\end{align}
여기에서 $E(Y\|X)$는 best predictor of $Y$ given $X$ 라고 하자. 편의상 $E(Y|X)$를 기호로 $\hat{Y}^{BP\|X}$라고 정의하자. 즉 
\begin{align}
\hat{Y}^{BP\|X}=E(Y|X)
\end{align}
이다. 
...
\end{align}
이때 $u^* (X)=\hat\beta_0+X\hat\beta_1$는 임의의 $u(X)$에 대하여 
\begin{align}
E\big[(Y-u^* (X))^2\big]\leq E\big[(Y-u(X))^2\big]
\end{align}
를 만족하므로, 아래와 같이 쓸 수 있다. 
\begin{align}
\hat{Y}^{BP\|X}=\hat\beta_0+X\hat\beta_1=E(Y\|X)
\end{align}
...
Z_t=\phi Z_{t-1}+\epsilon_t
\end{align}
이다. 우선 양변에 $Z_t$를 곱하고 평균을 취하면 아래가 얻어짐을 관찰하자. 
\begin{align}
\gamma_0 =\phi \gamma_1 
\end{align}
이제 
...
$\phi_{22}=0$인 이유는 
\begin{align}
\begin{vmatrix}  \gamma_0 & \gamma_1 \\\\ \\ \gamma_1 & \gamma_2 \end{vmatrix} = \begin{vmatrix}  \gamma_2\phi^2 & \gamma_2\phi \\\\ \\ \gamma_2\phi & \gamma_2 \end{vmatrix}=0
\end{align}
이기 때문이다. 

### AR(2)의 이론적 PACF
- AR(2)모델에 한정하여 아래가 성립한다. 
\begin{align}
\gamma_0=\phi_1\gamma_1+\phi_2\gamma_2\\\\ \\
\gamma_1=\phi_1\gamma_2+\phi_2\gamma_3\\\\ \\
\gamma_2=\phi_1\gamma_3+\phi_2\gamma_4\\\\ \\
\dots
\end{align}
