...
\end{align}
여기에서 $E(Y\|X)$는 best predictor of $Y$ given $X$ 라고 하고 기호로 $\hat{Y}^{BP_{\mbox{ given } X}}$라고 정의하자. 즉 
\begin{align}
\hat{Y}^{BP}=E(Y|X)
\end{align}
이다. 
...
\end{align}
이때 $u(X)=\beta_0+\beta_1X$로 생각하면 아래와 같이 쓸 수 있다. 
\begin{align}
\hat{Y}^{BP}=\hat\beta_0^{BP}+X\hat\beta_1^{BP}=E(Y|X)
\end{align}
