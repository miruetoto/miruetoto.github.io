...
그러면 아래가 성립한다. 
\begin{align}
\phi_2^* = \hat\phi_2^{BP\|Z_{t-1},Z_{t-2} }
\end{align}

- 증명을 하여보자. 
(1)의 결과는 아래와 같다. 
\begin{align}
Z_t^* =Z_t-E(Z_t\|Z_{t-1})=Z_t-\frac{cov(Z_t,Z_{t-1})}{V(Z_{t-1})}Z_{t-1}
\end{align}
(2)의 결과는 아래와 같다. 
\begin{align}
Z_{t-2}^* =Z_{t-2}-E(Z_{t-2}\|Z_{t-1})=Z_{t-2}-\frac{cov(Z_{t-2},Z_{t-1})}{V(Z_{t-1})}Z_{t-1}
\end{align}
따라서 
\begin{align}
