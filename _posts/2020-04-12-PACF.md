...
또한
\begin{align}
\phi_2^* =\frac{E\left[Z_t-E(Z_t\|Z_{t-1})\right]E\left[Z_{t-2}-E(Z_{t-2}\|Z_{t-1})\right]}{E\left[Z_t-E(Z_t\|Z_{t-1})\right]E\left[Z_{t-2}-E(Z_{t-2}\|Z_{t-1})\right]}
\end{align}
임을 쉽게 확인할 수 있다. 
...
### AP(p)
- 정상인 AP(2)모델의 관찰결과를 확장하면 정상인 AR(p)모델에 대하여서도 