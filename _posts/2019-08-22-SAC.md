...
\begin{align}
q_{\pi}(s,a)=\sum_{s',a}p(s',r \| s,a)\big[r+\gamma v_{\pi}(s') \big] 
\end{align}
보상이 랜덤이 아니라는 가정을 하면 아래식이 성립한다. 
\begin{align}
r({\bf s}_ t, {\bf a}_ t)=\sum_{r \in {\cal R}} \sum_{s',a}p(s',r \| s,a)r = \sum_{s',a}p(s',r \| s,a)r
\end{align}
이때 첫번째 등호는 정의에 의해서 두번째 등호는 보상이 랜덤이 아니라는 가정에 의해서 성립한다. 이 경우 큐펑션에 대한 벨만이퀘이션은 아래와 같이 된다. 
\begin{align}
q_{\pi}(s,a)=r({\bf s}_ t, {\bf a}_ t) \sum_{s',a}p(s',r \| s,a)\big[\gamma v_{\pi}(s') \big] 
\end{align}
