...
- 참고로 위의 쿨백라이블러-다이버전스를 풀면 아래와 같다. 
\begin{align}
D_{KL} & \bigg( \pi'(\cdot \| {\bf s}_ {t}) \bigg\\| \frac{\exp\big( \frac{1}{\alpha}Q_{\pi_{old} } ({\bf s}_ t, \cdot) \big)}{Z_{\pi_ {old} }({\bf s}_ t)} \bigg) \\\\ \\
& = - \mathbb{E}_ { {\bf a}_ t \sim \pi'} \Bigg[ \log \Bigg(\frac{\exp\big( \frac{1}{\alpha}Q_{\pi_{old} } ({\bf s}_ t, {\bf a}_ t) \big)}{Z_{\pi_ {old} }({\bf s}_ t)} \Bigg)\Bigg] \\\\ 
& =- \int_{\cal A} \pi'({\bf a}_ t \| {\bf s}_ t) \log \bigg(\frac{\exp\big( \frac{1}{\alpha}Q_{\pi_{old} } ({\bf s}_ t, {\bf a}_ t) \big)}{Z_{\pi_ {old} }({\bf s}_ t)} \bigg) d {\bf a}_ t.
\end{align}
특히 두번째 방정식을 잘 살펴보면 아래와 같이 정리됨을 쉽게 알 수 있다. 
\begin{align}
D_{KL} & \bigg( \pi'(\cdot \| {\bf s}_ {t}) \bigg\\| \frac{\exp\big( \frac{1}{\alpha}Q_{\pi_{old} } ({\bf s}_ t, \cdot) \big)}{Z_{\pi_ {old} }({\bf s}_ t)} \bigg)
= - \mathbb{E}_ { {\bf a}_ t \sim \pi'} \Bigg[ \log \Bigg(\frac{\exp\big( \frac{1}{\alpha}Q_{\pi_{old} } ({\bf s}_ t, {\bf a}_ t) \big)}{Z_{\pi_ {old} }({\bf s}_ t)} \Bigg)\Bigg]
\end{align}
...
- 여기에서 아래의 term
\begin{align}
\log \frac{\exp\big( \frac{1}{\alpha}Q_{\pi_{old} } ({\bf s}_ t, \cdot) \big)}{Z_{\pi_ {old} }({\bf s}_ t)}
\end{align}
