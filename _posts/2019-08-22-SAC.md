...
- 이제 $(\pi_1^* ,\alpha_1^* )$ 가 기븐된 상태에서 $(\pi_0,\alpha_0)$ 를 최적화하는 문제를 생각하여 보자. 먼저 수식 (15)로 부터 아래를 관찰하자. 이때 $t=0$ 이고 $T=1$이다. 
\begin{align}
Q_0^* ({\bf s}_ 0, {\bf a}_ 0 ; \pi_1^* ,\alpha_1^* ) = \mathbb{E}_ {({\bf s}_ 0, {\bf a}_ 0) \sim \rho_{\pi} } r({\bf s}_ 0, {\bf a}_ 0) + \mathbb{E}_ {({\bf s}_ 1, {\bf a}_ 1) \sim \rho_{\pi} } 
\end{align}
