...
\end{align}
이다. 환경은 현재상태 ${\bf s}_ t $ 와 액션 ${\bf a}_ t$에 따라서 보상 $r:{\cal S}\times{\cal A} \to [r_{min}, r_{max}]$를 주게 된다. 그리고 폴리쉬 $\pi$에 따른 밸류펑션 $\rho_{\pi}({\bf s}_ t)$ 와 $q$-펑션 $\rho_{\pi}({\bf s}_ t, {\bf a}_ t)$를 정의한다. 
