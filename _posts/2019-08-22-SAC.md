...
\bigg) -\alpha_1{\cal H}
\end{align}
이제 $\frac{\cal L}{\partial \pi_1 }=0$ 와 $\frac{\cal L}{\partial \alpha_1 }=0$ 을 연립해서 풀면 된다. 참고로 논문에서는 미분을 사용하지 않았고 제약된 최적화의 동치조건을 듀얼프라블럼으로 구했다. 아마도 미분이 불가능한 함수들의 클라스에 대하여서도 커버하고 싶어서 그런것 같다. 아무튼
\begin{align}
\frac{\cal L}{\partial \pi_1 }=0 \\\\ \\ 
\frac{\cal L}{\partial \alpha_1 }=0 
\end{align}
를 풀어서 나온 해를 각각 $\pi_1^* $ 와 $\alpha_1^* $ 라고 하자. 

- 이제 $(\pi_1^* ,\alpha_1^* )$ 가 기븐된 상태에서 $(\pi_0,\alpha_0)$ 를 최적화하는 문제를 생각하여 보자. 
