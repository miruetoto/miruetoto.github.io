...
\end{cases}
\end{align}
논문의 수식 (13) 의 우변을 푸는것과 동치이다. 그리고 논문에서 폴라쉬는 위의 반식으로 업데이트 하지 않고 파라메터 $\phi$ 로 모델링해서 업데이트 할것이기 때문에 실제적으로 풀어야 할 식은 위의 연립이 아니고 $\pi_1$ 을 어떻게 구했다고 치고 $ \frac{ \partial \cal L}{\partial \alpha_1 }=0$ 를 정리하여 푸는일만 남는다. 그리고 이 식을 푸는일은 수식 (14) 를 푸는것과 동치이다. 참고로 수식 (14) 에서 $\pi_T^* ({\bf a}_ T\| {\bf s}_ T)$ 가 아니라 $\pi_T^* ({\bf a}_ T\| {\bf s}_ T; \alpha_T)$ 로 표현한 이유는 $\pi_T^* $ 가 $\alpha_T$ 에 따라 달라지는 값임을 명시하기 위한것이다(원래는 연립방정식의 해이어야 하므로 function of $\alpha_T$ 임).

- 이제 $(\pi_1^* ,\alpha_1^* )$ 가 기븐된 상태에서 $(\pi_0,\alpha_0)$ 를 최적화하는 문제를 생각하여 보자. 즉 엔트로피가 제약된 상황에서 아래를 푸는것에 관심을 가지자. (요게 논문의 수식 (16)임)
\begin{align}
\max_{\pi_0} \bigg( \mathbb{E}_ {({\bf s}_ 0,{\bf a}_ 0) \sim \rho_{\pi} } r({\bf s}_ 0, {\bf a}_ 0) +\max_{\pi_1} \mathbb{E}_ {({\bf s}_ 1,{\bf a}_ 1) \sim \rho_{\pi} } r({\bf s}_ 1, {\bf a}_ 1)  \bigg)
\end{align}
...
Q_1^* ({\bf s}_ 1, {\bf a}_ 1) - \alpha_1^* \log \pi_1^* ({\bf a}_ {t+1} \| {\bf s}_ {t+1}) 
\bigg) 
\end{align}
위의 식과 $Q_1^* ({\bf s}_ 1, {\bf a}_ 1) = \mathbb{E}_ {({\bf s}_ 1, {\bf a}_ 1) \sim \rho_{\pi} } r({\bf s}_ 1, {\bf a}_ 1)$ 임을 이용하면 
