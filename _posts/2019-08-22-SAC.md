...
- 위의 크라이테리온은 언뜻 보기에 직관적이지 않다. 하지만 쿨백라이블러-다이버전스를 풀면 다소 직관적인 수식을 얻을 수 있다. 
\begin{align}
...
& := - \mathbb{E}_ { {\bf a}_ t \sim \pi'} \Bigg(\frac{1}{\alpha} Q_{\pi_{old} }({\bf s}_ t, {\bf a}_ t)- \log \pi'({\bf a}_ t \| {\bf s}_ t) - \log Z_{\pi_ {old} }({\bf s}_ t)} \Bigg) 
\end{align}
