...
이제 아래식이 성립함을 관찰하자. 
\begin{align}
cov(\Delta y_t,\Delta y_t)=1+(\phi-1)^2+\phi^2(\phi-1)^2+\phi^4(\phi-1)^2+\dots=\frac{2\sigma^2}{\phi+1}
\end{align}
\begin{align}
cov(\Delta y_t,\Delta y_{t-1})=(\phi-1)+\phi(\phi-1)^2+\phi^3(\phi-1)^2+\phi^5(\phi-1)^2+\dots=\frac{\sigma^2(\phi-1)}{\phi+1}
\end{align}
따라서 Lag=1 에서의 ACF는 아래와 같다. 
\begin{align}
\frac{cov(\Delta y_t,\Delta y_{t-1})}{\sqrt{cov(\Delta y_t,\Delta y_t)}\sqrt{cov(\Delta y_{t-1},\Delta y_{t-1})}}=\frac{\phi-1}{2}
\end{align}
