...
\end{align}
증명은 그렇게 어렵지 않다. MA(1)의 ACF는 아래와 같이 쓸 수 있다. 
\begin{align}
\rho(\theta)=\frac{\theta}{1+\theta^2}
\end{align}
이걸 미분하면 아래와 같이된다. 
\begin{align}
\rho'(\theta)=\frac{1-\theta^2}{(1+\theta^2)^2}
\end{align}
따라서 $\rho'(\theta)=0$을 만족하는 $\theta$는 1과 -1이다. 이를 각각 $\rho(\theta)$에 대입하면 원하는 것을 얻는다. 