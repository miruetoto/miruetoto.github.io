...
(1) $f:(\Omega,{\cal F}) \rightarrow (\mathbb{R},{\cal R})$ 이 어야 한다. 즉 $f$ 가 메저러블 맵핑 이어야 한다. 왜냐하면 적분 $\int f d \mu$ 를 수행하기 위해서는 임의의 $B \in {\cal R}$ 에 대한 inverse image $A:=\\{\omega: f \in B \\}$ 를 구하고 그 $A$에 대한 메저 $\mu(A)$를 계산해야 하는데 이 $\mu(A)$가 정의되기 위해서는 $A \in {\cal F}$이어야 하기 때문이다. <br/>
