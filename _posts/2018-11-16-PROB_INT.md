...
- 다시 $\mu_i$들을 $P_i$들로 바꾸어 생각해보자. 그리고 각각의 확률공간 $(\Omega_1,{\cal F}_ 1, P_1)$, $(\Omega_2,{\cal F}_ 2, P_2)$ 에서 정의된 확률변수 $X_1$, $X_2$를 상상해보자. 두 확률변수로 만든 확률벡터 ${\boldsymbol X}=(X_1,X_2)$는 명백하게 $(\Omega_1\times \Omega_2, {\cal F}_ 1 \times {\cal F}_ 2)$ 에서 $(\mathbb{R}^2, {\cal R}^2)$로 가는 메저러블 맵핑이다. 하지만 $P_1\times P_2$는 ${\boldsymbol X}$의 확률측도가 반드시 되는 것은 아니다. $P_1\times P_2$가 $\boldsymbol X$의 확률측도가 되려면 모든 $B \in {\cal R}^2$ 에 대하여 
\begin{align}
P({\boldsymbol X} \in B)
\end{align} 
가 well define 되어야 하는데 실제로는 $B \in \sigma({\cal S})$, ${\cal S}:=\\{A_1\times A_2 : A_1 \in {\cal F}_ 1, A_2 \in {\cal F}_ 2\\}$ 에 대하여 
