...
\begin{align}
Eh({\boldsymbol X})=\int_{\mathbb{R}^2} h({\boldsymbol x})\mu(d{\boldsymbol x}) 
\end{align}
여기에서 $\mu$는 $\boldsymbol X$의 ***distribution function*** 이다. 즉 $\mu(A) = P({\boldsymbol X} \in A)$가 성립한다. 또한 $h$는 (1) 양수이거나 (2) 적분가능인 함수로 제약하는데 이는 $h(\boldsymbol X)$의 평균을 잘 정의하기 위함이다. 
