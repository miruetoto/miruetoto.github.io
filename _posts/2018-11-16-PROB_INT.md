...
-  $\int f d \mu$를 정의하기 위해 필요한 기본가정을 살펴보자. <br/><br/>
...
A:=\\{\omega: f \in B \\}
\end{align}
를 구하고 그 $A$에 대한 메저 $\mu(A)$를 계산해야 하는데, 이 $\mu(A)$가 정의되기 위해서는 $A \in {\cal F}$이어야 하기 때문이다. <br/><br/>
**(2)** $\mu$가 **$\sigma$-finite measure** 이어야 한다. 왜냐하면 이 조건이 있어야 임의의 $A \in {\cal F}$에 대하여서도 $\mu(A)<\infty$가 성립하게 되어서 $d \mu$부분을 상당히 안정적으로 정의할 수 있다. <br/><br/>
**(3)** $f$ 와 $\mu$ 가 같은 시그마필드 $\cal F$를 공유하고 있어야 한다. <br/> <br/>
...
- 여기까지는 당연한거라서 우리의 관심은 <br/><center>
