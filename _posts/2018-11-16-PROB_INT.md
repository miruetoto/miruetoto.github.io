...
- 여기서 잠깐 정리해둘 노테이션이 있다. 우선 $X:(\Omega,{\cal F}) \rightarrow (\mathbb{R},{\cal R})$ 이고 $P:{\cal F} \rightarrow \mathbb{R}$을 probability 메져라고 하자. 그리고 $\tilde{\mu}$를 $X$ 의 ***distribution*** 이라고 하자. 즉 $\tilde{\mu}:=\mu \circ X^{-1}$라고 하자. 또한 $F(x)$를 $X$의 ***distribution function*** 이라고 하자. 즉 $F(x)=\tilde \mu(-\infty,x)$ 이고 $\tilde \mu((a,b])=F(b)-F(a)$ 이다. 그리고 $\omega$에 대한 $X$의 이미지를 $x=X(\omega) \in \mathbb{R}$라고 하자. 아래는 모두 같은 기호이다. 
\begin{align}
\int_{\Omega} X d P = \int_{\Omega} X(\omega) P(d\omega) = \int_{\mathbb{R}} x\tilde mu(dx)
\end{align}
가 성립한다. 
