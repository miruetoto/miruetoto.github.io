...
- **(듀렛책 4판 P.30, Thm 1.6.9)** $\bf X$를 $(\Omega, {\cal F})$와 $(\mathbb{R}^2,{\cal R}^2)$를 이어주는 메저러블-맵핑이고 $f$ 는  $(\mathbb{R}^2,{\cal R}^2)$와 $(\mathbb{R},{\cal R})$를 이어주는 메저러블-맵핑이라고 하자. (원래 듀렛책의 셋팅은 이게 아니지만 그냥 이렇게 간단화 시켜서 외우면 좋을것 같다) 만약에 $\bf X$의 평균이 아니라 $f(\bf X)$의 평균을 계산하는 방법은 아래와 같다. 
\begin{align}
Ef({\bf X})=\int_{\mathbb{R}^2} f({\bf y})\mu({\bf dy})=\int_{\mathbb{R}^2} f({\boldsymbol y})\mu(d{\boldsymbol y}) 
\end{align}
