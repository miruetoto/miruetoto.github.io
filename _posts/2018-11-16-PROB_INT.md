...
- 이제 메저러블-맵핑 $f$의 적분을 **모순없이** 정의하는 방법을 살펴보자. 적분은 기본적으로 넓이를 구하는 개념이므로 $f:\Omega \rightarrow \mathbb{R}$가 **simple function** 일때  $\mu:{\cal F} \rightarrow \mathbb{R}$에 대한 적분은 사각형넓이의 유한합으로 매우 명확히 정의할 수 있다. 

- 여기까지는 당연한거라서 우리의 관심은 **"$f$가 simple function이 아닐 경우에도 $\mu$에 대한 적분을 모순없이 잘 정의할 수 있을까?"**가 된다. 결론적으로 <br/><br/>
**(1)** $f$가 **bounded function** 인 경우 <br/>
**(2)** $f$가 **non-negative function**인 경우 <br/>
**(3)** $\int |f| d\mu < \infty$인 경우 <br/><br/>
...
- 여기서 잠깐 정리해둘 노테이션이 있다. 우선 $X:(\Omega,{\cal F}) \rightarrow (\mathbb{R},{\cal R})$ 이고 $P:{\cal F} \rightarrow \mathbb{R}$을 probability 메져라고 하자. 그리고 $\tilde{\mu}$를 $X$ 의 ***distribution*** 이라고 하자. 즉 $\tilde{\mu}:=P \circ X^{-1}$라고 하자. 그리고 $\omega$에 대한 $X$의 이미지를 $x=X(\omega) \in \mathbb{R}$라고 하고 $F(x)$를 $X$의 ***distribution function*** 이라고 하자. 즉 $F(x)=\tilde \mu(-\infty,x)$ 이고 $\tilde \mu((a,b])=F(b)-F(a)$ 이다. 메져 $\tilde \mu$ 가 르벡메져 $\lambda$에 대하여 ***absolutely continuous*** 하면 $\int_B d\tilde mu=\nu(B)$ $f(x)$를 $F(x)$의 라돈니코딤 도함수라고 하자. 아래는 모두 같은 기호이다. 
\begin{align}
\int_{\Omega} X d P = \int_{\Omega} X(\omega) P(d\omega) = \int_{\mathbb{R}} x\tilde \mu(dx) = \int_{\mathbb{R}} xdF(x)=\int_{\mathbb{R}} xf(x)dx.
\end{align}
가 성립한다. 
는 같은 기호이다. (Chung 책 p.42를 참고하면 알 수 있음) 또한 $(\Omega, {\cal F},\mu)=(\mathbb{R}, {\cal R}, \mu)$이고 적분을 하는 메져 $\mu$가 $\mu((a,b])=F(b)-F(a)$와 같은 꼴로 표현가능하다면 우리는 $\int f d\lambda$를 아래와 같이 쓸 수 있다. 
\begin{align}
...
\end{align}
와 같이 나타낼 수 있다. 그리고 $\Omega$가 countable set이고 ${\cal F}$는 ''all subset of $\Omega$'' 라고 하자. $\mu$는 counting measure라고 하자. 이때는 $\int f d\mu$는 아래와 같이 나타낼 수 있다. 
\begin{align}
...
- **(듀렛책 4판 P.30, Thm 1.6.9)** $X$를 $(\Omega, {\cal F})$와 $(S,{\cal S})$를 이어주는 메저러블-맵핑이라고 하자. $X$는 random variable일수도 있고, random vector 일수도 있다. 만약에 $X$가 random vector라면 $(S,{\cal S})=(\mathbb{R}^2,{\cal R}^2)$가 된다. 편의상 이 상황을 가정하자. 이제 $X$가 아니라 function of $X$에 관심을 가져보자. 함수 $f$를 $(\mathbb{R}^2,{\cal R}^2)$와 $(\mathbb{R},{\cal R})$를 이어주는 메저러블-맵핑이라고 하면 $f$는 function of $X$가 되며 $f(X)$역시 메저러블-맵핑이 된다.(즉 $f(X)$역시 확률변수가 된다는 의미임) 만약에 $X$의 평균이 아니라 function of $X$의 평균, 즉 $f(X)$의 평균을 계산하고 싶다면 어떻게 해야하는가? 이런 경우는 매우 흔한데 대표적으로 $X$의 분산을 계산하고 싶은 경우도 이에 해당한다. 원래는 아래와 같이 계산해야 한다. 
\begin{align}
Ef(X)=\int_{\mathbb{R}} f(y)dy =\int_{\mathbb{R}^2} f({\boldsymbol y})\mu(d{\boldsymbol y}) 
\end{align}

듀렛책에서는 $f(X)$가 양수이거나 적분가능할때 (즉 $f \geq 0$ 이거나 $E|f(X)|< \infty$일때) 아래와 같이 $f(X)$의 평균을 계산할 수 있다고 제시한다. 
\begin{align}
Ef(X)=\int_{S} f(y)\mu(dy)=\int_{\mathbb{R}^2} f({\boldsymbol y})\mu(d{\boldsymbol y}) 
\end{align}
여기에서 $\mu$는 $X$의 **distribution function** 이다. 즉 $\mu(A) = P(X \in A)$가 성립한다. 
