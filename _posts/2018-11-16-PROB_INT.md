...
- 하지만 위의 정리에서 존재하는 $P$가 이것이 공간 $(\Omega_1\times \Omega_2, {\cal F}_ 1 \times {\cal F}_ 2)$ 에서 정의되는 모든 확률벡터 ${\boldsymbol X}=(X_1,X_2)$ 의 probability measure 가 됨을 의미하는 것은 아니다. 여기에서 $X_1$, $X_2$ 는 각각 확률공간 $(\Omega_1,{\cal F}_ 1, P_1)$, $(\Omega_2,{\cal F}_ 2, P_2)$ 에서 정의된 확률변수이다. (이때 ${\boldsymbol X}$가 확률벡터가 되는 것은 듀렛책 Thm 1.3.1 을 쓰면 쉽게 증명할 수 있음) 
...
\begin{align}
\int_{\Omega_1} \int_{\Omega_2} h(\omega_1,\omega_2) P_2(d\omega_2)P_1(d\omega_1)= \int_{\Omega_1 \times \Omega_2} h dP= \int_{\Omega_2}\int_{\Omega_1} h(\omega_2,\omega_1)\tilde\mu_1(d\omega_1)\tilde\mu_2(d\omega_2)
\end{align}
