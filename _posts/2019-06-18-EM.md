...
\end{align}
그리고 이때 $E(W_i \| z_i)=I(z_i>0)$와 같이 구할 수 있다. 

- 이제부터 그냥 생각없이 쭉쭉 풀면된다. 
\begin{align}
...
을 만족하는 방정식을 풀면 
\begin{align}
n^+ = \sum_{i \in \\{i:z_i=0\\}} e^{\gamma+{\bf x}_ i \boldsymbol\beta}
\end{align}
이 된다. 여기에서 $n^+=\sum_{i=1}^{n} I(z_i>0)$ 이다. 정리하면 아래와 같이 된다. 
\begin{align}
\gamma=\log \Big(\frac{n^+}{\sum_{i \in \\{i:z_i=0\\}} e^{{\bf x}_ i \boldsymbol\beta}} \Big) 
\end{align}
