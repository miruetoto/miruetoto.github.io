...
\begin{align}
E_{\bf W}\left[L(\lambda_1,\lambda_2,p)\right]
\end{align}
는 랜덤이 아니므로 이것을 최적화 할 수는 있을것 **같다.** 여기에서 당연히 $E$는 ${\bf W}$ 에 대한 평균이다. 그런데 사실  $E_{\bf W}\left[L(\lambda_1,\lambda_2,p)\right]$ 를 구하면 될 것 같다고 생각하는 사람은 통계센스가 절망적인 사람이다. (나처럼..) 왜냐하면
위를 계산하려면 $p$ 를 알아야 하는데 우리가 지금 하고 있는게 사실 $p$를 알기 위한 것임을 생각해보면 이 과정에서 **순환논리의 모순**이 생기기 때문이다. *($p$를 구하기 위해서는 ${\bf W}$를 관측해야 했음. 그런데 ${\bf W}$를 관측못한 상태에서 $p$를 구하는 방법은 ${\bf W}$의 관측값들을 평균처리하는 기법이라고 함. 그런데 ${\bf W}$의 관측값들을 평균처리하려면 $p$를 알아야함. ??????????????)* 가만히 생각해보면 우리가 지금까지 풀어오던 문제는 <br/><br/>
