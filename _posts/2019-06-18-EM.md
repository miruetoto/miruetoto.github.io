...
\begin{align}
\log L({\boldsymbol\theta}) = \sum_{i=1}^{n}\left\{ \log f(z_i|w_i) + \log f(w_i) \right\} 
\end{align}
평균을 취하면 
\begin{align}
E\left[ \log L({\boldsymbol\theta}) | z_i \right] = \sum_{i=1}^{n} \left\{ E\left[\log f(z_i|w_i) | z_i \right]+ E\left[\log f(w_i) | z_i \right]\right\} 
\end{align}
