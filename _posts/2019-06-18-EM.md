...
- 우선 현재까지 내가 이해하기로는 EM 알고리즘은 래이턴트-베리어블이 있을 경우 MLE 를 구하는 문제에 쓰인다. 즉 통계학에서 흔히 말하는 missing problem을 푸는데 사용된다. **미싱프라블럼** 이란 우리는 사실 $\bf (Z,W)$를 관측하고 싶었는데 어떠한 이유로 $\bf Z$만 관측한 상황을 의미한다. 여기에서 ${\bf Z}=c(Z_1,\dots,Z_n)$ 이고 ${\bf W}=c(W_1,\dots, W)$ 이다. 이런 상황에서 우리는 $\bf (Z,W)$ 의 **조인트-pdf** 를 알
