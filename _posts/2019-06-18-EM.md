...
- **미싱-프라블럼** 이란 우리는 사실 $\bf (Z,W)$를 관측하고 싶었는데 어떠한 이유로 $\bf Z$만 관측한 상황을 의미한다. 여기에서 ${\bf Z}=c(Z_1,\dots,Z_n)$ 이고 ${\bf W}=c(W_1,\dots, W)$ 이다. 이때 보통 ${\bf Z}$와 ${\bf W}$의 각 관측치는 독립이라고 가정하는것 같다.
