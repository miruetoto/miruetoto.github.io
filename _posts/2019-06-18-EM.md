...
를 풀어도 되니까 
\begin{align}
E\left[\log L(\lambda_1,\lambda_2,p)\right]=E \left[ \sum_{i=1}^{n}\left(\log W_i\frac{pe^{-\lambda_1} \lambda_1^{z_i}}{z_i !} \right)+(1-W_i) + \sum_{i=1}^{n}\left(\frac{(1-p)e^{-\lambda_2} \lambda_2^{z_i}}{z_i !} \right) \right] 
\end{align}
를 구하자. 
