...
- **미싱-프라블럼** 이란 우리는 사실 ${\bf Y}_ {com}=({\bf Y}_ {obs},{\bf Y}_ {mis})$를 관측하고 싶었는데 어떠한 이유로 $\bf Y_$만 관측한 상황을 의미한다. 여기에서 ${\bf Z}=c(Z_1,\dots,Z_n)$ 이고 ${\bf W}=c(W_1,\dots, W)$ 이다. 이때 보통 ${\bf Z}$와 ${\bf W}$의 각 관측치는 독립이라고 가정하는것 같다. 즉 $Z_1,\dots,Z_n$은 서로 독립이고 $W_1,\dots,W_n$도 서로 독립이라고 가정한다. 

- 이때 ${\bf X}={\bf (Z,W)}$ 에서  ${\bf X}$ 를 컴플리트-데이터라고 하고 $\bf Z$를 observed data 라고 한다. 일반적으로 항상 
\begin{align}
{\bf X}={\bf (Z,W)}
\end{align}
의 관계가 성립하는 것은 아니다. 랭은 $t({\bf X})={\bf Z}$ 를 만족하는 some function $t$가 있다고만 하였는데 이게 일반적인 형태인듯 하다. 따라서 우리가 일반적으로 두는 셋팅 ${\bf X}={\bf (Z,W)}$ 는 $t$ 가 ***simple projection*** 인 특수한 경우라 해석할 수 있다. 
