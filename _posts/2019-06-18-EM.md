...
L(\lambda_1,\lambda_2,p)=\prod_{i=1}^{n}\left(\frac{pe^{-\lambda_1} \lambda_1^{z_i}}{z_i !} \right)^{W_i} \left(\frac{(1-p)e^{-\lambda_2} \lambda_2^{z_i}}{z_i !} \right)^{1-W_i}
\end{align}
이다. ($W_i$가 포함되어서 랜덤이 된다..) 즉 잘 보면 $L(\lambda_1,\lambda_2,p)$ 는 $W_i$의 값들에 의해서 좌우하는 랜덤변수가 된다. 굳이 설명하면 아래와 같은 꼴이다. 
\begin{align}
L(\lambda_1,\lambda_2,p)=g(W_1,\dots,W_n)
\end{align}
일단 우리는 $W_i$에 대한 값을 모르므로 $L(\lambda_1,\lambda_2,p)$ 를 최적화할수는 없다. 하지만 
\begin{align}
E\left[L(\lambda_1,\lambda_2,p)\right]
\end{align}
는 랜덤이 아니므로 이것을 최적화 할 수는 있다. 즉 아래를 푸는 것은 가능하다. 
\begin{align}
\begin{cases}
