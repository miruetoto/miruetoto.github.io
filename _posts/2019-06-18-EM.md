...
\end{align}
그리고 이때 $E(W_i \| z_i)=p_iI(z_i>0)$와 같이 구할 수 있다. 이제부터 아래와 같이 놓고 생각없이 쭉쭉 풀면(=미분하면) 된다. 
\begin{align}
...
=I(z_i>0)\tilde p_i\frac{1}{e^{\gamma+{\bf x}_ i {\boldsymbol\beta}}+1}+ (1-I(z_i>0)\tilde p_i)\frac{-e^{\gamma+{\bf x}_ i {\boldsymbol \beta}}}{e^{\gamma+{\bf x}_ i {\boldsymbol\beta}}+1}
\end{align}
따라서
\begin{align}
