...
L(\lambda_1,\lambda_2,p)=\prod_{i=1}^{n}\left(\frac{pe^{-\lambda_1} \lambda_1^{z_i}}{z_i !} \right)^{W_i} \left(\frac{(1-p)e^{-\lambda_2} \lambda_2^{z_i}}{z_i !} \right)^{1-W_i}
\end{align}
이다. 위의 식은 $W_i$가 포함되어있으므로 랜덤이 된다. 즉 $L(\lambda_1,\lambda_2,p)$ 는 $W_i$의 값들에 의해서 좌우하는 **"function of random variable"** 이라고 볼 수 있다. 아무튼 우리는 $W_i$에 대한 값을 모르므로 $L(\lambda_1,\lambda_2,p)$ 는 랜덤이 되고 따라서 이를 최적화할 수는 없다. 하지만 
\begin{align}
