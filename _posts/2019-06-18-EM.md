...
- $\boldsymbol\beta$로 미분해보자. (벡터미분이 어렵다고 $\beta_j$로 미분할생각하면 진짜 엄청난 계산이 기다린다. 벡터나 행렬에서 **엘리먼트와이즈 연산** 은 남들이 해놓은거 이해하기는 쉽지만 본인이 하려면 엄청 힘들고 머리도 좋아야 한다. 머리 나쁘면 그냥 벡터미분하는게 정신건강에 좋다.)
\begin{align}
