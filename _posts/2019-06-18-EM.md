...
\begin{align}
E& \left[\log L(\lambda_1,\lambda_2,p) \BBig| {\bf z} \right] \\\\ 
&=E \left[ \sum_{i=1}^{n}\left( W_i\log\frac{pe^{-\lambda_1} \lambda_1^{z_i}}{z_i !}+(1-W_i)\log\frac{(1-p)e^{-\lambda_2} \lambda_2^{z_i}}{z_i !} \right) \BBig ~ ~ |~ ~ {\bf Z=z} \right]  \\\\ \\\\
&=E \left[ \sum_{i=1}^{n}\left( W_i\log\bigcirc +(1-W_i)\log\bigtriangleup \right)  \BBig ~ ~ | ~ ~ {\bf Z=z}  \right]  \\\\ \\\\
&=E \left[ \sum_{i=1}^{n}\left( W_i\log\frac{\bigcirc}{\bigtriangleup}+\log\bigtriangleup \right) \BBig ~ ~ | ~ ~ {\bf Z=z} \right] \\\\ \\\\
&=\sum_{i=1}^{n}\left( E(W_i|{\bf Z=z})\log\frac{\bigcirc}{\bigtriangleup}+\log\bigtriangleup \right)
\end{align}
