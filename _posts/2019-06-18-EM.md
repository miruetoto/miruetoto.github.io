...
- **미싱-프라블럼** 이란 우리는 사실 ${\bf Y}^{com}=({\bf Y}^{obs},{\bf Y}^{mis})$를 관측하고 싶었는데 어떠한 이유로 ${\bf Y}^{obs}$만 관측한 상황을 의미한다. 여기에서 ${\bf Y}^{obs}=c(Y^{\obs}_ 1,\dots,Y^{obs}_ n)$ 이고 ${\bf W}=c(W^{obs}_ 1,\dots, W^{obs}_ n)$ 이다. 이때 보통 ${\bf Z}$와 ${\bf W}$의 각 관측치는 독립이라고 가정하는것 같다. 즉 $Z_1,\dots,Z_n$은 서로 독립이고 $W_1,\dots,W_n$도 서로 독립이라고 가정한다. 
