...
를 풀어도 되니까 
\begin{align}
E\left[\log L(\lambda_1,\lambda_2,p)\right]=E \left[ \sum_{i=1}^{n}\left( W_i\log\frac{pe^{-\lambda_1} \lambda_1^{z_i}}{z_i !}+(1-W_i)\log\frac{(1-p)e^{-\lambda_2} \lambda_2^{z_i}}{z_i !} \right) \right] 
\end{align}
를 구하자. 특별한 생각 없이 쭉쭉 전개하면 아래처럼 된다. 
\begin{align}
E\left[\log L(\lambda_1,\lambda_2,p)\right]& \\\\ \\\\ 
&=E \left[ \sum_{i=1}^{n}\left( W_i\log\frac{pe^{-\lambda_1} \lambda_1^{z_i}}{z_i !}+(1-W_i)\log\frac{(1-p)e^{-\lambda_2} \lambda_2^{z_i}}{z_i !} \right) \right]  \\\\ \\\\
&=E \left[ \sum_{i=1}^{n}\left( W_i\log\bigcirc +(1-W_i)\log\bigtriangleup \right) \right]  \\\\ \\\\
&=E \left[ \sum_{i=1}^{n}\left( W_i\log\frac{\bigcirc}{\bigtriangleup}+\log\bigtriangleup \right) \right] \\\\ \\\\
&=\sum_{i=1}^{n}\left( p\log\frac{\bigcirc}{\bigtriangleup}+\log\bigtriangleup \right)
\end{align}
