...
아래를 가정하자. <br/><br/>
**(1)** 모든 관측치는 서로 독립이다. <br/>
**(2)** $W$ 는 확률이 $p$인 베르누이 분포에서 생성되었다. 여기에서 $p_i=\frac{\exp(\gamma+{\bf x_i}{\boldsymbol\beta})}{\exp(\gamma+{\bf x_i}{\boldsymbol\beta})+1}$. 이때 ${\bf x_i}$는 $1\times p$ 벡터이다. <br/>
...
- $p$로 미분하면 아래와 같은 식을 얻을 수 있다.
\begin{align}
\frac{\sum_{i=1}^{n}I(z_i>0)}{\sum_{i=1}^{n}\Big(I(z_i>0)+I(z_i=0)\Big)}=\frac{m}{n}
\end{align}
$\gamma$를 $p$에 대하여 정리하여보자. 
\begin{align}
p=\frac{e^{\gamma}e^{\bf X \boldsymbol \beta}} {1+e^{\gamma}e^{\bf X \boldsymbol \beta}}
\end{align}
이므로
\begin{align}
e^{\gamma}=\frac{p}{(1-p)e^{\bf X \boldsymbol \beta}} 
\end{align}
가 된다. 따라서 
\begin{align}
\gamma=\log\Big(\frac{m}{n-m}\Big) -\bf 1' X \boldsymbol \beta
\end{align}
가 된다. 
...
=\frac{1}{\sigma^2}\bf \Bigg(\tilde X' z -\tilde X'\tilde X {\boldsymbol\beta}  \Bigg)
\end{align}
...
이다. 따라서 
\begin{align}
\sum_{i=1}^{n} \left( \frac{\partial}{\partial\boldsymbol \beta}E\big[\log f(w_i) \| z_i \big]  \right) = \bf \tilde X' \tilde q - \check X' \check p 
\end{align}
가 된다. 
