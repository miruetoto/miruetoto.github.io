...
\begin{align}
f(w_i)=\Bigg[\frac{\exp({\bf 1}'\gamma+{\bf x}_ i {\boldsymbol\beta})}{\exp({\bf 1}'\gamma+{\bf x}_ i{\boldsymbol\beta})+1}\Bigg]^{w_i}\Bigg[1-\frac{\exp({\bf 1}'\gamma+{\bf x}_ i{\boldsymbol\beta})}{\exp({\bf 1}'\gamma+{\bf x}_ i{\boldsymbol\beta})+1}\Bigg]^{1-w_i}
\end{align}
\begin{align}
f(z_i|w_i)=
...
- 여기에서 $E(W_i|z_i)=I(z_i>0)$과 같이 구할 수 있다. 이 예제의 경우 $E(W_i | z_i)=I(z_i>0)$가 모수 $\boldsymbol \theta$에 의존하지 않는것이 특징이다. 
