...
- this function pull the columns to start of dataframe. 
...
- Let me explain the missing value, *NA*. 
```r
...
- note that you can't check missing value by *x==NA*. you should ckeck it by 
```r
> is.na(x)
#> [1] TRUE
```
...
- (1) make new variable named "gain" and (2) remove the missing value and (3) make another new varible named "gain_zscore" using "gain".
```r
> flights %>% 
  mutate(gain = dep_delay - arr_delay) %>%
  filter(!is.na(gain)) %>%
