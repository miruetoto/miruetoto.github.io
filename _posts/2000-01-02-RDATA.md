...
- note that row-vector with single letter convert to scalar. 
\begin{align}
\begin{bmatrix} 
\mbox{"x"} & 1 & 2
...
- note that the function *str_c()* could be applied to col-vector. 
```r
...
- note that in the case of *str_c("x",1,2)* we diliver 3 inputs with dimension 1 and in the case of  *str_c(1:5)* we diliver 1 input with dimension 5. Thus in the case *str_c("x",1,2)* we toss row-vector such that 
\begin{align}
["x",1,2]
\end{align} 
...
- the following is also possible like *str_c(col-vec,scalar,col-vec)*. 
```r
> str_c(1:5,'-',2:6)
