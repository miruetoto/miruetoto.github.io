...
- 만약에 $Z_{t-k}$가 (아무리 커도 어떠한 고정된) 상수라면 $\|\phi\|<1$이라는 제약아래에서 $\lim_{k\to\infty}\phi^{k}Z_{t-k}=0$이 성립하겠다. 고정된 상수가 아니고 $\|Z_{t-k}\|\leq M$임을 알아도 극한이 0임을 주장할 수 있다. 하지만 $Z_{t-k}$이 확률변수이므로 이러한식의 주장을 할 수 없다. 결국 $Z_{t-k}$가 큰 값이 나올 **확률**이 너무 적지 않겠냐? 라는 식의 논리전개를 해야하고 따라서 **확률, 평균**적 개념을 넣어야 한다. 결론적으로 말하면 $k\to \infty$일때 아래와 같이 평균적인 개념을 통하여서만 $Z_t=\sum_{j=0}^{\infty}\phi^{j}
\begin{align}
Z_t=\sum_{h=0}^{\infty}\phi^h\epsilon_{t-h}
\end{align}
