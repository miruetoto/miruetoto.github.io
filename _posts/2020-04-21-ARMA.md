...
\end{align}
와 같이 표현된다. 일반적인 AR모델은 과거에서부터 순차적으로 미래로 확률변수들을 생성하는데, 이 모델은 미래에서 순차적으로 확률변수들을 생성한다고 생각하면 된다. 이런방식으로 무한한 과거상태로 간다면 이 모델은 정상상태에 있다고 주장할 수 있다. 이러한 모델을 ***stationary future dependent AR(1) model*** 이라고 한다. 
