...
\begin{align}
\begin{cases}
\mbox{AR식 표현: } \mbox{function of } \dots,Z_{t-1},Z_t,Z_{t+1},\dots = \epsilon_t \\\\ \\
\mbox{MA식 표현: } Z_t = \mbox{function of } \epsilon_{t-1},\epsilon_t,\epsilon_{t+1},\dots
\end{cases}
\end{align}
...
- 다시 짜증나기 시작하지만 당연히 해결책이 있다. 해결책 역시 AR의 경우와 유사한데 하나의 모델을 버린다. 이 예제에서는 $\\{Y_t\\}$를 버린다. $\\{Y_t\\}$를 버리는 이유를 알아보기 위해서 $\\{Z_t\\}$와 $\\{Y_t\\}$ 모두 AR류로 표현해보자. 
\begin{align}
