...
\end{align}
와 같은 식으로 표현해야 할 것 같은데 이러한 표현은 causal 하지 않다. 따라서 $\\{Y_t\\}$를 버린다. 이때 버려진 $\\{Y_t\\}$는 invertalbe 하지 않은 MA라고 하고 버려지지 않은 $\\{Z_t\\}$는 invertalbe 한 MA라고 표현한다. 

- $Z_t\leftrightarrow \epsilon_t$를 잠시 바꾸어서 생각해보면 $\overset{L_2}{=}$가 성립하게 하려면 어떻게 해야하는지에 대한 논리전개를 AR과 동등하게 할 수 있다. 따라서 MA과정이 invertable하기 위한 조건은 
\begin{align}
Z_t=\boldsymbol{\theta}(B)\epsilon_t
\end{align}
꼴로 표현하였을 경우 $\boldsymbol{\theta}(B)=0$의 근의 절대값이 모두 단위원 밖에 있어야 한다는 조건과 같음을 쉽게 유추할 수 있다. 

- 이와 유사하게 대부분의 경우에 $Z_t\leftrightarrow \epsilon_t$를 잠시 바꾸어서 생각해본뒤에 논리전개를 하면 AR과정에서 공부하였던 여러가지 성질들을 MA과정으로 바꾸어 생각할 수 있게 된다. 이러한 성질을 쌍대성이라고 한다. 예를들면 미래값에 의존하지 않는 정상AR(1)과정의 ACF가 지수적으로 감소하고 PACF는 lag=1까지만 존재하고 그 이후로는 절삭되는데 이를 통하여 invertable한 MA(1)과정의 PACF는 지수적으로 감소하고 ACF는 lag=1까지만 존재하고 그 이후로는 절삭됨을 유추할 수 있는 것과 비슷하다. 
