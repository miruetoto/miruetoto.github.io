...
\begin{align}
\epsilon_t=Z_t+0.2Z_{t-1}+0.2^2 Z_{t-2}+\dots \\\\ \\ 
\eta_t=Y_t+5Y_{t-1}+5^2 Y_{t-2}+\dots 
\end{align}
딱봐도 $Y_t$를 버려야 할 것처럼 생겼다. 

- 위에서 등호는 사실 진짜 등호가 아니라 $\overset{L_2}{=}$임을 기억하자. 따라서 등호를 성립하게 하려면 오히려 
\begin{align}
\eta_t=Y_t+0.2Y_{t+1}+0.2^2 Y_{t+2}+\dots 
\end{align}
와 같은 식으로 표현해야 할 것 같은데 이러한 표현은 causal하지 않다. 
