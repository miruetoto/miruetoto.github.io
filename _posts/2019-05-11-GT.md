...
\sigma(S):=\frac{E(S,V-S)}{\frac{d}{|V|}|S||V-S|}
\end{align}
을 잘 살펴보면 컷 $S$에 대한 스팔시티는 (1) 파티션(=컷) 사이의 연결수가 많을수록 (2) $|S|$와 $|V-S|$의 불균형이 심할수록 그 값이 크다는 것을 알 수 있다. 이때 분모에 있는 $|V|$는 고정된 값이므로 고려할 필요가 없다. 극단적으로 생각하면 가장 스필시티가 큰 경우는 하나의 점이 모든 다른점들과 연결되어 있는 따라서 $\sigma(S)$는 $S$에 대한 importance 비슷한 메져라고 생각할 수 있다. 즉 **스팔시티**는 중요성, 혹은 **언스무스니스(unsmoothness)** , **outlierness** 등으로 이해할 수 있다. 

- 아무튼 그래프 ${\cal G}$의 가능한 모든 컷의 조합중 가장 작은 스팔시티값 $\sigma(S)$를 **그래프의 스팔시티** (sparsity of a graph) 라고 정의하고 기호로는 $\sigma({\cal G})$라고 쓴다. 
\begin{align}
\sigma({\cal G})= \underset{S \subset V : S \neq \emptyset, S \neq V}{\operatorname{min}} \sigma(S)
\end{align}
따라서 그래프의 스팔시티는 전체자료의 언스무스니스 정도의 개념으로 이해할 수 있다. 그래프의 스팔시티가 작다는 것은 그래프안에 있는 모든 버티스들이 매끄럽게 (평등한 정도의 중요도를 가지고) 연결되어 있다는 것을 의미한다. 
