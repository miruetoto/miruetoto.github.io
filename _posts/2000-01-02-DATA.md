...
\mbox{"x1y2z3"}
\end{align}
이를 이용해서 아래와 같이 *tibble*형 자료에서 *mutate*와 같이쓰면 엄청난 위력을 발휘한다. 
```r
### r
...
- 참고로 *str_c()*는 벡터에도 적용가능하다. 예를들어 아래코드를 돌릴 수 있다. <br/><br/>
```r
### r
> str_c(c("x",1),c("y",2),c("z",3)) #인풋의개수:3 / 인풋의차원:2
[1] "xyz" "123"
```
기본적으로 *str_c()*안에 전달되는 입력값들은 *cbind()*로 묶여있다고 생각하면 편리하다. 이것은 아래와 같은 느낌이다. 
\begin{align}
\begin{bmatrix} 
...
\end{bmatrix}
\end{align}
이를 응용하면 1:5까지 숫자들을 생성한뒤 이를 문자열로 바꾸는 방법을 알 수 있다. 
```r
### r
