...
---
### str_c()
- 이 연산은 기본적으로 여러개의 스칼라 입력을 스칼라로 바꿔준다. <br/><br/>
```r
### r
> str_c("x",1,"y",2,"z",3) # 인풋의개수:6 / 인풋의차원:1
[1] "x1y2z3"
```
느낌을 이해하는게 중요한데 이는 마치 입력을 $["x",1,"y",2,"z",3]$와 같이 row-vector를 하나로 묶어버리는 느낌이 있다. 즉 기본적으로 *str_c()*안에 전달되는 입력값들은 *cbind()*로 묶여있다고 생각하면 편리하다. 
\begin{align}
\begin{bmatrix} 
\mbox{"x"} & 1 & \mbox{"y"} & 2 & \mbox{"z"} & 3
\end{bmatrix} \Longrightarrow 
\mbox{"x1y2z3"}
\end{align}
참고로 *str_c()*는 벡터에도 적용가능하다. 예를들어 아래코드를 돌릴 수 있다. 
```r
### r
> str_c(c("x",1),c("y",2),c("z",3)) #인풋의개수:3 / 인풋의차원:2
[1] "xyz" "123"
```
이것은 아래와 같은 느낌이다. 
\begin{align}
\begin{bmatrix} 
\mbox{"x"} & \mbox{"y"} & \mbox{"z"} \\\\ \\
1 & 2 & 3 
\end{bmatrix} \Longrightarrow 
\begin{bmatrix} 
\mbox{"xyz"} \\\\ \\
...
\end{bmatrix}
\end{align}
이를 응용하면 1:5까지 숫자들을 생성한뒤 이를 문자열로 바꾸는방법을 알 수 있다. 
```r
### r
