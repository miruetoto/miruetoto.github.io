...
# We don't know!```
```
따라서 NA인지 체크하기 위해서 x==NA 와 같은 방식으로 체크할 수 없다. 그래서 아래와 같이 한다. 
```r
### r 
is.na(x)
#> [1] TRUE
...
```
대충 아래의 순서로 처리된다. (1) 자료에서 year:day, 그리고 끝에 "delay"를 포함한 변수명, distance, air_time 을 선택한다. (2) 적당한 공식에 따라서 gain, speed를 정의한다. (3) gain의 결과를 보면 NA가 포함된것이 있다. filter()를 활용하여 결측치를 제거한다. (4) gain_zscore를 얻는다. 이때 가장 인상적인 부분은 gain_zscore인데 이는 mutate()가 mean(), sd()와 같은 aggregate function 들과 함꼐 활용될 수 있다는 점이다. 
