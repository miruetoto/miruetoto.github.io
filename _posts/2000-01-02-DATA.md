...
---
### str_c()
- 이 연산은 여러개의 스칼라 입력을 스칼라로 바꿔준다. <br/><br/>
```r
### r
> str_c("x",1,"y",2,"z",3)
[1] "x1y2z3"
```
느낌을 이해하는게 중요한데 이는 마치 $["x",1,"y",2,"z",3]$로 이루어진 row-vector를 하나로 묶어버리는 느낌이 있다. 즉 아래의 느낌이다. 
\begin{align}
\begin{bmatrix} 
...
\end{bmatrix}
\end{align}
즉 기본적으로 *str_c()*안에 전달되는 입력값들은 *cbind()*로 묶여있다고 생각하면 편리하다. 
