...
- 확률변수열 $X_1,X_2,\dots$ 이 적당한 확률변수 $X$로 분포수렴한다는 의미는 
\begin{align}
\lim_{n\to\infty}P(X_n\leq x)=P(X\leq x)
\end{align}
라는 뜻이다. 여기에서 $X_n$대신에 $\sqrt{n}(\bar{Y}-\mu_y)$을 $X$대신에 $Z^* \sim N(0,\sigma^2)$을 대입하면 중심극한 정리가 된다. 

- 그런데 $P(X_n\leq x)=F_n(x)$, $P(X\leq x)=F(x)$라고 두면 위의 정의를 아래와 같이 쓸 수 있다. 
\begin{align}
\lim_{n\to\infty}F_n(x)=F(x)
\end{align}

\begin{align}
\lim_{n\to\infty}F_n(x)=F(x), \quad \forall x:~~ x \in 
\end{align}
