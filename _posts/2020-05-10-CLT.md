...
\begin{align}
\sqrt{n}(\bar{X}-\mu)\overset{d}{\to} \sim N(0,\sigma^2)
\end{align}
...
\end{align}
[1차미분항의 다항식 부분]은 $x\to 0 $ 일 경우 $0$으로 수렴하지 않고 나머지항목들은 여전히 수렴한다. 왜냐하면 [1차미분항의 다항식부분]은 $(x-0)$과 똑같은 속도로 $0$으로 향해 갔는데 [2차미분항의 다항식부분]은 $(x-0)$보다 더 빠른 속도로 $0$으로 향했으며 [3차미분항의 다항식부분]은 더 빠륵 $0$으로 향해갔기 때문이다. 
...
& \Longleftrightarrow \lim\sup_{x\to a}\left\|\frac{f(x)}{g(x)}\right\| <  \infty 
\end{align}
