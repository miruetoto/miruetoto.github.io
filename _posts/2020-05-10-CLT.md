...
\begin{align}
\frac{\bar X-\mu}{\sigma^2/\sqrt{n} } \overset{d}{\to} Z, \quad Z\sim N(0,1)
\end{align}
그리고 $\frac{\bar X-\mu}{\sigma^2/\sqrt{n} }$가 표준정규분포를 따르는 확률변수 $Z$로 분포수렴한다고 한다. 
...
\begin{align}
\lim_{n \to \infty}mgf_{Z_n}(t)=mgf_Z(t) \forall t: \|t\|<h (\exist h>0) \quad \Longrightarrow \quad \lim_{n\to\infty}P(Z_n\leq x)=P(Z\leq x) \forall x
\end{align}
```
