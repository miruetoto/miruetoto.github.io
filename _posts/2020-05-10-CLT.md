...
\end{align}
와 같이 된다. 여기에서 $\dots$는 나머지텀이라고 부르는데 $x\to 0$ 일 수록 0에 빠르게 가까워지는 항목이다. 사실 $m(0)$을 제외한 모든 항목은 $x\to0$ 일수록 0에 가까워지는데, 이론적으로 고차항일수록 더욱 빠르게 가까워진다. 예를들어서 
\begin{align}
(x-0)m(0)
\end{align}
보다 
...
\frac{(x-0)^2}{2!}m(0)
\end{align}
이 더욱 빠르게 가까워지는데 그이유는 자명하게도 $(x-0)$보다 $\frac{(x-0)^2}{2!}$이 더욱 빠르게 $0$으로 수렴하기 때문이다. 나머지항들은 $\frac{(x-0)^2}{2!}$항보다 더욱 고차항이기 때문에 
