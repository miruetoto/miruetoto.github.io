...
\begin{align}
m(x)=1+xE\bigg(\frac{X_1-\mu}{\sigma}\bigg)+\frac{x^2}{2}E\bigg(\frac{X_1-\mu}{\sigma}\bigg)^2+\dots 
\end{align}
그런데 $E\big(\frac{X_1-\mu}{\sigma}\big)=0$이고 $E\big(\frac{X_1-\mu}{\sigma}\big)^2=1$이므로 
\begin{align}
m(x)=1+\frac{x^2}{2}+\dots
\end{align}
...
\end{align}
이라고 하였는데 $m(t/\sqrt{n})=1+\frac{t^2}{2n}+\dots$이라고 쓸 수 있으므로 아래와 같이 쓸 수 있다. 
\begin{align}
mgf_{\sqrt{n}(\bar{X}-\mu)/\sigma }(t)=\bigg(1+\frac{t^2}{2n}+\dots \bigg)^n
\end{align}

- 그런데 
\begin{align}
m(t/\sqrt{n})=mgf_{\frac{X_1-\mu}{\sigma} }(t/\sqrt{n})=mgf_{\frac{\sqrt{n}(X_1-\mu)}{\sigma} }(t)
\end{align}
이므로 결론적으로 
\begin{align}
m(t/\sqrt{n}) \to mgf_Z(t) \quad as ~n\to\infty
\end{align}
임을 보이면 된다. 
...
\begin{align}
m(x)=&m(a)+R(x,a), \quad \quad R(x,a)=O(x-a) \quad as ~ x \to a \\\\ \\
m(x)=&m(a)+(x-a)m^{(1)}(a) +R(x,a), \quad \quad R(x,a)=O\big((x-a)^2\big) \quad as ~ x \to a \\\\ \\
m(x)=&m(a)+(x-a)m^{(1)}(a)+\frac{(x-a)^2}{2!}m^{(2)}(a) +R(x,a), \quad \quad R(x,a)=O\big((x-a)^3\big) \quad as ~ x \to a \\\\ \\
\dots
\end{align}
