...
\lim_{n\to \infty} \log mgf_{\sqrt{n}(\bar{X}-\mu)/\sigma }(t) = t^2/2
\end{align}
임을 보이기 위해서는 아래를 보이면 될 것 같다.
\begin{align}
\log mgf_{\sqrt{n}(\bar{X}-\mu)/\sigma }(t) \approx n \log \bigg(1+\frac{t^2}{2n}\bigg) \to \frac{t^2}{2} \quad as ~ n \to \infty 
\end{align}
...
이 성립한다. 따라서 
\begin{align}
\log mgf_{\sqrt{n}(\bar{X}-\mu)/\sigma }(t) \approx n \log \bigg(1+\frac{t^2}{2n}\bigg) \approx \frac{t^2}{2} \to \frac{t^2}{2} \quad as ~ n \to \infty 
\end{align}
가 된다. 따라서 $n\to \infty$ 일수록 물결이 점점 등호에 가까워짐을 보이면 된다. 
