...
\begin{align}
f(x)=f(a)+(x-a)\times f^{(1)}(a)+\frac{(x-a)^2}{2!}\times f^{(2)} + \dots
\end{align}

- 우리의 문제는 $f$대신에 $m$을 대입하고 $a$대신에 $0$을 대입하면 된다. 
...
- 따라서 테일러 전개가 가능하려면 $m(x)$를 $x=0$에서 반복적으로 미분하였을때 미분값이 갑자기 무한대로 터지는 경우는 제외해야 한다. 이를 수학적으로는 **"$m(x)$가 $x=a$에서 무한번 미분가능하다면 $m(x)$를 $x=a$에서 테일러전개할수 있다"** 라고 표현한다. (우리의 예제는 $a=0$인 경우이다.)
