...
- 위의 말은 결국 분포수렴임을 보이려면 원래 cdf가 수렴함을 보여야하는데 극한분포가 연속일경우 (예를들면 표준정규분포 같은) mgf가 수렴함을 보이면 충분하다는 의미이다. 따라서 결국 우리는 $\frac{\sqrt{n}(\bar{X}-\mu)}{\sigma^2}$의 mgf가 $n \to \infty$일때 $e^{t^2/2}$로 수렴함을 보이면 된다. 즉 
\begin{align}
lim_{n\to \infty} mgf_{\frac{\sqrt{n}(\bar{X}-\mu)}{\sigma^2} }(t) = e^{t^2/2}
\end{align}
임을 보이면 된다. 