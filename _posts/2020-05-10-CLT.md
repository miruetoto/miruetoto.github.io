...
\begin{align}
m(x)=&m(a)+R(x,a), \quad \quad R(x,a)=O\big((x-a)\big) \quad as ~ x \to a \\\\ \\
...
간략하게 아래와 같이 쓰기도 한다. 
\begin{align}
m(x)=&m(a)+O\big((x-a)\big), \quad \quad as ~ x \to a \\\\ \\
...
\begin{align}
m(x)=&m(a)+o\big(1\big), \quad \quad as ~ x \to a \\\\ \\
m(x)=&m(a)+(x-a)m^{(1)}(a)+o\big((x-a)\big), \quad \quad as ~ x \to a \\\\ \\
