...
- 만약에 $(X'X)^{-1}$가 존재하지 않는다면 어떻게 되는가? 본질적으로 이런 경우는 $\bf X$의 랭크가 $p$보다 작은 경우에 생긴다. 이러한 일은 1) $n<p$이거나 2) $X$중에서 선형적으로 종속된 변수들이 있을 경우 자주 발생한다. 1)의 경우는 $rank({\bf X})=n$이고 2)의 경우는 $rank({\bf X})=p-\textup{선형종속된 변수들}$이 된다.  
