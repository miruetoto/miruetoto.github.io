...
\sigma(S):=\frac{E(S,V-S)}{\frac{d}{|V|}|S||V-S|}
\end{align}
을 잘 살펴보면 컷 $S$에 대한 스팔시티는 (1) 파티션(=컷) 사이의 연결수가 많을수록 (2) $|S|$ 가 작을수록 카다는 것을 알 수 있다. 따라서 $\sigma(S)$는 $S$에 대한 importance 비슷한 메져라고 생각할 수 있다. 즉 스팔시티는 중요성, 혹은 언스무스니스 (unsmoothness) , outlierness 등으로 이해할 수 있다. 
