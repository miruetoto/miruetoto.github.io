...
- 아무튼 우리의 목적은 주어진 그래프 ${\cal G}$에서 스팔시스트-컷을 찾는것이다. 즉 $\sigma(S)$를 최소화하는 컷 $(S,V-S)$를 찾는 것이다. 이때의 $\sigma(S)$를 **그래프의 스팔시티** (sparsity of a graph) 라고 정의하고 기호로는 $\sigma({\cal G})$라고 쓴다. 
\begin{align}
