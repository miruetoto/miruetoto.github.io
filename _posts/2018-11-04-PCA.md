...
\begin{align}	
\begin{cases}	
{\bf T}_ {p \times p'}=[T_1,\dots,T_ {p'}]=[{\boldsymbol \psi}_ 1, \dots,{\boldsymbol \psi}_ {p'}]={\bf \bar\psi}	 \\\\ \\
{\bf K}_ {p \times p'}=diag(k_ 1 , \dots , k_ {p'})=diag(\lambda_ 1 , \dots , \lambda_ {p'})={\bf \bar\Lambda}	
\end{cases}	
\end{align}	
이다. 여기에서 ${\bf \bar\psi}_ {p \times p'}$ 와 ${\bf \bar\Lambda}_ {p \times p'}$ 는 각각 ${\bf \psi}_ {p\times p}$ 와 ${\bf \Lambda}_{p\times p}$ 의 짤린버전이다. 

- 위의 증명은 그냥 ${\bf X}'{\bf X}{\bf T}={\bf T}{\bf K}$ 에다가 ${\bf T}_ {p \times p'}=[{\boldsymbol \psi}_ 1, \dots, {\boldsymbol \psi}_ {p'}]$를 대입하면 쉽게 할 수 있다. (식 ${\bf T}'{\bf T}={\bf I}$는 고유벡터의 정의에 의해서 그냥 성립하므로 체크할 필요도 없다.) 	
\begin{align}	
{\bf X}'{\bf X}{\bf T}&
