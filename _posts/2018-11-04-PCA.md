...
\end{cases}	
\end{align}	
을 만족하는 ${\bf T}_ {p\times p'}$는 ${\bf X}'{\bf X}$의 고유벡터모임 ${\bf \Psi}_ {p\times p}$에서 **뒷부분에 위치한 덜 중요해보이는 $(p-p')$개의 고유벡터를 삭제한 것**과 같고, 위의 식을 만족하는 ${\bf \Lambda}$는 ${\bf X}'{\bf X}$의 고유치모임 ${\bf \Lambda}_ {p\times p}$에서 **뒷 부분에 위치한 덜 중요해보이는 $(p-p')$개의 고유치를 삭제한것**과 같다. 즉 	
\begin{align}	
\begin{cases}	
{\bf T}_ {p \times p'}=cbind({\boldsymbol \psi}_1, \dots,{\boldsymbol \psi}_p')	
