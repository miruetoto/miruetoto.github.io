...
- 종종 left-eigen vector (좌고유벡터) 라는 용어를 쓰는 사람도 있다. 좌고유벡터는 우고유벡터와는 반대로 매트릭스의 왼쪽에 곱해주는 벡터이다. (따라서 모양은 당연히 가로로 긴 row가 된다.) 우고유벡터와 좌고유벡터를 곱하는 느낌은 대충 아래와 같다. 
\begin{align}
\begin{bmatrix}
...
- ${\bf A}$의 좌고유벡터를 얻기 위해서는 ${\bf A}'$의 (우)고유벡터를 구한뒤 row형태로 써주기만 하면 된다. 

- ${\bf A}$가 대칭행렬이면 좌고유벡터와 우고유벡터의 원소가 같다. 
