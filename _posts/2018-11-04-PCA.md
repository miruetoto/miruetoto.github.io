...
\end{cases}	
\end{align}	
을 만족하는 ${\bf T}_ {p\times p'}$는 ${\bf X}'{\bf X}$의 고유벡터모임 ${\bf \Psi}_ {p\times p}$에서 **뒷부분에 위치한 덜 중요해보이는 $(p-p')$개의 고유벡터를 삭제한 것**과 같고, 위의 식을 만족하는 ${\bf K}$는 ${\bf X}'{\bf X}$의 고유치모임 ${\bf \Lambda}_ {p\times p}$에서 **뒷 부분에 위치한 덜 중요해보이는 $(p-p')$개의 고유치를 삭제한것**과 같다. 즉 	
\begin{align}	
\begin{cases}	
{\bf T}_ {p \times p'}=[T_1,\dots,T_ {p'}]=[{\boldsymbol \psi}_ 1, \dots,{\boldsymbol \psi}_ {p'}]	 \\\\ \\
{\bf K}_ {p \times p'}=diag(k_ 1 , \dots , k_ {p'})=diag(\lambda_ 1 , \dots , \lambda_ {p'})	
\end{cases}	
\end{align}	
이다. 증명은 그냥 ${\bf X}'{\bf X}{\bf T}={\bf T}{\bf K}$ 에다가 ${\bf T}_ {p \times p'}=cbind({\boldsymbol \psi}_ 1, \dots, {\boldsymbol \psi}_ {p'})$를 대입하면 쉽게 할 수 있다. (식 ${\bf T}'{\bf T}={\bf I}$는 고유벡터의 정의에 의해서 그냥 성립하므로 체크할 필요도 없다.) 	
\begin{align}	
{\bf X}'{\bf X}{\bf T}&={\bf X}'{\bf X}cbind({\boldsymbol \psi}_ 1, \dots, {\boldsymbol \psi}_ {p'}) \\\\ 
&=cbind({\bf X}'{\bf X}{\boldsymbol \psi}_ 1, \dots, {\bf X}'{\bf X}{\boldsymbol \psi}_ {p'}) = cbind({\boldsymbol \psi}_ 1 \lambda_ 1, \dots, {\boldsymbol \psi}_ {p'}\lambda_{p'}) \\\\ 	
&=cbind({\boldsymbol \psi}_ 1, \dots, {\boldsymbol \psi}_ {p'})diag(\lambda_1,\dots,\lambda_{p'})={\bf T} {\bf K}	
\end{align}

### 자유로운 식 변환 (주성분분석, 고유치분해, SVD)

- 자료를 축소하지 않으면 ${\bf T}_ {p \times p}={\bf V}_ {p \times p}$ $$라고 주장할 수 있다. 여기에서 $\bf V$는 ${\bf X}'{\bf X}$의 고유벡터이다. 즉 아래가 성립한다. 
\begin{align}
{\bf X}'{\bf X}={\bf V}{\bf D}{\bf V}'
\end{align}
참고로 ${\bf X}$의 SVD는 아래와 같이 표현가능하다. 
\begin{align}
\end{align}

- 자료를 축소하지 않으면 아래와 같은 식이 성립한다. 
\begin{align}	
{\bf Z}_ {n\times p} = {\bf X}_ {n \times p} {\bf T}_ {p \times p} 	
\end{align}	
여기에서 ${\bf T}{\bf T}'={\bf I}$ 임을 활용하면 아래가 성립한다. 
\begin{align}	
{\bf X} = {\bf Z}{\bf T}' 
\end{align}	
