...
- 종종 left-eigen vector 와 right-eigen vector 라는 용어를 쓰는 사람이 있다. 우고유벡터는 col형태로 오른쪽에 곱하며 좌고유벡터는 row형태로 왼쪽에 곱한다. 모양은 대충 아래와 같은 느낌이다. 
\begin{align}
\begin{bmatrix}
...
- 따라서 정의상 아래문장이 성립한다. 
```
A left eigenvector of $\bf A$ is the same as the transpose of a right eigenvector of ${\bf A}^T$ with same eigen value.
'''
...
\end{align}
여기에서 가운데 $\bf \Gamma=DV'VD'$는 새로운 대각행렬로 볼 수 있다. 따라서 $\bf U$는 $\bf XX'$ 의 right-eigen vector 가 되고 $\bf \Gamma$는 그에 대응하는 고유치가 된다. 

 left eigenvector of {\displaystyle A}A is the same as the transpose of a right eigenvector of {\displaystyle A^{\textsf {T}}}{\displaystyle A^{\textsf {T}}}, 
