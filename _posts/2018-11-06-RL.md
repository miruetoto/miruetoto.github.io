...
{\bf\tilde  P}: {\cal S} \times {\cal A} \times {\cal R} \times {\cal S}  \to [0,1]
\end{align}
로 정의할 수 있고 ${\bf\tilde  P}[s,a,r,s']$ 의 차원을 $\Big(\|{\cal S}\|,\|{\cal A}\|,\|{\cal R}\|,\|{\cal S}\|\Big)$ 로 생각해도 무방하다. 그리고 당연하겠지만 given $(s,a)$ 일때 환경이 줄 수 있는 모든 경우의 수 $(r,s')$ 에 대한 확률의 총합은 1 이므로 ${\bf\tilde  P}[s,a,r,s']$ 에서 $(s,a)$를 고정시켜서 나오는 2차원 array의 총합도 1 이다. 
...
\end{align}
이건 간단하게 말해서 에이전트가 상태 $S_t=s$ 에서 행동 $A_t=a$ 을 할 확률을 의미한다. (우리는 MDP를 가정하고 있으므로 앞으로 아래첨자는 생략하여 쓰겠다.) 따라서 polish는 차원이 ${\cal S} \times {\cal A}$ 인 ${\boldsymbol \Pi}[s,a]$ 와 같은 테이블에 $[0,1]$ 사이의 확률값들을 기록한 것으로 생각 할 수 있다. 포스팅 도입부에 소개한 $4\times 4$ 그리드가 있는 예제를 다시 떠올려보자. 에이전트가 $s$에 상관없이 로봇을 동서남북 아무방향이라 랜덤으로 움직이는 정책을 가지고 있다 가정하면 모든 $16\times 4$ 차원의 테이블에 모두 0.25의 값을 넣은 것과 같다. 즉 모든 $(s,a)$ 에 대하여 
\begin{align}
{\boldsymbol \Pi}[s,a] = 0.25 
\end{align}
와 같이 된다. 

- 특정 상태 $s \in {\cal S}$에 대하여 정책 $\pi^{(1)}(a|s)$가 좋은 정책인지 나쁜 정책인지 평가할 수 있다. 여기에서 숫자 1 은 첫번째 정책이라는 의미이다. 환경이 가진 테이블 $p(s',r|s,a)$과는 다르게 에이전트가 가진 테이블 $\pi(a|s)$는 업데이트가 된다. 즉 
\begin{align}
\pi^{(1)}(a|s) \to \pi^{(2)}(a|s) \to .. 
\end{align}
이런식으로 업데이트 하면서 더 좋은 테이블로 점점 수정해 나간다. 정책 $\pi^{(1)}(a|s)$ 이 좋은 정책인지 나쁜정책인지는 어떻게 알 수 있을까? 정책 $\pi(a|s)$ 가 좋은 정책인지 나쁜정책인지 평가하기 위해서는 아래를 계산해야 한다. 
\begin{align}
v_{\pi}(s):=\mathbb{E}_ {\pi} \bigg( \sum_{k=0}^{\infty}\gamma^k R_{t+k+1} \bigg\| S_t=s\bigg).
\end{align}
요걸 ***value function*** 이라고 한다. 정책 $\pi(a|s)$에 대한 value function $v_{\pi}(s)$은 정책 $\pi(a|s)$를 반복하여 썼을때 특정 상태 $s \in {\cal S}$에서 받게되리라 기대되는 보상값(=단기+장기 합쳐서)이라고 해석 할 수 있다. 만약 모든 $s \in {\cal S}$에 대하여 아래가 성립한다면 
\begin{align}
v_{\pi'}(s) > v_{\pi}(s)
\end{align}
정책 $\pi'(a|s)$ 가 정책 $\pi(a|s)$ 보다 좋은 정책이라고 생각할 수 있다. 이런식으로 확장하면 더 이상 개선할 수 없는 정책이 있을텐데 이를 최적의 정책 $\pi^* (a|s)$ (이제부터 ***optimal policy*** 라고 부르자) 라고 한다. 좀더 엄밀하게는 가능한 모든 $\pi(a|s)$ 에 대하여 
\begin{align}
\forall s \in {\cal S}: v_{\pi^* }(s) \geq v_{\pi}(s)
\end{align}
이 성립할때 $\pi^* (a|s)$ 를 optimal policy 라고 한다. 

- 앞으로 논의를 편하게 하기위해서 현재의 정책을 $\pi(a|s)$라고 하고 나중의 정책은 $\pi'(a|s)$ 이라고 하자. 그리고 아래를 가정하자. 
\begin{align}
\begin{cases}
\pi^{(1)}(a|s) = \pi(a|s) \\\\ \\ 
\pi^{(2)}(a|s) = \pi'(a|s) \\\\ 
...
\end{align}
지금까지의 논리를 종합하면 <br/>
**step 1.** 특정 정책 $\pi$ 에 대한 value function $v_{\pi}(s)$ 를 계산하고 <br/>
**step 2.** 정책 $\pi$를 $\pi'$ 로 업데이트 하는것을 반복하면 <br/>
최적의 정책 $\pi^* $ 를 찾을 수 있을 것 같다. 이런식으로 $\pi^* $ 를 찾아내는 과정을 ***polish iteration algorithm*** 이라고 한다. 

- **step 1.** 이제 value function을 계산하는 방법에 대하여 알아보자. Value function $v_{\pi}(s)$를 아날래틱하게 풀기 위해서는 모든 $s \in {\cal S}$에 대하여 아래의 식을 만족하는 $v_{\pi}(s)$ 값을 찾아야 한다. 
\begin{align}
v_{\pi}(s)=\sum_{a}\pi(a|s)\sum_{s',r}p(s',r|s,a)\Big[r+\gamma v_{\pi}(s')\Big] 
\end{align}
이것을 ***Bellman equation*** 이라고 한다. 이 Bellman equation 은 value function 의 정의로부터 유도가능한데 이 유도과정은 p.63에 나와있다. 식이 직관적으로 이해가능하여 굳이 여기에서 유도하진 않겠다. (심심할때 한번씩 풀어보면 좋을듯) 참고로 벨만방정식을 아날래틱하게 푸는것이 힘들어서 보통 알고리즘으로 풀어낸다. 알고리즘적으로 풀어내는 방법은 수렴할때까지 $k$를 증가시키면서 아래를 반복하는 것이다. 
\begin{align}
...
- 참고로 아래와 같은 Bellman equation 에서 
\begin{align}
v_{\pi}(s)=\sum_{a}\pi(a|s)\sum_{s',r}p(s',r|s,a)\Big[r+\gamma v_{\pi}(s')\Big] 
\end{align}
뒷부분 $\sum_{s',r}p(s',r|s,a)\Big[r+\gamma v_{\pi}(s')\Big]$ 을 ***$q$-function*** 이라고 한다. 즉 $q_{\pi}(s,a)$는 아래와 같이 정의할 수 있다. 
\begin{align}
q_{\pi}(s,a)=:\mathbb{E}_ {\pi}\Big[R_{t+1}+\gamma v_{\pi} (S_{t+1}) ~\|~ S_t=s, A_t=a \Big] =\sum_{s',r}p(s',r|s,a)\Big[r+\gamma v_{\pi}(s')\Big] 
\end{align}
이것은 상태 $s$ 에서만 행동 $a$를 **강제적으로** 취하고 나머지 상태에서는 정책 $\pi$를 계속 따를 때 얻어지는 보상의 기대값으로 해석할 수 있다. 

- **step 2.** 이제 $\pi$로 부터 더 나은 정책 $\pi'$을 찾는 과정을 논의하여 보자. 편의상 $\pi$, $\pi'$ 이 모두 deterministic policy 라고 하자. 따라서 given $s$에 대하여 항상 같은 action을 취하게 된다. 이 경우 그냥 $\pi(s)=a$ 와 같은 식으로 정의할 수 있다. Sutton의 교재 p.82에 보면 ***policy improvement theorem*** 이라는 것이 있는데 이것은 임의의 두 deterministic polish $\pi, \pi'$ 에 대하여 
\begin{align}
\forall s \in {\cal S}: q_{\pi}(s,a)=q_{\pi}(s,\pi'(s)) \geq v_{\pi}(s)
\end{align} 
이면 아래가 성립한다는 이론이다. 
\begin{align}
\forall s \in {\cal S}: v_{\pi'}(s) \geq v_{\pi}(s)
\end{align}
증명은 p.83 을 참고하자. 요 이론은 $\pi$를 $\pi'$로 업데이트 하기 위해서는, 즉 $\pi$ 보다 더 나은 $\pi'$를 찾기위해서는 <br/>
**(1)** given $\pi$ 일때 $v_{\pi}(s)$를 구하고 그것을 바탕으로 $q$-fucntion $q_{\pi}(s,a)$를 구한다. <br/>
**(2)** 모든 $s \in {\cal S}$에 대하여 $q$-function 을 maximize 하는 action 을 구하고 이를 새로운 정책에 반영한다. 즉 
\begin{align}
\pi'(s) \leftarrow \underset{a}{\operatorname{argmax}} q_{\pi}(s,a)
\end{align}
를 수행한다. 이러한 방식으로 업데이트 된 정책 $\pi'$를 greedy policy 라고 한다. 

- 이 과정을 거치면 $\pi$ 보다 더 나은 $\pi'$를 찾을 수 있음을 알겠다. 그래서 위의 과정에서 step 1,2 를 반복해 더 이상 개선될 수 없는 정책 $\pi'$를 찾았다고 하자. 즉 모든 $s \in {\cal S}$ 에 대하여 $v_{\pi}(s)=v_{\pi'}(s)$ 가 성립한다고 하자. 이 정책 $\pi'$ 가 optimal policy $\pi^* $ 라고 주장할 수 있을까? 분명 $\pi' $는 초기 정책에 비해서 업데이트와 업데이트를 거듭하여 구해진 **나름 최적의 값**이지만 이것이 과연 모든 $\pi$ 를 고려하여도 최적일 수 있을까? 결론은 yes 이다. 보충설명으로 Sutton p.83. 하단에 있는 내용을 풀어 쓰겠다. 우선 $v_{\pi'}(s)$ 는 아래와 같이 쓸 수 있다. 
\begin{align}
v_{\pi'}(s)=\sum_{a}\pi(a|s)\sum_{r,s'}p(s,r'|s,a)\Big[r+\gamma v_{\pi'}(s')\Big]
\end{align}


- polish iteration algorithm 보다 빠른것이 ***value iteration algorithm*** 이다. 이것은 **optimal policy $\pi^* $ 를 알고 있다면 그에 대응하는 $v_{\pi^* }(s)$를 유추할 수 있듯이 $v_{\pi^* }(s)$를 알면 그에 대응하는 optimal policy $\pi^* $ 를 쉽게 유추할 수 있다** 는 점을 이용해서 $v_{\pi^* }$를 먼저 찾은다음에  $\pi^* $ 를 찾는 전략이다. 이것은 value function 이 policy 를 결정하는데 필요한 모든 정보를 제공하기 때문에 가능한 것이다. 

- polish iteration algorithm 은 정책과 가치함수를 동시에 업데이트 하면서 $(\pi,v_{\pi}(s))$를 동시에 최적화 하지만 value iteration algorithm 은 $v_{\pi}(s)$ 를 먼저 최적화하고 그담에 $\pi$를 찾는 방법이다. 이것은 $v_{\pi}(s)$ 만 최적화 하면 되기 때문에 빠르다. $v_{\pi}(s)$를 최적화 하는 자세한 과정은 4장 다이나믹 프로그래밍에서 다룬다. 

- optimal policy 에 대응하는 value function 을 ***optimal value function*** 이라고 하고 엄밀하게는 아래와 같이 정의한다. 
\begin{align}
v_{\pi^* }(s)=v_{* }(s):=\max_{\pi} v_{\pi}(s), \quad \forall s \in {\cal S}
\end{align}

- 어차피 4장에서 다루겠지만 $v_{\pi^* }(s)$를 찾는 핵심아이디어는 아래와 같이 정의되는 ***$q$-function*** 이라는 것을 이용하는 것이다. 
\begin{align}
q_{\pi}(s,a):=\mathbb{E}_ {\pi}\bigg(\sum_{k=0}^{\infty}\gamma^k R_{t+k+1} \bigg\| S_t=s, A_t=a \bigg).
\end{align}
이것은 $S_t=s$ 에서만 행동 $a$를 **강제적으로** 취하고 그 뒤로는 정책 $\pi$를 계속 따를 때 얻어지는 보상의 기대값이다. 보면 알겠지만 $q$-function을 알면 value function 을 알 수 있으므로, $q$-function 은 value function 보다 좀 더 고급 정보에 해당한다고 이해할 수 있다. 아무튼 $q_{\pi}(s,a)$ 가 $\pi$ 에 대응하듯이 $\pi^* $ 에 대응하는 $q_{\pi^* }(s,a)$ 역시 아래와 같이 정의할 수 있다. 
\begin{align}
q_{\pi^* }(s,a)=q_{* }(s,a):= \max_{\pi}q_{\pi}(s,a), \quad \forall s \in {\cal S} \mbox{ and } a \in {\cal A}(s)
\end{align}
이것을 ***optimal $q$-function*** 이라고 정의한다. 말했듯이 $q$-function 은 value function 보다 고급정보이므로 결국 $q_{\pi^* }(s,a)$를 구하면 우리의 목적인 $\pi^* $를 구할 수 있다. 
...
- 다이나믹 프로그래밍을 배우기 앞서서 다양한 형태의 Bellman eqaution 들을 모아보았다. <br/><br/>
...
- **(policy improvement theorem)** $\pi$ 와 $\pi'$을 deterministic policy 라고 하자. 아래를 가정하자. 
\begin{align}
\forall s \in {\cal S}: q_{\pi}(s,a)=q_{\pi}(s,\pi'(s)) \geq v_{\pi}(s)
\end{align} 
그러면 아래가 성립한다. 
