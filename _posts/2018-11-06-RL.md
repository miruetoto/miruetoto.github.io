...
<figcaption> Sutton 교재 p.80에 있는 그림, terminal state로 가기전까지 계속 음의보상값 $-1$ 을 받는다. </figcaption>	
</center>	
</figure>
<br/>
...
\begin{align}
\\{\gamma v_{\pi}^{(1)}(s=1), \gamma v_{\pi}^{(1)}(s=5), \gamma v_{\pi}^{(1)}(s=0), \gamma v_{\pi}^{(1)}(s=2) \\} 
\end{align}
가 되고 따라서 
\begin{align}
\\{ -\gamma, -\gamma , 0, -\gamma \\} 
...
<figcaption> Sutton 교재 p.81에 있는 그림. </figcaption>	
</center>	
</figure>
<br/>

왼쪽 컬럼은 랜덤 팔리쉬에 대한 평가를 위하여 $v_{\pi}(s)$를 업데이트하며 추론한 것이다. 왼쪽 컬럼을 쭉 따라서 내려오는 과정이 step 1 (Policy Evaluation) 에 해당하는 것이다. 오른쪽은 그에 대응하는 그리디팔리쉬 이다. 왼쪽의 맨마지막 로우 ($k=\infty$) 에서 오른쪽을 구하는과정이 step 2 (Policy Improvement) 이다. 이 예제의 경우 한번의 Policy Evaluation 한번의 Policy Improvement 로 수렴하였다. 하지만 보통은 
