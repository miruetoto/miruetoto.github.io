...
- 설명의 편의를 위해서 아래와 같이 $4\times 4$개의 격자가 있는 세계에서 로봇이 움직이는 일반적인 예제를 특정하자. 
...
- **state**: $4\times 4$ 격자위에서 로봇이 움직이고 있으므로 로봇이 존재할 수 있는 all possible state 는 총 16개이다. 


예제의 경우는 음영처리된 부분에 도착하면 과업이 종료된다고 가정하였다. *참고로 이렇게 일정한 시간이 지나면 언제가 끝이나는 task 를 **에피소드-태스크** 라고 한다. 이 예제를 포함하여 바둑이나 장기와 같은것도 일정한 시간이 지나면 언젠가 끝나기 때문에 **에피소드-테스크** 의 한 예이다. 반대로 시간이 지나도 끝나지 않는 task 를 **컨티뉴잉-태스크** 라고 한다.* 아무튼 **에피소드-태스크** 에서는 위와 같이 음영처리된 상태각각 $0$과 $15$라고 한다면 
\begin{align}
부분이 최종상태라고 하여 16개의 상태를 각각 $s_1,\dots,s_{16}$ 이라고 정의하고 이들의 집합을 ${\cal S}$라고 하자. 즉 ${\cal S}:=\\{s_1,\dots,s_{16}\\}$ 이다. 

- **action** 로봇이 취할 수 있는 액션을 정의하자. 본디 로봇은 동서남북으로 움직일수 있으므로 로봇이 취할 수 있는 all possible actions은 4가지 행동이다. 다만 경우에 따라서 특정상태에서 취할수 있는 행동에 제약이 있을 수 있다. 예를들어서 

편의상 이런 action들을 $a_1,\dots,a_4$라고 하자. 그리고 이들의 집합을 ${\cal A}(S_t)$라고 하자. 즉 ${\cal A}:=\\{a_1,\dots,a_4\\}$ 이다. 


-라고 정의하자. 특정 상태 $s\in {\cal S}$에서 다른 특정 상태 $s'\in {\cal S}$로 이동할 확률을 모두 정의한 매트릭스 ${\bf P}$를 만들고 transition matrix라고 하자. 위의 예제에서는 $|{\cal S}|=16$이다. 따라서 ${\bf P}$의 차원은 $16 \times 16$이다. 위의 예제에서 초기상태 $s$에서 다음상태 $s'$으로 이동할 확률이 모두 $\frac{1}{16}$로 동일하다면 ${\bf P}=\frac{1}{16}{\bf J}$와 같이 정의할 수 있다. 

- **state** at $t$
\begin{align}
S_t \in {\cal S}
\end{align}
where ${\cal S}$ is set of all possible states in environment.  

- **action** at $t$
\begin{align}
A_t \in {\cal A}(S_t)
\end{align}
where ${\cal A}(S_t)$ is set of all possible actions in $S_t$. 

- **reward** from $A_t$
\begin{align}
R_{t+1} \in {\cal R} \subset \mathbb{R}
\end{align}
where ${\cal R}$ is set of all possible reward from environment. 

- **probability of $(s',r)$ given $(s,a)$**
\begin{align}
...
\begin{align}
r(s,a):= \mathbb{E} \Big( R_{t+1} \Big\| S_t=s, A_t=a \Big) = \sum_{r \in {\cal R}} r \sum_{s' \in {\cal S}} p(s',r\|s,a).
\end{align}
