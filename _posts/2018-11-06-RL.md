...
\end{align}
와 같다고 **어떠한 확률관련된 함수**이다. 이게 노테이션이 너무 확률변수 $(S_t,A_t,R_{t+1},S_{t+1})$에 대응하는 ***probability measure***처럼 생겼는데 사실은 $\mu:\Big( \Big) \times \Big(\mathbb{R}^2,{\cal R}^2 \Big) \to [0,1]$ 에 대응하는 

아무튼 $\mathbb{P}$를 정의하기 위해서는 크기가 
\begin{align}
\Big(\|{\cal S}\|,\|{\cal A}\|,\|{\cal R}\|,\|{\cal S}^+\|\Big)
\end{align}
인 4차원 array 혹은 tensor 에 각각 $[0,1]$ 사이의 값을 코딩해야 한다. 편의상 이러한 array 를 ${\bf P}[s,a,r,s']$이라고 생각하자. 확률에 $0$값을 줄 수 있다는 것을 이용하면 
\begin{align}
\mathbb{P}: {\cal S}^+ \times {\cal A} \times {\cal R} \times {\cal S}^+ \to [0,1]
\end{align}
