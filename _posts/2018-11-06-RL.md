...
- At each time step $t$, the agent receives some representation of the environment's state, $S_t \in {\cal S}$, where ${\cal S}$ is the set of possible states, and on that basis selects(choose, determinded) an action, $A_t \in {\cal A}(S_t)$ where ${\cal A}(S_t)$ is the set of actions available in state $S_t$. 

- One time step later, the agent receives a numerical reward, $R_{t+1} \in {\cal R} \subset \mathbb{R}$ and finds itself in a new state, $S_{t+1}$. 이때 $R_{t+1}$은 액션 $A_t$에 대한 결과라고 볼 수 있으므로 $R_t$라고 쓰는 문헌들도 있다. 

- **policy**
\begin{align}
\pi_t:=\pi_t(a|s):=Pr\Big(A_t=a \Big\| S_t=s\Big).
\end{align}
...
- **expected rewards for state-action-next_state triples** 
\begin{align}
...
- **(Bellman equation)** 초보적인 계산(마코프성질 한번 사용함)으로 아래의 식이 성립함을 관찰할 수 있다. 
\begin{align}
...
- **optimal polish** 
\begin{align}
\pi^*:= \underset{\pi}{\operatorname{argmax}} v_{\pi}(s), \quad \forall s \in {\cal S}
\end{align}
...
- **(Bellman optimality equation)** 역시 초보적인 계산으로 아래가 성립함을 관찰할 수 있다. 
\begin{align}
...
- **(policy evaluation)** 이제부터 무엇인가 논의하기 시작한다. 이전 챕터에서는 그냥 정의만 나열하고 말았었다. **벨만류**(앞에서 언급한 Bellman 방정식들은 종류가 너무 많고 그게 그거 같아서 그냥 앞으로 뭉뚱그려서 벨만류라고 부르겠다)의 방정식을 몇개 탐구하긴 하였으나 당연한 소리를 써놓고 있어서 머 어쩌란 것인지 몰랐다. 
