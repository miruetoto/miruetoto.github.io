...
\begin{align}
v_{\pi}^{(k+1)}(s)=\sum_{a}\pi(a|s)\sum_{s',r}p(s',r|s,a)\Big[r+\gamma v_{\pi}^{(k)}(s')\Big] 
\end{align}
즉 $v_{\pi}^{(k)}(s) \to v_{\pi}(s)$ as $k \to \infty$ 가 성립한다. 요 내용은 p.78 에 있다. 
...
- **step 2.** 이제 $\pi$로 부터 더 나은 정책 $\pi'$을 찾는 과정을 논의하여 보자. 편의상 $\pi$, $\pi'$ 이 모두 deterministic policy 라고 하자. 따라서 given $s$에 대하여 항상 같은 action을 취하게 된다. 예를 들어서 아래의 그림 
<center><img src="https://github.com/miruetoto/miruetoto.github.io/blob/master/img/RL/RL_fig1.png?raw=true" height="60%" width="60%"></center>
에서 상태 $s=15$ 에서 
편의상 $\pi(a|s)=a아래를 가정하자. 
\begin{align}
