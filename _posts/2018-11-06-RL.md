...
(2) (인간,환경) **Policy Evaluation** $V_{\pi^{(0)} }(s)$를 구한다. 구하는 방법은 각 상태를 fix하고 각각 벨만이퀘이션을 수렴시킨다. 즉 
\begin{align}
v(s) \leftarrow \sum_{a}\pi(a|s)\sum_{s',r}p(s',r|s,a)\Big[r+\gamma v_{\pi}(s')\Big] 
\end{align}

(3) (인간) $V_{\pi^{(0)} }(s)$ 로부터 $q_{\pi^{(0)} }(s,a)$ 를 구한다. 이때 인간은 MDP 정보를 참조한다. 
(4) (인간) 더 나은 정책 $\pi^{(1)}$을 찾는다. 이때 인간은 $q^{(0)}(s_1,a)$ 를 참조한다. 
(5) (1)-(4) 를 만복한다. 

- 밸류이터레이션 알고리즘
(1) (인간) $\pi^{(0)}(s)$ 를 초기화 한다. <br/>
(2) (인간,환경) **Policy Evaluation** 벨만-이퀘이션을 이용하여 $V_{\pi^{(0)} }(s)$를 구한다. 
(3) (인간) $V_{\pi^{(0)} }(s)$ 로부터 $q_{\pi^{(0)} }(s,a)$ 를 구한다. 이때 인간은 MDP 정보를 참조한다. 
(4) (인간) 더 나은 정책 $\pi^{(1)}$을 찾는다. 이때 인간은 $q^{(0)}(s_1,a)$ 를 참조한다. 
(5) (1)-(4) 를 만복한다. 
