...
\end{align}
가 된다. 여기에서 state $s=0$ 과 $s=15$ 가 의미하는 곳이 각각 어디인지는 아마 말하지 않아도 알 것이라 생각한다. 

- **action** 로봇이 취할 수 있는 액션을 정의하자. 본디 로봇은 동서남북으로 움직일수 있으므로 로봇이 취할 수 있는 all possible actions은 4가지 행동이다. 다만 경우에 따라서 특정상태에서 취할수 있는 행동에 제약이 있을 수 있다. 예를들어서 편의상 이런 action들을 $a_1,\dots,a_4$라고 하자. 그리고 이들의 집합을 ${\cal A}(S_t)$라고 하자. 즉 ${\cal A}:=\\{a_1,\dots,a_4\\}$ 이다. 
