...
<figcaption> Sutton 교재 그림 4.3. </figcaption>	
</center>	
</figure>
...
- 이 그림에서 폴리쉬-이터레이션은 왼쪽위부터 아래로 쭉 계산하고 (step 1, 폴리쉬-이벨류에이션) 그리고 수렴한뒤에 $v_{\pi}^{(\infty)}$ 에 대한 그리피팔리쉬를 구했다. 즉 위의 그림에서 L형태로 계산이 진행된다. 내가 헷갈렸던 부분은 L형태로 계산을 하지 않고 **한번만** 폴리쉬-이벨류에이션을 하고 그것으로 부터 그리디팔리쉬를 구하고 다시 그 그리디팔리쉬에 대한 폴리쉬-이벨류에이션을 하고 요걸 반복하는 방식으로 (L자가 아닌 지그재그형태로) 하면 더 효율적이지 않을까? 라고 생각했기 때문이다. 그리고 이렇게 하는 방식이 바로 벨류-이터레이션-알고리즘이다. 

- 다음은 벨류-이터레이션 방식과 폴리쉬-이터레이션 방식의 알고리즘이다. 비교해 보면 좋을것 같다. 
<br/>
<figure>	
<center>	
<img src="https://github.com/miruetoto/miruetoto.github.io/blob/master/img/RL/RL4345.png?raw=true" height="100%" width="100%">
<figcaption> Sutton 교재 그림 4.3 과 4.5. 왼쪽이 폴리쉬-이터레이션이고 오른쪽이 벨류-이터레이션임.  </figcaption>	
</center>	
</figure>
