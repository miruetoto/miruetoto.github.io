...
\end{cases}
\end{align}
지금까지의 논리를 종합하면 **step 1.** 특정 정책 $\pi$ 에 대한 value function $v_{\pi}(s)$ 를 계산하고 **step 2.** 정책 $\pi$를 $\pi'$ 로 업데이트 하는것을 반복하면 최적의 정책 $\pi^* $ 를 찾을 수 있을 것 같다. 이런식으로 $\pi^* $ 를 찾아내는 과정을 ***polish iteration algorithm*** 이라고 한다. 
