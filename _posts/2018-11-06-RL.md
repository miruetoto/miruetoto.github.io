...
환경은 $(s,a)$가 정해지면 $(s',r)$를 던져준다. 이 확률을 $p(s',r ~ \| ~ s,a)$ 라고 한다. 즉 
\begin{align}
...
\end{align}
인 4차원 array 에 각각 $[0,1]$ 사이의 값을 코딩해야 한다. 편의상 이러한 array 를 ${\bf P}[s,a,s',r]$이라고 생각하자. 그런데 확률에 $0$값을 줄 수 있다는 것을 이용하면 
\begin{align}
p: {\cal S}^+ \times {\cal A} \times {\cal R} \times {\cal S}^+ \to [0,1]
\end{align}
