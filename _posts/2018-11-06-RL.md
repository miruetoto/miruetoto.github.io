...
- 폴리시이터레이션 알고리즘 
(1) (인간) $\pi^{(0)}(a\|s)$ 를 정한다. <br/>
(2) (인간,환경) $v_{\pi^{(0)} }(s)$를 구한다. 구하는 방법은 각 상태를 순서대로 방문 하면서 아래의 벨만이퀘이션을 한번씩 업데이트 한다. 
\begin{align}
v(s) \leftarrow \sum_{a}\pi(a|s)\sum_{s',r}p(s',r|s,a)\Big[r+\gamma v(s')\Big] 
\end{align}
(3) 수렴할때까지 (2)를 반복한다. 수렴값은 $v_{\pi^{(0)} }(s)$ 이다. 
(4) (인간) $V_{\pi^{(0)} }(s)$ 로부터 $q_{\pi^{(0)} }(s,a)$ 를 구한다. 이때 인간은 MDP 정보를 참조한다. <br/>
(5) (인간) 더 나은 정책 $\pi^{(1)}$을 찾는다. 이때 인간은 $q^{(0)}(s_1,a)$ 를 참조한다. (구체적으로는 그리디방법을 씀) <br/>
(6) (1)-(5) 를 반복한다. 수렴된 정책이 최적의 정책 $\pi^* $ 이다. 

- 밸류이터레이션 알고리즘
(1) (인간,환경) $v(s)$ 를 초기화한다. 그리고 각 상태를 순서대로 방문 하면서 아래의 벨만이퀘이션을 한번씩 업데이트 한다. 
\begin{align}
v(s) \leftarrow \max_{a} \sum_{s',r}p(s',r|s,a)\Big[r+\gamma v(s')\Big] 
\end{align}
(2) 수렴할때까지 (2)를 반복한다. 수렴값은 $v_{\pi^* }(s)$ 이다. 
(3) (인간) $V_{\pi^* }(s)$ 로부터 $q_{\pi^* }(s,a)$ 를 구한다. 이때 인간은 MDP 정보를 참조한다. <br/>
(4) (인간) 최적의 정책 $\pi^* $을 찾는다. 이때 인간은 $q_{\pi^* }(s,a)$ 를 참조한다. <br/>
