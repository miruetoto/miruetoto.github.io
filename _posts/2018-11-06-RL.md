...
- **reward**: 행동 $A_t$로 부터 얻어지는 보상을 $R_{t+1}$라고 정의한다. 이 책에서 $t$시점의 행동에 대한 보상은 $t+1$ 에 주어진다고 가정하므로 $R_t$가 아니라 $R_{t+1}$ 로 정의하였다. 받을 수 있는 모든 보상의 집합을 ${\cal R}$이라고 정의하자. 예를 들어서 그리드 밖으로 나가면 $-1$점씩, 그리고 terminal-state에 도달하면 +100점씩, 그외에는 무조건 0점씩 준다고 하면 ${\cal R}=\Big\\{-1,0,100\Big\\}$ 이다. 그런데 같은 상태에서 같은 행동을 취해도 다른 보상을 줄 수 있다. (될놈될.. 잘 보면 $R_t$가 랜덤변수임..) 따라서 아래와 같이 상태 $S_t=s$ 에서 행동 $A_t=a$를 취했을 때 얻는 보상 $R_{t+1}$의 평균과 같은 개념을 생각해 볼 수 있다. 
\begin{align}
...
이다. 보는것 처럼 $p$는 
\begin{align}
p: {\cal S} \times {\cal A} \times {\cal S}^+ \times {\cal R} \to [0,1]
\end{align}
인 함수이다. 따라서 $p$를 정의하기 위해서는 크기가 
\begin{align}
\Big(\|{\cal S}\|,\|{\cal A}\|,\|{\cal S}^+\|,\|{\cal R}\|\Big)
\end{align}
인 4차원 array 에 각각 $[0,1]$ 사이의 값을 코딩해야 한다. 편의상 이러한 array 를 ${\bf P}[s,a,s',r]$이라고 생각하자. 그런데 확률에 $0$값을 줄 수 있다는 것을 이용하면 
\begin{align}
p: {\cal S}^+ \times {\cal A} \times {\cal S}^+ \times {\cal R} \to [0,1]
\end{align}
이라고 정의해도 괜찮다. 따라서 일반성을 잃지않고 
\begin{align}
p: {\cal S} \times {\cal A} \times {\cal S} \times {\cal R} \to [0,1]
\end{align}
로 정의할 수 있고 ${\bf P}[s,a,s',r]$ 의 차원을 $\Big(\|{\cal S}\|,\|{\cal A}\|,\|{\cal S}\|,\|{\cal R}\|\Big)$ 로 생각해도 무방하다. 여기에서 ${\bf P}[s,a,s',r]$ 은 **환경(environment)** 가 가지고 있는 궁극의 테이블 (혹은 비밀노트?) 라고 보면 된다. 환경이 에이전트에게 주는 모든 종류의 피드백은 ${\bf P}[s,a,s',r]$ 에 근거한다. 
