...
\end{align}
인 함수이다. 따라서 $p$를 정의하기 위해서는 크기가 
\begin{align}
\Big(\|{\cal S}\|,\|{\cal A}\|,\|{\cal R}\|,\|{\cal S}^+\|\Big)
\end{align}
인 4차원 array 에 각각 $[0,1]$ 사이의 값을 코딩해야 한다. 편의상 이러한 array 를 ${\bf P}[s,a,r,s']$이라고 생각하자. 그런데 확률에 $0$값을 줄 수 있다는 것을 이용하면 
\begin{align}
p: {\cal S}^+ \times {\cal A} \times {\cal R} \times {\cal S}^+ \to [0,1]
\end{align}
...
\end{align}
위의 식은 그냥 array ${\bf P}$에서 $r$차원을 marginally out 한 것이다. 즉 아래와 같이 코딩하여 구할수 있다.
\begin{align}
p(s'|s,a):=P_{ss'}^{a}:=\mbox{sum}\Big({\bf P}[s,a,:,s']\Big)
\end{align}
또한 ${\bf P}[s,a,r,s']$ 로 부터 **expected rewards for (state,action,next-state) triples** 을 아래와 같이 구할 수 있다. 
\begin{align}
r(s,a,s'):=\mathbb{E}\Big( R_{t+1} ~~ \Big\|  ~~ S_t=s, A_t=a, S_{t+1}=s' \Big)=\frac{\sum_{r \in {\cal R}}rp(s',r|s,a)}{p(s'|s,a)}:=R_{ss'}^{a}.
\end{align}
이것 역시 아래와 같이 코딩하여 구할 수 있다. 
\begin{align}
r(s,a,s'):=R_{ss'}^{a}:=\frac{\mbox{sum}_ r \Big({\cal R}[r] {\bf P}[s,a,r,s']\Big)}{\mbox{sum}\Big({\bf P}[s,a,:,s']\Big)}
\end{align}


- **policy**: 
\begin{align}
\pi_t:=\pi_t(a|s):=Pr\Big(A_t=a ~~ \Big\| ~~ S_t=s\Big) \in \Pi.
\end{align}
where $\Pi$ is set of all possible policy. 
