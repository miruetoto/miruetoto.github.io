...
p: {\cal S} \times {\cal A} \times {\cal R} \times {\cal S}  \to [0,1]
\end{align}
로 정의할 수 있고 ${\bf P}[s,a,r,s']$ 의 차원을 $\Big(\|{\cal S}\|,\|{\cal A}\|,\|{\cal R}\|,\|{\cal S}\|\Big)$ 로 생각해도 무방하다. 여기에서 ${\bf P}[s,a,r,s']$ 은 **환경(environment)** 가 가지고 있는 궁극의 테이블 (혹은 비밀노트?) 라고 보면 된다. 환경이 에이전트에게 주는 모든 종류의 피드백은 ${\bf P}[s,a,r,s']$ 에 근거한다. 그리고 당연하겠지만 given $(s,a)$ 일때 환경이 줄 수 있는 모든 경우의 수 $(r,s')$ 에 대한 확률의 총합은 1이므로 아래가 성립한다. 
\begin{align}
\forall (s,a) \in {\cal S} \times {\cal A}: ~ \sum_{r \in {\cal R}} \sum_{s \in {\cal S}} {\bf P}[s,a,r,s'] = 1 
\end{align}
또한 ${\bf P}[s,a,r,s']$ 로 부터 아래와 같이 **state-transition probabilites** 를 구할 수 있다. 
\begin{align}
p(s'|s,a):= \mathbb{P}\Big(S_{t+1}=s'~  \Big\| ~ S_t=s, A_t=a \Big)= \sum_{r \in {\cal R}}p(s',r|s,a):=P_{ss'}^{a}.
\end{align}
위의 식은 그냥 array ${\bf P}$에서 $r$을 marginally out 한 것이다. 즉 간단히 $\sum_{r \in {\cal R}}\Big({\bf P}[s,a,r,s']$이다. 
