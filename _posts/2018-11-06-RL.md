...
- **reward**: 행동 $A_t$로 부터 얻어지는 보상을 $R_{t+1}$라고 정의한다. 이 책에서 $t$시점의 행동에 대한 보상은 $t+1$ 에 주어진다고 가정하므로 $R_t$가 아니라 $R_{t+1}$ 로 정의하였다. 받을 수 있는 모든 보상의 집합을 ${\cal R}$이라고 정의하자. 예를 들어서 그리드 밖으로 나가면 $-1$점씩, 그리고 terminal-state에 도달하면 +100점씩, 그외에는 무조건 0점씩 준다고 하면 ${\cal R}=\Big\\{-1,0,100\Big\\}$ 이다. 

- 그런데 같은 상태에서 같은 행동을 취해도 다른 보상을 줄 수 있다. (될놈될.. 잘 보면 $R_t$가 랜덤변수임..) 따라서 아래와 같이 상태 $S_t=s$ 에서 행동 $A_t=a$를 취했을 때 얻는 보상 $R_{t+1}$의 평균과 같은 개념을 생각해 볼 수 있다. 
\begin{align}
r(s,a):= \mathbb{E} \Big( R_{t+1} ~ \Big\|~ S_t=s, A_t=a \Big)
\end{align}
이것을 **expected rewards for (state,action) pairs** 라고 한다. 주의할것이 $r$은 $t+1$시점에서의 보상 $R_{t+1}$의 실현값이고 $r(s,a)$는 given $(s,a)$에서 $R_{t+1}$의 평균값이라는 것이다. 

- **probability of $(s',r)$ given $(s,a)$**<br/><br/>
환경은 $(s,a)$가 정해지면 $(s',r)$을 던져준다. 이 확률을 $p(s',r ~ \| ~ s,a)$ 라고 한다. 즉 
\begin{align}
p(s',r ~ \| ~ s,a) := \mathbb{P}\Big( S_{t+1}=s', R_{t+1}=r ~ \Big\| ~ S_t=s, A_t=a \Big).
\end{align}
이다. 보는것 처럼 $\mathbb{P}$는 
\begin{align}
\mathbb{P}: {\cal S} \times {\cal A} \times {\cal R} \times {\cal S}^+  \to [0,1]
\end{align}
인 probability measure이다. 따라서 $\mathbb{P}$를 정의하기 위해서는 크기가 
\begin{align}
\Big(\|{\cal S}\|,\|{\cal A}\|,\|{\cal R}\|,\|{\cal S}^+\|\Big)
\end{align}
인 4차원 array 에 각각 $[0,1]$ 사이의 값을 코딩해야 한다. 편의상 이러한 array 를 ${\bf P}[s,a,r,s']$이라고 생각하자. (사실 엄청 엄밀하게 말하면 이것은 probability measure $\mathbb{P}$를 코딩한 것이 아니라 $\mathbb{P}$에 대응하는 ~~distribution fuction, 그리고 그에 대응하는 cumulative function, 그리고 그것의 라돈니코딤-derivation인~~ probability density function 즉 pdf를 코딩한것이다.) 확률에 $0$값을 줄 수 있다는 것을 이용하면 
\begin{align}
p: {\cal S}^+ \times {\cal A} \times {\cal R} \times {\cal S}^+ \to [0,1]
\end{align}
이라고 정의해도 괜찮다. 따라서 일반성을 잃지않고 ${\cal S}^+ = {\cal S}$ 라고 놓아도 무방하다. 
\begin{align}
p: {\cal S} \times {\cal A} \times {\cal R} \times {\cal S}  \to [0,1]
\end{align}
로 정의할 수 있고 ${\bf P}[s,a,r,s']$ 의 차원을 $\Big(\|{\cal S}\|,\|{\cal A}\|,\|{\cal R}\|,\|{\cal S}\|\Big)$ 로 생각해도 무방하다. 그리고 당연하겠지만 given $(s,a)$ 일때 환경이 줄 수 있는 모든 경우의 수 $(r,s')$ 에 대한 확률의 총합은 1이므로 아래가 성립한다. 
\begin{align}
\forall (s,a) \in {\cal S} \times {\cal A}: ~ \sum_{r \in {\cal R}} \sum_{s \in {\cal S}} {\bf P}[s,a,r,s'] = 1 
\end{align}

- 언급하였듯이 ${\bf P}$는 그러니까 집합 ${\cal S}, {\cal A}, {\cal R}, {\cal S}^+$에 대응하는 **시그마필드**가 각각 $\sigma(S_t),\sigma(A_t),\sigma(R_{t+1}),\sigma(S_{t+1})$ 으로 있는 것이다. 편의상 이러한 시그마 필드를 

- 여기에서 ${\bf P}[s,a,r,s']$ 은 **환경(environment)** 가 가지고 있는 궁극의 테이블 (혹은 비밀노트?) 라고 보면 된다. 환경이 에이전트에게 주는 모든 종류의 피드백은 ${\bf P}[s,a,r,s']$ 에 근거한다. 예를들어 ${\bf P}[s,a,r,s']$ 를 이용하면 아래와 같이 **state-transition probabilites** 를 구할 수 있다. 
\begin{align}
p(s'|s,a):= \mathbb{P}\Big(S_{t+1}=s'~  \Big\| ~ S_t=s, A_t=a \Big)= \sum_{r \in {\cal R}}p(s',r~|~s,a):=P_{ss'}^{a}.
\end{align}
...
- **value function** : 환경이 가지고 있는 궁극의 테이블이 ${\bf P}[s,a,r,s']$ 이라고 언급하였다. 에이전트가 가지는 궁극의 테이블은 무엇인가? 그것은 바로 아래와 같이 정의되는 **policy** 이다. 
- **policy**: 
\begin{align}
\pi_t:=\pi_t(a|s):=\mathbb{P}\Big(A_t=a ~ \Big\| ~ S_t=s\Big) \in \Pi.
\end{align}
이건 간단하게 말해서 에이전트가 상태 $S_t=s$ 에서 행동 $A_t=a$ 을 할 확률을 의미한다. 따라서 polish는 차원이 ${\cal S} \times {\cal A}$ 인${\boldsymbol \Pi}[s,a]$ 와 같은 매트릭스에 $[0,1]$ 사이의 값들을 코딩한 것으로 생각 할 수 있다. 그리고 조건부확률의 정의에 따라서 당연히 아래가 성립한다. 
\begin{align}
\sum_{s \in {\cal S}} {\boldsymbol \Pi}[s,a]
\end{align}
