...
- 특정 상태 $s \in {\cal S}$에 대하여 정책 $\pi_t^{(1)}(a|s)$가 좋은 정책인지 나쁜 정책인지 평가할 수 있다. 여기에서 숫자 1은 첫번째 정책이라는 의미이다. (환경이 가진 테이블 $p(s',r|s,a)$과는 다르게 에이전트가 가진 테이블 $\pi_t(a|s)$는 업데이트가 된다. 즉 $\pi_t^{(1)}(s,a) \to \pi_t^{(2)}(s,a) \to .. $이런식으로 업데이트 하면서 더 좋은 테이블로 점점 수정해 나간다.) 첫번째 정책 $\pi_t^{(1)}(s,a)$가 좋은 정책인지 나쁜정책인지 평가하는 방법은 아래를 계산하는 것이다. 
\begin{align}
v_{\pi_t^{(1)}}(s):=\mathbb{E}_ {\pi_t^{(1)}} \Big(G_t ~~ \Big\| ~~ S_t=s\Big) = \mathbb{E}_ {\pi_t^{(1)}} \bigg( \sum_{k=0}^{\infty}\gamma^k R_{t+k+1} \bigg\| S_t=s\bigg).
\end{align}
요걸 ***value function*** 이라고 한다. 이때 $G_t$를 **expected return** 이라고 하고 
$
G_t=R_{t+1}+\gamma R_{t+2}+\gamma^2 R_{t+3}+\dots = \sum_{k=0}^{\infty}\gamma^kR_{t+k+1}
$
와 같이 정의한다. 따라서 $v_{\pi_t^{(1)}}(s)$는 정책 $\pi_t^{(1)}(a|s)$를 반복하여 썼을때 특정 상태 $s \in {\cal S}$에서 평균적으로 받게 되는 보상의 기대값이라고 볼 수 있다. $v(\pi_1,s)$를 계산하는 방법은 아래의 순환식을 푸는 것이다. 
\begin{align}
v_{\pi_1}(s)=\sum_{a}\pi_1(a|s)\sum_{s',r}p(s',r|s,a)\Big[r+\gamma v_{\pi_1}(s')\Big] 
\end{align}
이것을 벨만방정식이라고 한다. 벨만방정식은 
$v_{\pi_1}(s):=\mathbb{E}_ {\pi_1} \Big(G_t ~~ \Big\| ~~ S_t=s\Big)$ 로 부터 유도가능한데 이 유도과정은 p.63에 나와있다. 식이 직관적으로 이해가능하여 굳이 여기에서 유도하진 않겠다. (심심할때 한번씩 풀어보면 좋을듯) 참고로 벨만방정식을 알고리즘적으로 풀어내는 쉬운방법은 수렴할때까지 $k$를 증가시키면서 아래를 반복하는 것이다. 
\begin{align}
v_{k+1}(s)=\sum_{a}\pi_1(a|s)\sum_{s',r}p(s',r|s,a)\Big[r+\gamma v_{k}(s')\Big] 
\end{align}




이런식으로 모든 상태 $s \in {\cal S}$에 대하여 정책 $\pi_1(a|s)$를 썼을때 보상의 기대값을 계산할 수 있는데 이를 정책 $\pi_1(a|s)$대한 가치함수라고 하고 기호로는 $v_{\pi_1}:{\cal S}\rightarrow \mathbb{R}$로 쓴다. 
\begin{align}
