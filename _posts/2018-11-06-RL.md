...
\begin{align}
\tilde\mathbb{P}: {\cal S} \times {\cal A} \times {\cal R} \times {\cal S}^+  \to [0,1]
\end{align}
이다. **틸드**를 붙인 이유는 확률벡터 $(S_t,A_t,R_{t+1},S_{t+1})$에 대응하는 probability measure $\mathbb{P}$와 구분하고 **conditionality** 를 강조하기 위함이다. 아무튼 $\tilde\mathbb{P}$를 정의하기 위해서는 크기가 
\begin{align}
