...
- HST를 매트릭스를 이용하여 정의하여 보자. 신호 $f(V)$를 길이가 $n$인 벡터라고 하고 매트릭스 ${\bf A}^{(\ell)}$ 을 $n\times n$ 매트릭스라고 하자. 눈이 내린횟수가 $\ell-1$이라고 할때 노드 $v_j$ 에서 노드 $v_i$로 눈이 이동할 수 있으면 ${\bf A}^{(\ell)}$의 $(i,j)$의 원소가 1이고 그렇지 않으면 0 이라고 하자. 즉 ${\bf A}^{(\ell)}$ 는 아래와 같이 쓸 수 있다. 
\begin{align}
{\bf A}^{(\ell)}_ {ij}=\begin{cases} 1 & j \in {\cal N}(i,\ell-1)  \\\\ \\0 & j \notin {\cal N}(i,\ell-1)\end{cases} 
\end{align}
여기에서 ${\bf A}^{(\ell)}$ 가 ***symm-matrix*** 가 아님을 유의하자. 
\begin{align}
