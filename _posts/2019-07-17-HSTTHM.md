...
- 앞으로 편의상 아래와 같이 정의하자. 
\begin{align}
{\boldsymbol X}=\begin{pmatrix} X_1 \\\\ \\ X_2 \\\\ \dots \\\\ X_n \end{pmatrix}=\begin{pmatrix}
=\begin{pmatrix} f(v_1) \\\\ \\ f(v_2) \\\\ \dots \\\\ f(v_n) \end{pmatrix}
\end{align} 
...
\begin{align}
T(\tilde F_; E)=\sum_{(i,j) \in E} \\| \boldsymbol{\dot h}(v_i)-\boldsymbol{\dot h}(v_j) \\|_ 2^2
\end{align}
라고 하자. 
