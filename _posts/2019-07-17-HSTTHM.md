...
\end{align}
여기에서 $\mu_i=g((\mu_1,\dots,\mu_{i-1},\mu_{i+1},\dots,\mu_n)',({\boldsymbol h}_ 1,\dots,{\boldsymbol h}_ {i-1},{\boldsymbol h}_ {i+1},\dots,{\boldsymbol h}_ n)' )$ 를 만족하는 함수 $g$ 가 있다고 하자. 편의상 아래와 같이 가정하자. 
\begin{align}
\begin{cases}
{\boldsymbol \mu}_ {(-i)}:&=(\mu_1,\dots,\mu_{i-1},\mu_{i+1},\dots,\mu_n)'  \\\\ \\ 
{\boldsymbol \mu}_ {(-i)}:&=({\boldsymbol h}_ 1,\dots,{\boldsymbol h}_ {i-1},{\boldsymbol h}_ {i+1},\dots,{\boldsymbol h}_ n)' \\\\ \\ 
\end{cases}
\end{align}
