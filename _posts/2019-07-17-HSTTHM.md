...
\end{align}
라고 하자. 그러면 각 $v_i$에 대응하는 확률벡터 $\boldsymbol{\dot h}(v_i)$ 역시 서로 독립이고 같은 분포를 가진다. 즉  
\begin{align}
\boldsymbol{\dot h}(v_1),\dots \boldsymbol{\dot h}(v_n) ~ \sim ~ i.i.d. ~ F\big(\boldsymbol{\dot h}(v);\eta\big)
\end{align}
이다. 이때 $F\big(f(v);\theta}\big):\mathbb{R} \to \mathbb{R}$ 이고 $F\big(\boldsymbol{\dot h}(v);\eta\big): \mathbb{R}^{\tau} \to \mathbb{R}$ 임을 유의하라. 

- $T$를 분포 $F_{\boldsymbol{\eta}}$에서 $\mathbb{R}$ 로 가는 ***functional*** 이라고 하자. 여기에서
\begin{align}
T(F_{\boldsymbol{\eta}} ; E)=\sum_{(i,j) \in E}  \mathbb{E} \\| \boldsymbol{\dot h}(v_i)-\boldsymbol{\dot h}(v_j) \\|_ 2^2
\end{align}
라고 하자. 
