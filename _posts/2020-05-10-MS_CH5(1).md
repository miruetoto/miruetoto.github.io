...
가 된다. 따라서 
\begin{align}
e^{\log mgf_{\sqrt{n}(\bar{X}-\mu)/\sigma }(t)} \approx e^{t^2/2}
\end{align}
물결은 $t/\sqrt{n}$이 $0$으로 갈때 성립하고 두번째 물결은 $t^2/2n$이 $0$으로 갈때 성립하므로 $n\to\infty$일때 두 물결이 모두 등호가 된다. (두번째 물결이 첫번째 물결보다 빠르게 등호로 된다.)
