...
- ***(Definition)*** The array ${\bf X}$ is called a locally stationary process if for each rescaled time point $u\in[0,1]$, there exists a strictly stationary process $\\{X_t(u):t\in\mathbb{Z}\\}$ with the property that 