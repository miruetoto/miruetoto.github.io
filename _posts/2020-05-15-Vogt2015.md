...
\begin{align}
\\|{\boldsymbol X}_ {t,T}-X_t(u)\\| \leq \bigg(\left\|\frac{t}{T}-u \right\|+\frac{1}{T} \bigg)U_{t,T}(u), \quad a.s. 
\end{align}
Here, $\\|\cdot\\|$ denotes a norm on $\mathbb{R}^d$, and $\\{U_{t,T}:t=1,2,\dots,T\\}_ {T=1}^{\infty}$ is an array of positive random variable whose $\rho$-th moment is uniformly bounded for some $\rho>0$; that is, 
