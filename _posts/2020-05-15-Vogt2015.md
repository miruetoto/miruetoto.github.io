...
- Author mentioned that the process $\\{ {\boldsymbol X}_ {t,T}\\}$ can be

- Donote ${\boldsymbol \lambda}_ {t,T}$ some ***time-varying feature*** of the locally stationary process $\\{X_ {t,T}\\}$ such as $\mathbb{E} [{\boldsymbol X}_ {t,T}]$ and $V (X_ {t,T})$. 

 - Generally speaking, ${\boldsymbol \lambda}_ {t,T}$ can be anything if it satisfying the following: 
\begin{align}
\labda_{t,T}= \mahtbb{E}\big( f(X_ {t,T}) \big)
\end{align}
(2) 
