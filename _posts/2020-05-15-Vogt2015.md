...
\end{cases}
\end{align}
where $\eta_t \sim U(0,1)$. Suppose that we choose $u=50/100=0.5$. Denote $U_{t,T}=200$. Then clearly 
\begin{align}
|X_ {t,T}- 0| \overset{a.s.}{\leq} \bigg( \left|\frac{t}{100}-0.5 \right|+\frac{1}{100}\bigg)U_ {t,T}(u)
\end{align}
where $u=0.5$. Thus we can set $X_t(0.5)$ as $\epsilon_t$. Similarly, with arbirary $u$ in $[0,1]$, we can set $X_t(u)$ as 
\begin{align}
X_t(u)=\begin{cases}
