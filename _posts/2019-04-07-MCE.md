...
\end{align}
이제 증명을 하여보자. 먼저 편의상 $D_n$과 $D_0$가 모두 랜덤이 아니라고 생각하여보자. 즉 조건들은 아래와 같이 재정리된다. 
**(1)** $\theta = \underset{\theta \in {\cal C}}{\operatorname{argmin}}D_0(\theta)$ **(2)** $\theta_n = \underset{\theta \in {\cal C}}{\operatorname{argmin}}D_n(\theta)$ **(3)** $\underset{\theta \in {\cal C}}{\operatorname{sup}}\left|D_n - D_0\right|  \overset{p_{\theta}}{\rightarrow} 0  ~~ as ~ n\rightarrow \infty$ 
