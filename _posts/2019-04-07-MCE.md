...
\end{align}
이제 증명을 시작해보자. ${\cal C}$가 compact set이므로 모든 $k\in\mathbb{N}$에 대하여 $\theta_k \in {\cal C}-\bar{\cal B}$이고 아래가 성립하게 하는 수열 $\\{\theta_k\\}$ 잡을 수 있다. 
\begin{align}
\theta_k \rightarrow \underset{\theta \in {\cal C}-\bar{\cal B}}{\operatorname{inf}}
\end{align}
