...
\end{align}
첫번째 부등호가 성립함을 좀 더 자세히 살펴보자. 먼저 ${\cal C}$를 아래와 같은 2부분으로 나누자. 
\begin{align}
{\cal C}={\cal C}^+ \cup {\cal C}^- = \\{\theta: \underset{\theta \notin {\cal B}}{\operatorname{inf}} D_0(\theta) - D_n(\theta)>0\\} \cup \\{\theta: \underset{\theta \notin {\cal B}}{\operatorname{inf}} D_0(\theta) - D_n(\theta) \leq 0\\}
\end{align}
여기에서 
