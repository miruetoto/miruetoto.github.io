...
\end{align}
가 되도록 하는 적당한 $\epsilon>0$이 존재한다고 가정하자. 위의 식의 의미는 무엇일까? $\theta_0$주위에 적당한 반경 $\epsilon>0$을 잡았다고 할때 이 반경안에는 $\theta_n$이 들어오지 않는다는 의미이다. 편의상 이러한 반경을 $\bar{{\cal B}}:=\\{\theta:|\theta_0-\theta|<\epsilon \\}$라고 하자. 따라서 $D_0(\theta_0)$는 반경 ${\cal B}$에서 최소값을 가지고 $D_n(\theta)$은 ${\cal C}-{\cal B}$에서 최소값을 가진다. 
이제
\begin{align}
...
\end{align}
증명을 시작하기 전에 $\theta_0$의 **유일성**이 반드시 위의 부등식을 보장하는건 아니라는 점을 언급하고 싶다. $\theta_0$가 **유일**하더라도 $\theta \in \partial {\cal B}$이라면 아래식이 만족한다. 
\begin{align}
