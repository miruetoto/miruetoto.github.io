...
\end{align}
로 변경된다. 즉 확률수렴에 관련된 부분들이 모두 일반수렴으로 변화하게 된다. 정리하면  
\begin{align}
\theta = \underset{\theta \in {\cal C}}{\operatorname{argmin}}D_0(\theta)
\end{align}
\begin{align}
...
\theta_n \rightarrow \theta_0
\end{align}
임을 증명하면 된다. 귀류법을 쓰기위해 $\theta_n \rightarrow \theta_0$가 아니라고 생각하자. 그러면 
\begin{align}
...
\begin{align}
\underset{\theta \notin {\cal B}}{\operatorname{inf}} D_0(\theta) -\underset{\theta \notin {\cal B}}{\operatorname{inf}} D_n(\theta)  
\leq \left| \underset{\theta \notin {\cal B}}{\operatorname{inf}} D_0(\theta) -\underset{\theta \notin {\cal B}}{\operatorname{inf}} D_n(\theta) \right|
...
\end{align}
\begin{align}
\underset{\theta \notin {\cal B}}{\operatorname{sup}} D_0(\theta)= - \underset{\theta \notin {\cal B}}{\operatorname{inf}} D_0(\theta)
\end{align}
를 각각 이용하면 아래를 얻을 수 있다. 
\begin{align}
\left| \underset{\theta \notin {\cal B}}{\operatorname{inf}} D_n(\theta)  - \underset{\theta \notin {\cal B}}{\operatorname{inf}}  D_0(\theta) \right| 
\leq \underset{\theta \notin {\cal B}}{\operatorname{sup}} \left| D_0(\theta) - D_n(\theta) \right| 
\end{align}
이제 아래를 이용하면 쉽게 우리가 원했던 바를 증명할 수 있다. 
\begin{align}
\underset{\theta \notin {\cal B}}{\operatorname{sup}} \left| D_0(\theta) - D_n(\theta) \right| \leq \underset{\theta \in {\cal C}}{\operatorname{sup}} \left| D_0(\theta) - D_n(\theta) \right| 
\end{align}
