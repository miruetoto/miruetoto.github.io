...
- 그럼 $\hat{\theta}$은 보통 어떻게 구할까? 대충 다음과 상황을 가정할 수 있다. (1) $\theta$는 특정한 식 $D_0(\theta)$를 최소화 하여 구할 수 있음을 알고 있는 상황이다. (2) $\theta$는 $D_0(\theta)$를 유일하게 최소화한다. 혹은 유일하게 최소화하는 *compact set* $\cal C$를 잡을 수 있다. 즉 우리는 아래식을 만족하는 적당한 $D_0(\theta)$와 *campact set* ${\cal C}$를 알고 있다. 
\begin{align}
