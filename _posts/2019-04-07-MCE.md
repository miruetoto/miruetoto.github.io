...
\end{align}
여기에서 첫번째 부등호가 성립함을 좀 더 자세히 살펴보자. 아래식이 성립한다. 
\begin{align}
D_0(\theta) = D_0(\theta) - D_n(\theta) + D_n(\theta)
\end{align}
따라서 
\begin{align}
\underset{\theta \notin {\cal B}}{\operatorname{sup}} D_0(\theta) \leq \underset{\theta \notin {\cal B}}{\operatorname{sup}} D_0(\theta) - D_n(\theta) + \underset{\theta \notin {\cal B}}{\operatorname{sup}}  D_n(\theta)
\end{align}
