...
- 콘트라스트란 다른물체와 구별되는 시각적인 특성을 의미한다. 여기에서는 콘트라스트 함수라는 것이 등장하는데 보통 이를 기호로 $\rho_0(\theta)$라고 쓰는 것 같다. 그리고 이것의 샘플버전을 $\rho_n(\theta)$라고 하는것 같다. 또한 콘트라스트 함수의 기대값을 $D_0(\theta)$라고 하고 이것의 샘플버전을 $D_n(\theta)$라고 하는것 같다. 

### Thm (and background story) 
- 확률공간 $(\Omega,{\cal F},P_{\theta})$에서 랜덤변수 $X$를 생각하자. 이때 보통 우리는 $\theta$에 관심이 있다. 그런데 이것을 알기 위해서는 $X$의 모든 $\omega \in \Omega$에서의 realization을 모은 집합 $\\{X(\omega), \omega \in \Omega\\}$ 즉 **앙상블**에 대한 통계량을 고려해야 한다. 예를들어 $\theta=(\mu,\sigma^2)$이라면 아래를 계산해야 한다. 
\begin{align}
...
\end{align}
집합 $\\{X_i, i \in \mathbb{N}\\}$을 **샘플**이라고 한다. (여기에서 샘플의 원소가 꼭 *i.i.d.*이어야 할것처럼 예를 들었지만 꼭 그런것은 아니다.) 아무튼 각설하고 주된 스토리는 "우리는 **앙상블**을 통해서만 구할 수 있는 $\theta$를 알고 싶은데 **앙상블**을 못구한다. 그래서 대신 **샘플**을 가지고 $\theta$를 추정하는 estimator $\hat{\theta}$를 구한다."는 것이다. 

- 우리가 구하고 싶은 특정 모수값을 $\theta_0$라고 하자. 또한 $\theta_0$에 대한 추정량으로 $\hat{\theta_0}$를 생각하자. 그럼 $\hat{\theta_0}$을 어떻게 구할까? 이번 포스트에서는 대충 다음과 상황에서 $\theta_0$를 추정한다. <br/><br/>
**(1)** $\theta_0$는 특정한 식 $D_0(\theta)$를 최소화 하여 구할 수 있음을 알고 있다. <br/><br/>
...
\end{align}<br/><br/>
**(3)** $D_0(\theta)$는 *compact set* ${\cal C}$에서 연속이다. <br/><br/>
...
\underset{\theta \in {\cal C}}{\operatorname{sup}}\left|D_n(\theta) - D_0(\theta)\right|  \overset{p_{\theta}}{\rightarrow} 0  ~~ as ~ n\rightarrow \infty
\end{align}
의 조건은 좀 과하다는 생각이 든다. 왜 이렇게 과한 조건을 줄까? 

- 사실 우리가 알고 싶은건 
\begin{align}
\lim_{n \rightarrow \infty} \underset{\theta \in {\cal C}}{\operatorname{argmin}}D_n(\theta) 
= \underset{\theta \in {\cal C}}{\operatorname{argmin}}\lim_{n \rightarrow \infty} D_n(\theta)
\end{align}
이 언제 성립가능한것인가? 이다. 위의 조건이 성립한다면 $\lim_{n \rightarrow \infty} D_n(\theta) = D_0(\theta)$가 되므로 원하는 바가 바로 증명된다. $\operatorname{argmin}$함수가 연속함수라면 이 조건이 그냥 성립하겠지만 그렇지는 않아서 좀 더 강한 수렴조건이 필요하다. 그러한 조건이 바로 
\begin{align}
\underset{\theta \in {\cal C}}{\operatorname{sup}}\left|D_n(\theta) - D_0(\theta)\right|  \overset{p_{\theta}}{\rightarrow} 0  ~~ as ~ n\rightarrow \infty
\end{align}
이라고 이해하자. 
...
\end{align}
을 편의상 $\Delta_n$이라고 정의하자. 즉 $\Delta_n$은 **앙상블**에서의 콘트라스트 함수와 **샘플**에서의 콘트라스트 함수간  차이를 each point $\theta \in {\cal C}$에서 구한것들의 최대값을 의미한다. 증명은 아래 식이 성립함을 이해하는 것에서 시작한다.  
\begin{align}
