...
\end{align}
$D_0(\theta)$는 **앙상블**이 있어야 구할 수 있다. **앙상블**은 우리가 실제 구할 수 없는 집합이므로 $D_0(\theta)$의 **샘플**버전 $D_n(\theta)$가 필요하다. **샘플**버전은 항상 샘플크기 $n$이 커질수록 **앙상블**과 비슷해져야하는 것이 바람직하다. 따라서 당연히 $D_n$은 $n \rightarrow \infty$일수록 점점 $D_0(\theta)$에 비슷해지는 성질이 있다. 우리는 아래식을 만족하는 적당한 $D_n(\theta)$을 구할 수 있다. 
