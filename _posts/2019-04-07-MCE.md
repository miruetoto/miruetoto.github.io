...
\end{align}
여기에서 $\underset{\theta \in {\cal C}-{\cal B}}{\operatorname{inf}} D_0(\theta) - D_0(\theta_0):=\delta(\epsilon)$으로 정의하자. 처음에 우리가 $\left|\theta_n-\theta_0\|>\epsilon$을 가정하였으므로 아래와 같이 주장할 수 있다. 
\begin{align}
P\left(\left|\theta_n-\theta_0\|>\epsilon\right)\leq P\left(\Delta_n \leq \frac{1}{2}\delta(\epsilon) \right)
\end{align}
