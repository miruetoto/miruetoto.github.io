...
\end{align}
라고 두고 싶다. 여기에서 $\mathbb{E} \\| \boldsymbol{\dot h}(v_i)-\boldsymbol{\dot h}(v_j) \\|_ 2^2$ 는 ***quadratic form of graph Laplacian*** 이다. 그런데 $T(G ; E)$ 가 ***Fisher consistent*** 를 만족하려면 
\begin{align}
T(G ; E)=\sum_{(i,j) \in E}  \mathbb{E} \\| \boldsymbol{\dot h}(v_i)-\boldsymbol{\dot h}(v_j) \\|_ 2^2 = \theta
\end{align}
라고 둘 수 있어야 하는데 이것이 좀 애매한게 $\boldsymbol{\dot h}(v)$ 의 분포를 결정짓는 파라메터가 $\theta$ 
