...
- ***claim 1.*** $f(v_i)$가 랜덤이 아니라고 하자. 그러면 아래가 성립하게 하는 적당한 함수 $g$가 존재한다. 
\begin{align}
\frac{\bf Lf}{\mbox{something}} \to \frac{1}{\mbox{vol}({\cal M})}{\cal L}_ {\cal M}f(x)
\end{align}
이때 ${\cal L}_ {\cal M}$는 매니폴드 ${\cal M}$에 대한 라플라스-벨트라미 오퍼레이터를 의미한다. 그리고 something 은 bandwidth 와 관련된 항목인데 $n$이 커짐에 따라서 작아
