...
- 여기에서는 헴펠교재 P.316 의 내용을 참고하였다. 선형모델에서 모수 $\boldsymbol{\beta}$의 추정량 $T_n$은 아래를 푸는 것이고 
\begin{align}
\min_{T_n}\sum_{i=1}^{n}\rho(y_i-{\bf x}_ i T_n)
\end{align}
이것은 다시 아래의 방정식의 해를 푸는것과 같다. 
\begin{align}
\sum_{i=1}^{n}\psi\big(y_i-{\bf x}_ i T_n\big){\bf x}_ i^T=0 
\end{align}
이런 추정량 $T_n$을 $M$-estimator라고 
