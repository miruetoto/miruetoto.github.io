...
\begin{align}
w_{ij}=1_ {(i,j)\in E} \frac{-\\|\boldsymbol{\dot h}_ i-\boldsymbol{\dot h}_ j\\|^2}{b_{\tau}}
\end{align}
여기에서 $b_{\tau}$은 눈에 쌓임에 따라서 커지는 snow-distance 에 대한 어떤 보정치이다. 
...
즉 ${\cal G}=(V,E)$ 와 같은 스트럭처에서 각 노드에 매칭되어있는 $f(v_i)$을 관측하였다. 
