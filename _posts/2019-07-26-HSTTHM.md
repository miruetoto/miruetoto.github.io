...
\begin{align}
w_{ij}=\begin{cases}
\exp \Big(-\frac{d_1^2(v_i,v_j)}{2\theta^2}\Big) & (i,j) \in E \\\\ \\
0 & o.w.
\end{cases}
\end{align}
이다. 
...
\end{align}
여기에서 $F(x)=\mu(-\infty,x]$ , $\mu:=P \circ {\boldsymbol h}^{-1}$ 이고 ${\cal A}$ 는 $(\Omega,{\cal F})$ 에서 정의가능한 모든 ***finited-signed measure*** 들의 집합 혹은 그것의 convex subset 이다. 이때 $T$ 에 대한 ***$\tau+1$-dimensional influence function*** 은 아래와 같이 정의한다. 
\begin{align}
IF({\boldsymbol h}; T,F)= \lim_{h\downarrow 0}\frac{T\left[(1-{\boldsymbol h})F +h \right]}{h}
\end{align}
...
- 벨킨의 논문에는서 ${\cal M}$ 은 유클리드 스페이스 $\mathbb{R}^{N}$에 아이소메트릭하게 임베딩된 ***compact and smooth*** 매니폴드라고 가정한다(벨킨, 2008, p.1290). 이러한 가정은 매니폴드에서 샘플된 자료들에 대한 엣지를 고려하지 않고 단순히 거리만으로 유사성을 따지기 위함이다. 그리고 아래와 같은 성질을 모순없이 가정하기 위함인듯 하다. 
\begin{align}
{\cal G} \to {\cal M} \quad as~ n\to \infty 
\end{align}
하지만 우리는 위의 상황을 직접 가정하였기 때문에 유클리드 스페이스에 임베딩되었다는 가정은 필요하지 않아보인다. 그리고 compact and smooth 역시 필요없어보인다. 

- 우리가 분석하고 싶은 신호는 특정 매니폴드를 도메인으로 가지는 함수 $f$이다. 즉 
\begin{align}
f:{\cal M} \to \mathbb{R} 
\end{align}
이다. 우리는 이것을 관찰한 것이 아니라 이것의 샘플버전 $f(v_i)$를 관측한 것이다. 벨킨 2008 의 논문에서는 그래프라플라시안을 아래와 같이 정의하였다. 
\begin{align}
\Big({\cal L}_ n^h\Big)_ {ij}=
\begin{cases} 
-w_{ij} & i\neq j \\\\ \\
\sum_k w_{ik} & i=j. 
\end{cases} 
...
여기에서 
\begin{align}
w_{ij}=\exp{\Bigg(\frac{-\\|x_i-x_j\\|^2}{4h}\Bigg)}
\end{align}
을 의미한다. 하지만 이것은 그래프의 노드가 유클리드 공간에 임베딩된 어떤 매니폴드에서의 샘플이고 그래서 거리를 정의할 수 있기 때문에 가능한 것이다. 반면에 우리가 분석하고 싶은 그래프는 노드간의 거리를 정의할 수 없다. 따라서 이 논문에서는 $w_{ij}$를 아래와 같이 정의한다. 
\begin{align}
w_{ij}=d_1(v_i,v_j). 
\end{align}
혹은 $d_1(v_i,v_j)$ 대신 $d_2(v_i,v_j)$로 정의할수도 있다. HST의 그래프라플라시안은 다른논문들과 유사하게 아래와 같은 매트릭스로 정의하자. 
\begin{align}
{\bf L}_ {ij}=
\begin{cases} 
-w_{ij} & i\neq j \\\\ \\
...
- 우리의 정의에서는 $h$와 같은 밴드윗의 역할이 빠져있다. 후에 ***asymptotic property*** 를 증명하기 위해서는 밴드윗이 0으로 간다는 조건이 필수적인데 HST의 아이디어로 비춰볼때 이 조건은 
\begin{align}
