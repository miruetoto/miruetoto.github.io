...
- ***claim 1.*** $f:{\cal M} \to \mathbb{R}$ 이라고 하자. $P_{\cal M}$를 ${\cal M}$에서 정의된 ***probability measure*** 라고 하고 $p_{\cal M}$ 을 $P_{\cal M}$ 에 대응하는 ***probability density function*** 이라고 하자. 이제 $v_1,\dots,v_n$을 ${\cal M}$에서 추출된 어떠한 sample 이라고 하자. $f(v_i)$를  랜덤이 아니라고 하자. 
\begin{align}
{\bf Lf}=
\end{align}
...
그러면 아래가 성립한다. 
\begin{align}
\frac{\bf Lf}{\mbox{something}} \to \frac{1}{\mbox{vol}({\cal M})}{\cal L}_ {\cal M}f(x)
\end{align}
이때 ${\cal L}_ {\cal M}$는 매니폴드 ${\cal M}$에 대한 라플라스-벨트라미 오퍼레이터를 의미한다. 그리고 something 은 bandwidth 와 관련된 항목인데 $n$이 커짐에 따라서 작아지는 특징을 가지고 있다고 하자. 
