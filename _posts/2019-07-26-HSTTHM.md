...
\begin{align}
w_{ij}=1_ {(i,j)\in E} \exp{\Bigg(\frac{-\\|\boldsymbol{\dot h}_ i-\boldsymbol{\dot h}_ j\\|^2}{b_n}\Bigg)}
\end{align}
여기에서 $b_n$은 $n$에 비례하는 어떠한 수열이다. $b_n$은 밴드윗의 역할을 한다. 제한된 매니폴드 공간 ${\cal M}$에서 뽑은 샘플수가 많아질수록 두 노드의 유사도는 높아진다고 해석할 수 있으므로 유사도에 $n$에 대한 함수를 넣었다. 
