...
- 노드간의 snow-dist 역시 $\boldsymbol{h}(v_i)$ 의 함수이므로 $T$로 표현가능하다. 따라서 $f(v_i)$ 의 ***i.i.d.*** 를 가정한다면 노드간의 snow-dist 역시 $N(0,V(T,F))$ 를 따른다. 따라서 ***i.i.d.*** 가정하에서 노드간의 snow-distance 를 히스토그램으로 그려보면 정규분포와 비슷한 모양이 된다. 이때 snow-dist들의 평균거리가 0으로 수렴한다는 조건이 추가적으로 있으면 claim2 에 의해서 모든 노드들이 한점으로 뭉쳐버리게 된다. 

- 거리들의 히스토그램이 정규분포처럼 보이지 않는 경우는 우선 각 노드들의 ***i.i.d*** 를 보장할 수 없다고 주장할 수 있다. 그렇다면 각각의 거리가 정규분포처럼 보이는 그룹이 2개가 있다면 이것은 각 노드가 서로다른 2개의 분포에서 생성된 랜덤변수라고 주장할 수 있을까? 아래가 성립하면 이렇게 주장할 수 있다. 

- ***claim 3.*** 모든 $\tau$에 대하여 아래를 만족하는 거리 (혹은 유사거리) $d^* $ 가 존재한다. 
\begin{align}
d^* \big(\boldsymbol{h}(v_i),\boldsymbol{h}(v_j)\big) = d \big(F^* (f(v_i)),F^* (f(v_j))\big)
\end{align}
여기에서 $F^* (f(v)) :=\mu_1(-\infty,h_1]\times \mu_1(-\infty,\infty] \dots \times \mu_{\tau+1}(-\infty,\infty]$ 이고 
