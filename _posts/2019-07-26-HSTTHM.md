...
\begin{align}
IF({\boldsymbol h}; T,F)= \lim_{t\downarrow 0}\frac{T\Big((1-t)F + t \delta(\boldsymbol{h}) \Big)-T(F)}{t}
\end{align}
우리는 아래와 같이 $IF({\boldsymbol h}; T,F)$의 ***sample version*** 을 생각해 볼 수 있다. 
\begin{align}
SC_{n}({\boldsymbol h})= \frac{T\Big((1-1/n)F_{n-1} + 1/n \delta(\boldsymbol{h}) \Big)-T(F_{n-1})}{1/n}
\end{align}
