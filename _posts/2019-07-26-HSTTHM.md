...
\end{align}
이때 $v_i \in {\cal M}$ 주변의 점들을 ${\cal N}(v_i)$ 라고 하자(이것은 ${\cal N}_ i$의 continuous 버전이라고 볼 수 있음). 만약에 $\int_{v \in {\cal N}(v) dP(v)>0$ 이면 $n \to \infty$ 일때 $\|{\cal N}_ i \| \to \infty$ 가 성립할것 같다. 이러한 조건하에서 아래가 성립할것 같다. 
\begin{align}
{\bf Lf}(v_i) \superset{\operatorname pr}{\rightarrow} {\cal L}_ {\cal M}f(v_i):=\frac{1}{\mbox{something} }\int_{\cal M} e^{ -\frac{d_1^2(v_i,v)}{2b^2} } \big(f(v_i)-f(v)\big)dP_{\cal M}(v)
\end{align}
