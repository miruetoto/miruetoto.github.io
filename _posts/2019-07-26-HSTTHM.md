...
\end{cases} 
\end{align} 
이때 $w_{ij}$는 아래와 같이 정의한다. 
\begin{align}
w_{ij}=\begin{cases}
\exp\bigg(-\frac{d_1(v_1,v_2)}{2a_n} \bigg) & (i,j) \in E \\\\ \\
0 & o.w.
\end{cases}
\end{align}
여기에서 $a_n$은 $n$이 증가함에 따라 점점 증가하는 어떠한 수열이다. (왠지 $\sqrt{\sum_{(i,j) \in E} d_1(v_1,v_2)}$로 정의하면 될것 같다.) 
