...
\begin{align}
s_2^{\tau}(f):=\sum_{(i,j)\in E} w_{ij} \big(f(v_i)-f(v_j)\big)^2={\bf f}^T {\bf L}^{\tau}{\bf f}
\end{align}
...
\begin{align}
IF({\boldsymbol h}; T^{\tau},F)= \lim_{t\downarrow 0}\frac{T\Big((1-t)F(\boldsymbol{h}) + t \delta(\boldsymbol{h}) \Big)-T(F(\boldsymbol{h}))}{t}
\end{align}
