...
- ***claim 1.*** 각각의 노드 $v_i$에서 정의된 확률변수 $f(v_i)$가 서로 독립이고 같은분포를 가진다면 $\boldsymbol{h}(v_i)$ 역시 서로 독립이고 같은 분포를 가질것이다. $F_n$을 $\boldsymbol{h}(v_1),\dots,\boldsymbol{\dot h}(v_n)$에 대응하는 empirical-cdf 라고 하자. 글리벤코-칸텔리정리는 $F_n \to F$ 임을 보여준다. 따라서 적당히 큰 $n$에 대하여 $F_n$은 $F$의 neighborhood에 속한다고 볼 수 있다. 따라서 
\begin{align}
...
- ***claim 2.*** $V(T,F)$ 는 $\tau \to \infty$ 일 경우 $0$ 으로 수렴한다. 왜냐하면 $\tau$ 가 커질수록 
\begin{align}
\boldsymbol{h}(v_1)\approx \boldsymbol{h}(v_2) \approx \dots \approx \boldsymbol{h}(v_n)
\end{align}
와 같이 되기 때문이다. 

- 노드간의 snow-dist 역시 $\boldsymbol{h}(v_i)$ 의 함수이므로 $T$로 표현가능하다. 따라서 $f(v_i)$ 의 ***i.i.d.*** 를 가정한다면 노드간의 snow-dist 역시 $N(0,V(T,F))$ 를 따른다. 그리고 $V(T,F)$ 의 값은 $\tau$ 가 커질수록 점점 작아지므로 $f(v_i)$ 들이 같은 분포를 가진다면 $\tau$가 커질수록 점점 평균거리 근처로 모이는 것을 쉽게 예측할 수 있다. 만약에 $\tau$가 커질수록 평균거리가 점점 0에 가까워진다는 조건을 주면 (어떻게??) 적딩히 큰 $\tau$에 대하여 모든 노드들을 한그룹에 뭉쳐보이게끔 만들 수 있다. 

- 반대로 말하면 적당히 큰 $\tau$ 에서 다른 노드들은 한그룹으로 뭉치는데 그곳에 뭉쳐지지 않는 점이 있다면 그것은 ***i.i.d*** 가정을 따르지 않는 노드라고 볼 수 있다. 

- 지금까지의 내용을 종합하면 즉 각각의 노드 $v_i$에서 정의된 확률변수 $f(v_i)$가 서로 독립이고 같은 분포를 가지는 경우에는 각 노드간의 snow-dist의 히스토그램은 정규분포를 따른다는것이다. 그리고 반대로 말하면 거리들의 히스토그램이 정규분포처럼 보이지 않는 경우는 우선 각 노드들의 ***i.i.d*** 를 보장할 수 없다는 의미가 된다. 그렇다면 각각의 거리가 정규분포처럼 보이는 그룹이 2개가 있다면 이것은 각 노드가 서로다른 2개의 분포에서 생성된 랜덤변수라고 주장할 수 있을까? 그리고 그룹간의 거리가 (그래프 ${\cal G}$를 만들어낸) 매니폴드 ${\cal M}$의 unsmoothness를 반영한다고 볼 수 있을까?
...
- ***claim 3.*** 여기서는 given ${\cal M}$ 에 대하여 특정한 확률로 ${\cal G}$ 가 뽑힌 경우를 고려한다. $f:{\cal M} \to \mathbb{R}$ 이라고 하자. $P_{\cal M}$ 를 ${\cal M}$ 에서 정의된 ***probability measure*** 라고 하고 $p_{\cal M}$ 을 $P_{\cal M}$ 에 대응하는 ***probability density function*** 이라고 하자. 이제 $v_1,\dots,v_n$을 $P_{\cal M}$에 따라 ${\cal M}$ 에서 추출된 어떠한 sample 이라고 하자. $f(v_i)$를  랜덤이 아니라고 하자. 
\begin{align}
