...
\begin{align}
s_2(f):=\sum_{(i,j)\in E} w_{ij} \big(f(v_i)-f(v_j)\big)^2={\bf f}^{\bf L}{\bf f}
\end{align}
...
\end{align}
여기에서 $F(x)=\mu(-\infty,x)$ , $\mu:=P \circ {\boldsymbol h}^{-1}$ 이고 ${\cal A}$는 $(\Omega,{\cal F})$ 에서 정의가능한 모든 ***finited-signed measure*** 들의 집합 혹은 그것의 convex subset 이다. 
