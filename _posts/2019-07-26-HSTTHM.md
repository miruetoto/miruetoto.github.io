...
- 거리들의 히스토그램이 정규분포처럼 보이지 않는 경우는 우선 각 노드들의 ***i.i.d*** 를 보장할 수 없다고 주장할 수 있다. 그렇다면 각각의 거리가 정규분포처럼 보이는 그룹이 2개가 있다면 이것은 각 노드가 서로다른 2개의 분포에서 생성된 랜덤변수라고 주장할 수 있을까? 아래가 성립하면 이렇게 주장할 수 있을것이다. 
