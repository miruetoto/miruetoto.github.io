...
- 콘다를 실행해서 아래와 같이 가상환경을 만든다. 둘 중 아무거나 실행해도 된다. <br/><br/>
```
...
- 아래를 실행하여 TeX을 깐다. 그리고 Ubuntu Software에서 TexStudio를 검색해서 깐다. <br/><br/>
```
sudo apt install texlive-full
```

- 이제 sublime과 latex을 연결하여보자. [여기](https://latextools.readthedocs.io/en/latest/install/#requirements-and-setup)를 참고하자. sublime을 키고 컨트롤+쉬프트+p 를 눌러 Package Control: Install Package 를 실행한다. 그러면 바로 검색창이 나오는데 거기서 LaTeXTools를 입력해서 실행한다. 다시 컨트롤+쉬프트+p를 누르고 LaTeXTools: Check system을 누른다. 모두 available이 나오면 잘된것이다. 


***주피터에 R커널 연결*** 
- 콘다환경으로 가서 아래를 실행한다. (이러면 콘다환경에는 R이 깔리고 base에는 R이 깔리지 않는다.) <br/><br/>
```
conda install -c r r
```
...
***Rstudio server***
- 이제 Rstudio server를 설치한다. 이건 콘다환경의 R과 연결이 안된다. (내가 알기론 그렇다.) 그래서 (base)에 R을 깔고 그 R과 연결해야한다. 그런데 우분투 18.04는 base에서 아래와 같이 실행하면 기본적으로 R이 3.4버전으로 깔린다. <br/><br/>
```
sudo apt install r-base
```

- 아래와 같이 하면 R 3.6을 깔 수 있다. (높은 버전을 까는게 좋다..) <br/><br/>
```
...
- 위와같이 해서 아래같은 에러가 날 수 있다. (안날수도있음) <br/><br/>
```
