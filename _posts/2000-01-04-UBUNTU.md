...
```
(base) 높은 버전의 R을 억지로 까는 방법이 있긴 있다. 추천하는 방법은 아니다. 대신 콘다환경 (py20190129)에서 가서 억지로 Rstudio를 연결하는 방법을 알아보자. 

- 내용은 [여기](https://github.com/grst/rstudio-server-conda)에 있다. 시작은 (base)에서 작업하자. 먼저 Rstudio를 깐다. 참고로 Rstudio server 설치하는법은 [여기](https://rstudio.com/products/rstudio/download-server/debian-ubuntu/)를 참고하라. <br/><br/>
```
sudo apt-get install gdebi-core
wget https://download2.rstudio.org/server/bionic/amd64/rstudio-server-1.2.5033-amd64.deb
sudo gdebi rstudio-server-1.2.5033-amd64.deb
```
보통 여기까지하면 Rstudio 설치가 끝난다. 그담에는 예를들어 http://192.168.0.4:8787 와 같은 곳에 접속하면 Rstudio를 쓸 수 있다. 여기에서 192.168.0.4은 ifconfig 쳐서 나오는 주소이다. 그런데 기본포트를 변경하고 싶을때가 있다. 나의 경우는 8786으로 변경하고 싶었다. 이럴 경우 아래를 실행하고 www-port=8786 이라고 저장한다. <br/><br/>
```
...
- 높은 버전의 R을 억지로 까는 방법을 알아보자. (알아보기만 하고 하지말자..) 예를들면 아래와 같이 하면 R 3.6을 깔 수 있다. 
```
...
```
위의 오류를 수정할때 [이곳](https://askubuntu.com/questions/1188794/unable-to-install-latest-r-version-on-ubuntu-18-04-linux)의 답변이 도움되었다. 답변을 요약하면 다음과 같다. 
