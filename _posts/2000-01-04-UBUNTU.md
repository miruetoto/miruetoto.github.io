...
- 이제 Rstudio에서 devtools와 같은 프로그램을 깔아보자. 이게 원래 그냥 깔면되는데 지금의 경우는 R을 억지로 3.6에 맞추어 깔았기 때문에 문제가 발생한다. 
아마 아래와 install.packages("devtools")를 하면 아래와 같은 에러를 받을 것이다. <br/><br/>
```
sudo rstudio-server restart
```
