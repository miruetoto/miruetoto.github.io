...
- 패스워드 없이 주피터를 띄우려면 아래와 같이 한다. <br/><br/>
```
...
conda install keras
```
그러면 알아서 CUDA, cuDNN, pytorch, tensorflow-gpu, keras가 깔린다. 현재는 파이썬 환경이 너무 높으면 깔리지 않는 현상이 있으니 환경을 만들때 파이썬버전을 3.7.6으로 통일하자. 
...
***`devtools::install_github()`가 먹히지 않을 경우***
- Rstudio를 쓰기전에 커맨드에 가서 아래를 실행하자. <br/><br/>
```
...
***keras with R***
- 이제 아까 띄운 Rstudio-server에서 아래를 설치하자. <br/><br/>
```
...
pip install rpy2
```
절대로 conda install rpy2로 설치하지말자. 기본적으로 낮은버전 rpy2가 설치되는데 내가 만든 push등 여러파일들이 3.0 이상에서 동작하도록 만들어져 있다. 따라서 코드가 모두 꼬여버린다. 

***pandas, matplotlib***
- 이제 콘다환경에서 나머지 패키지들을 깔아보자. <br/><br/>
```
conda activate py20190129
...
- 이제 sublime과 latex을 연결하여보자. [여기](https://latextools.readthedocs.io/en/latest/install/#requirements-and-setup)를 참고하자. (1) sublime을 키고 컨트롤+쉬프트+p 를 눌러 Install Package Control (2) 다시 컨트롤+쉬프트+p 를 눌러 Package Control: Install Package 를 실행한다. (3) 그러면 바로 검색창이 나오는데 거기서 LaTeXTools를 입력해서 실행한다. (4) 다시 컨트롤+쉬프트+p를 누르고 LaTeXTools: Check system을 누른다. 모두 available이 나오면 잘된것이다. 이게 끝이다. 
...
- 텍스트파일을 만들기 원하는 폴더로가서 아래를 입력한다. <br/><br/>
```
cat > sample.txt
```
아무것이나 치고 (예를들어 asdf) 컨트롤 C를 눌러서 빠져나온다. 

### github 사용방법

***clone***
- 깃을 설치할 폴더로 이동하여 아래를 입력한다. <br/><br/>
```
...
***download***
- 깃이 설치된 가장 상위폴더(Documents/Github/miruetoto.github.io)에 가서 아래를 입력한다. <br/><br/>
```
git pull
```
.
...
git push 
```
아이디(miruetoto)와 비밀번호를 차례로 입력한다. 
...
***branch***
- 서버에 이미 guebin이라는 브랜치가 있다면 아래와 같이 동기화 시킨다. <br/><br/>
```
git chechout guebin
...
### pip
- 이미 깔린 패키지를 강제로 업그레이드 하는방법 <br/><br/>
```
