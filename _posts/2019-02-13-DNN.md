...
\end{align} **굳이 따지면** 여기에서 **(1)** ${\bf x}_ i$ : $1\times p$인 row-vector, **(2)** ${\bf W}_ 1$: $p\times p_1$ matrix, **(3)** $f:\mathbb{R}^{1 \times p_1} \rightarrow \mathbb{R}^{1 \times p_1}$ 인 vector function, **(4)** ${\bf W}_ 2$: $p_1 \times p_2 $ matrix, **(5)** $g:\mathbb{R}^{1 \times p_2} \rightarrow \mathbb{R}^{1 \times p_2}$ 인 vector function, **(6)** ${\bf W}_ 3$: $p_2\times p_3$ matrix, **(7)** ${\bf y}_ i$ 와 ${\boldsymbol\epsilon}_ i$ 은 $p_3 \times 1$ row-vector 이다. 잘 살펴보면 입력데이터 ${\bf X}$를 어떠한 변환 ${\bf W}_ 1$를 활용하여 선형변환하고 그 뒤에 비선형함수 $f$를 씌우는 것을 반복하는 형태이다. 진짜 별 내용없는것 같다. 즉 결국에는 아래와 같은 형태로 표현할 수 있다. 
\begin{align}
