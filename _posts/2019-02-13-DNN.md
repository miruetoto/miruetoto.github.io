...
\end{align} **굳이 따지면** 여기에서 **(1)** ${\bf x}_ i$ : $1\times p$인 row-vector, **(2)** ${\bf W}_1$: $p_1\times p_2$ matrix, **(3)** $f:\mathbb{R}^{1 \times p_1} \rightarrow \mathbb{R}^{1 \times p_2}$ 인 vector function, **(4)** ${\bf W}_2$: $p_2 \times p_3 $ matrix, **(5)** $g:\mathbb{R}^{1 \times p_2} \rightarrow \mathbb{R}^{1 \times p_3}$ 인 vector function, **(6)** ${\bf W}_3$: $p_3\times m$ matrix, **(7)** ${\bf y}_ i$ 와 ${\boldsymbol\epsilon}_ i$ 은 $m \times 1$ row-vector 이다. 잘 살펴보면 입력데이터 ${\bf X}$를 어떠한 변환 ${\bf W}_1$를 활용하여 선형변환하고 그 뒤에 비선형함수 $f$를 씌우는 것을 반복하는 형태이다. 진짜 별 내용없는것 같다. 즉 결국에는 아래와 같은 형태로 표현할 수 있다. 
\begin{align}
...
**(3)** $f$가 소프트맥스함수인 경우 
\begin{align}
f \big({\bf x}_ i {\bf W}_1\big) = \frac{e^{ {\bf x}_ i {\bf W}_1 } }{e^{ {\bf x}_ i {\bf W}_1 }{\bf 1}}
\end{align}
소프트맥스 함수에서 $\exp$와 $/$는 **elementwise operation** 이다. 따라서 위에서 $e^{ {\bf x}_ i {\bf W}_1 }$는 row-vector가 된다. (왜냐하면 ${\bf x}_ i {\bf W}_1$ 가 row-vector 이니까) 따라서 분자는 row-vector 이고 분모는 row-vector $\times$ col-vector 꼴이니까 scalar가 된다. 그래서 $f \big({\bf x}_ i {\bf W}_1\big)$ 의 차원은 row-vector 가 된다. (이런것도 다 알려줘야하나..) 소프트맥스의 경우 입력이 row-vector ${\bf x}_ i {\bf W}_1$이고, 출력 $f \big({\bf x}_ i {\bf W}_1\big)$ 역시 row-vector 이므로 row-vector 를 row-vector 로 맵핑하는 함수가 된다. 
