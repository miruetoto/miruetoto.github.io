...
\end{align}
이제 하나씩 풀어보자. 
$J={\boldsymbol\epsilon}'{\boldsymbol\epsilon}=\Big({\bf y}-g\big(f({\bf X} {\bf T}){\bf S} \big){\bf R}\Big)'\Big({\bf y}-g\big(f({\bf X} {\bf T}){\bf S} \big){\bf R}\Big)$으로 표현가능하다. 여기에서 잠시 ${\bf G} = g\big(f({\bf X} {\bf T}){\bf S} \big)$로 놓고 $\frac{\partial J}{\partial \bf R}=0$을 계산하면 아래와 같이 된다. 
\begin{align}
{\bf G}'{\bf G}{\bf R} = {\bf G}'{\bf y} 
\end{align}
이제 $\frac{\partial J}{\partial \bf S}$ 를 계산하여 보자. 이번에는 ${\bf F}=f({\bf X} {\bf T})$이라고 정의하자. 
\begin{align}
