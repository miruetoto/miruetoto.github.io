...
{\bf y} = g\big(f({\bf X} {\bf T}){\bf S} \big){\bf R} + {\boldsymbol \epsilon}
\end{align} 
우리의 목적은 $J=\big({\bf y}-{\bf\hat y}\big)'\big({\bf y}-{\bf\hat y}\big)$을 최소화하는 적당한 $\bf T, S, R$을 찾는 것이다. 여기에서 $J$는 스칼라이다. $J={\boldsymbol\epsilon}'{\boldsymbol\epsilon}=\Big({\bf y}-g\big(f({\bf X} {\bf T}){\bf S} \big){\bf R}\Big)'\Big({\bf y}-g\big(f({\bf X} {\bf T}){\bf S} \big){\bf R}\Big)$으로 표현가능하므로, 
\begin{align}
asdf
\end{align}
