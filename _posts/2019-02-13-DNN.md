...
\end{align}
이제 하나씩 풀어보자. 
$J={\boldsymbol\epsilon}'{\boldsymbol\epsilon}=\Big({\bf y}-g\big(f({\bf X} {\bf T}){\bf S} \big){\bf R}\Big)'\Big({\bf y}-g\big(f({\bf X} {\bf T}){\bf S} \big){\bf R}\Big)$으로 표현가능하다. 여기에서 잠시 ${\bf \tilde X} = g\big(f({\bf X} {\bf T}){\bf S} \big)$로 생각하고 $\frac{\partial J}{\partial \bf R}=0$을 계산하면 아래와 같이 된다. 
\begin{align}
{\bf \tilde X}'{\bf \tilde X}{\bf R} = {\bf \tilde X}'{\bf y} 
\end{align}
..
