...
- 이때 isometric 이라는 ***relation*** 은 보는것 처럼 ***equivalence relation*** 이다. 즉 아래가 성립한다.  <br/><br/>
...
- ***convergent sequence*** $\\{a_n\\}$ 와 ***Cauchy sequence*** $\\{c_n\\}$ 은 비슷해보이지만 미묘하게 다른점이 있다. <br/><br/>
**(1)** 수렴하는 수열 $\\{a_n\\}$ 은 위상공간 $(X,{\cal T})$ 만 있으면 정의할 수 있지만 코시수열 $\\{c_n\\}$ 은 그 위상공간이 메트릭스페이스 이어야 한다는 제약이 있다. 왜냐하면 수렴하는 수열은 정의에 오픈셋만 필요하지만 코시수열은 볼이 필요하고 볼은 메트릭에 의해서만 정의되기 때문이다.  <br/>
...
- 컴플리트하지 않은 메트릭스페이스 $(X,d)$를 컴플리트한 메트릭스페이스 $(X^* , d)$ 로 바꿀 수 없을까? 유주얼메트릭(usual metric) $d$ 와 $X=(0,1)$ 로 만들어지는 메트릭스페이스는 컴플리트하지 않지만 $d$ 와 $X^* =[0,1]$ 로 만들어지는 메트릭스페이스는 컴플리트하다. 이런 경우 $(X^* ,d) $ 는 $(X,d)$ 의 ***completion*** 이라고 한다. 즉 <br/><br/>
**(1)** $(X^* ,d)$ 가 컴플리트한 메트릭스페이스 이고 <br/>
**(2)** $X\subset X^* $ , $(X,d) \overset{ism}{\sim} (X^* ,d)$ <br/><br/>
이면 $(X^* ,d) $ 는 $(X,d)$ 컴플리션이라고 부른다.    
