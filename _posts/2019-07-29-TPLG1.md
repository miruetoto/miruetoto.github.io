...
- 저 정리가 생각보다 중요하다. 그리고 이 정리를 **나이테정리** 라고 기억하자. 이 정리는 ${\cal O}$-오픈셋이 아닌 일반적인 ${\cal T}$-오픈셋에 대하여서도 성립한다. 즉 $(X,{\cal T})$가 위상공간이고 $T$가 ${\cal T}$의 임의의 집합이라 하자. $T$의 임의의 원소 $p$에 대하여 (1) $p$ 를 포함하지만 (2) $T$ 보다 작은 다른 ${\cal T}$-오픈셋 $T_p$ 가 항상 존재한다. 그리고 교재에 따라서는 위와 같은 성질을 만족하는 것을 오픈셋이라고 정의하기도 한다. 
