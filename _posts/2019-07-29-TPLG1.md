...
\end{align}
이 집합을 $\\{a_n\\}$의 ***equivalence class on ${\cal C}[X]$*** 라고 부른다. ${\cal C}[X]$에서의 이퀴배런스-클래스는 본질적으로 파티션과 관련이 있다.  어떠한 집합 ${\cal C}[X]$에서 이퀴배런스-릴레이션이 존재하면 그 집합을 배타적인 이퀴배런스-클래스의 합집합으로 표현가능하다. 즉 파티션이 존재한다. 아래와 같은 식으로 말이다. 
\begin{align}
