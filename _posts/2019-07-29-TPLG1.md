...
- 잠시 바이너리-릴레이션(***binary relation***), 이퀴배런스-릴레이션(***equivalence relation***), 이퀴배런스-클래스(***equivalence class***) 그리고 코션트셋(***quotient set***)에 대하여 설명하고 넘어가겠다. 

- 집합 ${\cal C}[X]$의 두 원소 $\\{a_n\\}$, $\\{b_n\\}$ 간 바이너리-릴레이션 $R$이 존재한다는 문장은 집합론적인 언어로 표현가능하다. 구체적으로는 (1) $R$을 곱집합 ${\cal C}[X] \times {\cal C}[X]$ 의 적당한 부분집합으로 설정하고 (2) 순서쌍 $\big(\\{a_n\\},\\{b_n\\}\big)$ 이 $R$ 의 원소라는 식으로 표현한다. 예를 들면 아래와 같은 식으로 말이다.  
\begin{align}
...
- 그리고 ${\cal C}[X]$ 위에서의 바이너리-릴레이션 $R$이 ***(i) reflexivity (ii) symmetricity (iii) transitivity*** 를 만족하면 이 릴레이션을 특별히 이퀴배런스-릴레이션 이라고 말한다. 

- 그리고 아래와 같이 ${\cal C}[X]$에서 $\\{a_n\\}$와 이퀴배런스-릴레이션을 가지는 원소들을 모아놓은 집합을 생각할 수 있다. 
\begin{align}
\big[\\{a_n\\}\big]_ R:=\big\\{ \\{x_n\\} : \\{x_n\\} \overset{R}{\sim} \\{a_n\\} \mbox{ and } \\{x_n\\} \in {\cal C}[X]\big\\} 
\end{align}
이 집합을 $\\{a_n\\}$의 ***equivalence class on ${\cal C}[X]$ by $R$*** 이라고 부른다. 보통은 $R$을 생략하여 $\big[\\{a_n\\}\big]$와 같이만 표현하지만 나는 기호의 명확성을 위해서 관계까지 명시하였다. 

- 이퀴배런스-클래스는 본질적으로 파티션과 밀접한 연관이 있다. 여기에서 클래스 ${\cal P}_ X$ 가 집합 $X$의 파티션이란 의미는 클래스 ${\cal P}_ X$ 에 속한 모든원소의 합이 $X$ 이며 클래스 ${\cal P}_ X$ 의 각 원소는 서로 배타적이라는 의미이다. 이퀴배런스-클래스가 그럼 왜 파티션과 관련이 있을까? 그것은 어떠한 집합에서 이퀴배런스-릴레이션이 존재하면 그 집합을 배타적인 이퀴배런스-클래스의 합집합으로 표현가능하기 때문이다. 즉 이퀴배런스-릴레이션 혹은 이퀴배런-클래스의 존재는 파티션의 존재를 의미한다. 
\begin{align}
