...
- $X=(0,1)$ 위의 usual metric 에 의해서 유도되는 위상공간 $(X,{\cal T})$ 를 생각하자. 수열 $\big\\{\frac{1}{2},\frac{1}{3},\frac{1}{4},\dots,\big\\}$ 은 $X$ 에서 정의된 코시수열이지만 그 극한 $0$ 은 $X$ 의 원소가 아니므로 이 수열은 $X$ 에서 수렴하는 수열은 아니다. (참고로 수렴 역시 확률변수처럼 단독으로 정의되지 않는다. "$X$에서 수렴한다" 와 같이 표현해야 맞는 표현이다.) 이처럼 모든 코시수열은 ***limit*** 을 가지지만 그 limit이 반드시 $X$에 존재한다는 보장은 없다. 이러한 이유로 ***complete*** 한 메트릭스페이스라는 개념을 만들었는데 이런 메트릭스페이스에서는 모든 코시수열의 limit 을 포함한다. 
