...
- ***convergent sequence*** 와 ***Cauchy sequence*** 는 비슷해보이지만 미묘하게 다른점이 있다. <br/><br/>
**(1)** 수렴하는 수열은 위상공간 $(X,{\cal T})$ 만 있으면 정의할 수 있지만 코시수열은 그 위상공간이 메트릭스페이스 이어야 한다는 제약이 있다. 왜냐하면 수렴하는 수열은 정의에 오픈셋만 필요하지만 코시수열은 볼이 필요하고 볼은 메트릭에 의해서만 정의되기 때문이다.  <br/>
**(2)** 수렴하는 수열과 코시수열 모두 열의 각 항이 $X$의 원소이어야 한다는 조건이 있다. 하지만 수렴하는 수열은 그 ***limit*** 까지 $X$의 원소이어야 하지만 코시수열은 그렇지 않다. 

- $X=(0,1)$ 위의 usual metric 에 의해서 유도되는 메트릭스페이스 $(X,{\cal T})$ 를 생각하자. 수열 
\begin{align}
\big\\{\frac{1}{2},\frac{1}{3},\frac{1}{4},\dots,\big\\}
\end{align}
$X$에서 정의된 코시수열이지만 $X$에서 수렴하는 수열은 아니다. 

- ***(proposition 14.1)*** 메트릭스페이스 한정으로 수렴하는 수열은 모두 코시수열이다(메트릭스페이스가 아닌 일반적인 위상공간에서는 당연히 성립하지 않는데, 그 이유는 그런 공간에서는 코시수열자체가 정의 될 수 없기 때문). 이 정리의 역은 성립하지 않는다. 즉 메트릭스페이스에서 정의된 코시수열이 반드시 수렴하는 수열이라는 보장은 없다. 하지만 그 메트릭스페이스가 ***complete*** 하다면 이 정리의 역도 성립한다. 

- 어떠한 메트릭스페이스 $(X,d)$가 ***complete*** 하다는 의미는 $X$에서 정의가능한 모든 코시수열이 수렴하는수열 이라는 의미이다.  
...
- 즉 아래의 조건들을 만족하면 $(X^* ,d) $ 는 $(X,d)$ 의 ***completion*** 이라고 부른다. <br/><br/>
**(1)** $X\subset X^* $ <br/>
...
\end{align}
이라고 표현하자. 보는것처럼 관계 $\overset{slim}{\sim}$ 은 $X$에서 정의가능한 모든 코시수열들의 집합 ${\cal C}[X]$ 에서 equivalence relation 이다. 

- 이제 $X^* := {\cal C}[X] ~ / \overset{slim}{\sim}$ 을 생각하여 보자. 여기에서 $X^* $ 는 $\overset{slim}{\sim}$ 에 의한 ${\cal C}[X]$ 의 ***quotient set*** 이라고 한다. 

- 잠시 바이너리-릴레이션(***binary relation***), 이퀴배런스-릴레이션(***equivalence relation***), 이퀴배런스-클래스(***equivalence class***) 그리고 코션트셋(***quotient set***)에 대하여 설명하고 넘어가겠다. 집합 ${\cal C}[X]$의 두 원소 $\\{a_n\\}$, $\\{b_n\\}$ 간 바이너리-릴레이션 $R$이 존재한다는 문장은 집합론적인 언어로 표현가능하다. 구체적으로는 (1) $R$을 곱집합 ${\cal C}[X] \times {\cal C}[X]$ 의 적당한 부분집합으로 설정하고 (2) 순서쌍 $\big(\\{a_n\\},\\{b_n\\}\big)$ 이 $R$ 의 원소라는 식으로 표현한다. 예를 들면 아래와 같은 식으로 말이다.  
\begin{align}
& \\{a_n\\} \mbox{ and } \\{b_n\\} \mbox{ has relation with } R \\\\ \\
& \Longleftrightarrow  \big(\\{a_n\\},\\{b_n\\}\big) \in R \subset {\cal C}[X] \times {\cal C}[X] \\\\ \\
& \Longleftrightarrow  \\{a_n\\} \overset{R}{\sim} \\{b_n\\}
\end{align}
그리고 ${\cal C}[X]$ 위에서의 바이너리-릴레이션 $R$이 ***(i) reflexivity (ii) symmetricity (iii) transitivity*** 를 만족하면 이 릴레이션을 특별히 이퀴배런스-릴레이션 이라고 말한다. 그리고 아래와 같이 ${\cal C}[X]$에서 $\\{a_n\\}$와 이퀴배런스-릴레이션을 가지는 원소들을 모아놓은 집합을 생각할 수 있다. 
\begin{align}
\big[\\{a_n\\}\big]_ R:=\big\\{ \\{x_n\\} : \\{x_n\\} \overset{R}{\sim} \\{a_n\\} \mbox{ and } \\{x_n\\} \in {\cal C}[X]\big\\} 
\end{align}
이 집합을 $\\{a_n\\}$의 ***equivalence class on ${\cal C}[X]$ by $R$*** 이라고 부른다. 보통은 $R$을 생략하여 $\big[\\{a_n\\}\big]$와 같이만 표현하지만 나는 기호의 명확성을 위해서 관계까지 명시하였다. ${\cal C}[X]$에서의 이퀴배런스-클래스는 본질적으로 파티션으로 이해가능하다. 어떠한 집합 ${\cal C}[X]$에서 이퀴배런스-릴레이션이 존재하면 그 집합을 배타적인 이퀴배런스-클래스의 합집합으로 표현가능하다. 즉 파티션이 존재한다. 아래와 같은 식으로 말이다. 
\begin{align}
{\cal C}[X]= \bigcup_{a \in X} \big[\\{a\\}\big]_ {slim}
\end{align}
여기에서 $\\{a\\}$는 
\begin{align}
a,a,a,\dots 
\end{align}
와 같은 수열을 의미하고 따라서 $\big[\\{a\\}\big]_ {slim}$은 수렴값이 $a$로 같은 코시수열들의 모임을 의미한다. 그리고 이제 ${\cal C}[X]$의 $ /\overset{slim}{\sim}$에 대한 코션트셋을 아래와 같이 정의하자. 
\begin{align}
{\cal C}[X]~ /\overset{slim}{\sim} := \Big\\{\big[\\{a\\}\big]_ {slim}: a \in X \Big\\}
\end{align}
즉 ${\cal C}[X]~ /\overset{slim}{\sim}$ 은 관계 $/\overset{slim}{\sim}$ 에 의해 유도되는 ${\cal C}[X]$의 파티션(=이퀴배런스-클래스)들의 집합이다. 따라서 당연히 ${\cal C}[X]~ / \overset{slim}{\sim}$의 모든원소들의 합집합은 ${\cal C}[X]$가 된다.  
