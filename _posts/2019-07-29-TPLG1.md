...
- ***(proposition 14.1)*** 메트릭스페이스 한정으로, 수렴하는 수열은 모두 코시수열이다(메트릭스페이스가 아닌 일반적인 위상공간에서는 당연히 성립하지 않는데, 그 이유는 그런 공간에서는 코시수열자체가 정의 될 수 없기 때문). 

- 위 정리의 역은 성립하지 않는다. 즉 메트릭스페이스에서 정의된 코시수열이 반드시 수렴하는 수열이라는 보장은 없다. 하지만 그 메트릭스페이스가 ***complete*** 하다면 위 정리의 역도 성립한다. 
