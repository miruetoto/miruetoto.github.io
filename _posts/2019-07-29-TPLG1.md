...
- 잠시 바이너리-릴레이션(***binary relation***), 이퀴배런스-릴레이션(equivalence relation), 이퀴배런스-클래스(***equivalence class***) 그리고 코션트셋(quotient set)에 대하여 설명하고 넘어가겠다. 집합 ${\cal C}[X]$의 product space 
\begin{align}
{\cal C}[X] \times {\cal C}[X]
\end{align}
를 생각하자. 이 곱집합의 원소는 $(\\{a_n\\},\\{b_n\\})$ 와 같이 표현될 것이다. 이때 $R \subset  {\cal C}[X] \times {\cal C}[X]$ 을 정의하자. 그리고 $(\\{a_n\\},\\{b_n\\}) \in R $ 이면 **$\\{a_n\\}$과 $\\{b_n\\}$은 $R$의 관계에 있다** 라고 말하자. 
