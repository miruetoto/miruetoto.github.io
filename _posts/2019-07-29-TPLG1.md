...
\end{align}
그리고 바이너리-릴레이션 $R$이 ***(i) reflexivity (ii) symmetricity (iii) transitivity*** 를 만족하면 이퀴배런스-릴레이션 이라고 말한다. 그리고 아래와 같이 ${\cal C}[X]$ 의 원소중에서 $\\{a_n\\}$ 과 이퀴배런스-릴레이션인 원소들을 모아놓은 집합을 생각할 수 있다. 
\begin{align}
\big[\\{a_n\\}\big]:=\big\\{ \\{x_n\\} : \\{a_n\\} \overset{R}{\sim} \\{x_n\\} \mbox{ and } \\{x_n\\} \in {\cal C}[X]\big\\} 
\end{align}
이 집합을 $\\{a_n\\}$ 의 이퀴배런스-클래스 라고 부른다. 
