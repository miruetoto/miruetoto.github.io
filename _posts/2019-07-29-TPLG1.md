...
- 잠시 바이너리-릴레이션(***binary relation***), 이퀴배런스-릴레이션(equivalence relation), 이퀴배런스-클래스(***equivalence class***) 그리고 코션트셋(quotient set)에 대하여 설명하고 넘어가겠다. 집합 ${\cal C}[X]$의 두 원소간 바이너리-릴레이션 $R$이 존재함은 두 원소의 순서쌍이 $R \subset {\cal C}[X] \times {\cal C}[X]$ 의 원소라는식으로 표현가능하다. 예를 들면 아래와 같은 식으로 말이다.  
\begin{align}
