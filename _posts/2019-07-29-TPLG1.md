...
\end{align}
이라고 표현하자. 이때 관계 $\overset{slim}{\sim}$ 은 $X$에서 정의가능한 모든 코시수열들의 집합 ${\cal C}_ X$ 에서 ***equivalence relation*** 이 된다고 한다.  
...
- 집합 ${\cal C}_ X$ 의 두 원소 $\\{a_n\\}$, $\\{b_n\\}$ 간 바이너리-릴레이션 $R$이 존재한다는 문장은 집합론적인 언어로 표현가능하다. 구체적으로는 (1) $R$을 곱집합 ${\cal C}_ X \times {\cal C}_ X$ 의 적당한 부분집합으로 설정하고 (2) 순서쌍 $\big(\\{a_n\\},\\{b_n\\}\big)$ 이 $R$ 의 원소라는 식으로 표현한다. 예를 들면 아래와 같은 식으로 말이다.  
\begin{align}
...
- 그리고 ${\cal C}_ X$ 위에서의 바이너리-릴레이션 $R$이 ***(i) reflexivity (ii) symmetricity (iii) transitivity*** 를 만족하면 이 릴레이션을 특별히 이퀴배런스-릴레이션 이라고 말한다. 

- 그리고 아래와 같이 ${\cal C}_ X$에서 $\\{a_n\\}$와 이퀴배런스-릴레이션을 가지는 원소들을 모아놓은 집합을 생각할 수 있다. 
\begin{align}
...
\begin{align}
{\cal C}_ X= \bigcup_{a \in X} \big[\\{a\\}\big]_ {slim}
\end{align}
여기에서 $\\{a\\}$는 
\begin{align}
a,a,a,\dots 
\end{align}
와 같은 수열을 의미하고 따라서 $\big[\\{a\\}\big]_ {slim}$은 수렴값이 $a$로 같은 코시수열들의 모임을 의미한다. 그리고 이제 ${\cal C}_ X$의 $ /\overset{slim}{\sim}$에 대한 코션트셋을 아래와 같이 정의하자. 
\begin{align}
...
- 이제 $X^* := {\cal C}_ X ~ / \overset{slim}{\sim}$ 을 생각하여 보자. 여기에서 $X^* $ 는 $\overset{slim}{\sim}$ 에 의한 ${\cal C}[X]$ 의 ***quotient set*** 이라고 한다. 
