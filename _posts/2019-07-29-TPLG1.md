...
- 잠시 바이너리-릴레이션(***binary relation***), 이퀴배런스-릴레이션(***equivalence relation***), 이퀴배런스-클래스(***equivalence class***) 그리고 코션트셋(***quotient set***)에 대하여 설명하고 넘어가겠다. 집합 ${\cal C}[X]$의 두 원소 $\\{a_n\\}$, $\\{b_n\\}$ 간 바이너리-릴레이션 $R$이 존재한다는 문장은 (1) $R$을 곱집합 ${\cal C}[X] \times {\cal C}[X]$ 의 적당한 부분집합으로 설정하고 (2) 순서쌍 $\big(\\{a_n\\},\\{b_n\\}\big)$ 이 $R$ 의 원소라는 식으로 표현가능하다. 예를 들면 아래와 같은 식으로 말이다.  
\begin{align}
