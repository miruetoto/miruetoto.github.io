...
- ***convergent sequence*** 는 $\\{a_n\\}$은 위상공간 $(X,{\cal T})$ 에서 정의되는 개념이다. 하지만 ***Cauchy sequence*** $\\{c_n\\}$은 메트릭스페이스 $(X,d)$에서 정의되는 개념이다. 둘은 미묘하게 비슷하면서도 다른점이 있다. <br/>
**(1)** 이때 $\\{a_n\\}$이 $(X,{\cal T})$에서 정의된다는 의미는 $a_n \in X$ 라는 의미이고 $\\{c_n\\}$이 메트릭스페이스 $(X,d)$에서 정의된다는 의미도 같은 의미이다. 
왜냐하면 코시수열은 그 정의상 거리의 개념이 필요하기 때문이다. 그외에도 미묘한 차이점이 있는데 컨버전트-시컨스는 애초에 정의할때 ***limit*** 을 명시하며 그 limit 이 반드시 $X$ 위에 있어야 하는 반면 코시수열은 ***limit*** 을 명시적으로 정의하지 않으며 그 리밋이 $X$위에 존재할 필요도 없다. 

- 


- $X$에서 정의된 어떠한 수열이 $X$에서 수렴한다는 의미는 (1) 그 수열이 ***limit*** 을 가지며 (2) ***limit*** 이 $X$위에 있다는 의미이다. 

- $X=(0,1)$ 위의 usual metric 에 의해서 유도되는 위상공간 $(X,{\cal T})$ 를 생각하자.

- 수열 $\big\\{\frac{1}{2},\frac{1}{3},\frac{1}{4},\dots,\big\\}$ 은 $X$ 에서 정의된 코시수열이지만 그 극한 $0$ 은 $X$ 의 원소가 아니므로 이 수열은 $X$ 에서 수렴하는 수열은 아니다. (참고로 수렴 역시 확률변수처럼 단독으로 정의되지 않는다. "$X$에서 수렴한다" 와 같이 표현해야 맞는 표현이다.) 

- 이처럼 모든 코시수열은 ***limit*** 을 가지지만 그 limit이 반드시 $X$에 존재한다는 보장은 없다. 이러한 이유로 ***complete*** 한 메트릭스페이스라는 개념을 만들었는데 이런 메트릭스페이스에서는 모든 코시수열의 limit 을 포함한다. 
