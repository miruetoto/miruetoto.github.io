...
- 잠시 바이너리-릴레이션(***binary relation***), 이퀴배런스-릴레이션(equivalence relation), 이퀴배런스-클래스(***equivalence class***) 그리고 코션트셋(quotient set)에 대하여 설명하고 넘어가겠다. 집합 ${\cal C}[X]$의 두 원소간의 관계 $R$은 ${\cal C}[X] \times {\cal C}[X]$ 의 부분집합으로 표현가능하다. 예를 들면 아래와 같은 식으로 말이다.  
\begin{align}
\\{a_n\\} \mbox{ and } \\{b_n\\} \mbox{ has relation with } R \quad \Longleftrightarrow \quad \big(\\{a_n\\},\\{b_n\\}\big) \in R 
\end{align}
