...
\begin{align}
\big[\\{a_n\\}\big]:=\big\\{ \\{x_n\\} : \\{x_n\\} \overset{R}{\sim} \\{a_n\\} \mbox{ and } \\{x_n\\} \in {\cal C}[X]\big\\} 
\end{align}
이 집합을 $\\{a_n\\}$의 ***equivalence class on ${\cal C}[X]$ by $R$*** 이라고 부른다. ${\cal C}[X]$에서의 이퀴배런스-클래스는 본질적으로 파티션과 관련이 있다.  어떠한 집합 ${\cal C}[X]$에서 이퀴배런스-릴레이션이 존재하면 그 집합을 배타적인 이퀴배런스-클래스의 합집합으로 표현가능하다. 즉 파티션이 존재한다. 아래와 같은 식으로 말이다. 
\begin{align}
{\cal C}[X]= \bigcup_{a \in \mathbb{R}} \Big\\{ \\{x_n\\} : \\{x_n\\} \overset{slim}{\sim} \\{a\\} \mbox{ and } \\{x_n\\} \in {\cal C}[X]\Big\\}
\end{align}
여기에서 $\\{a\\}$는 
\begin{align}
a,a,a,\dots 
\end{align}
와 같은 수열을 의미하고 따라서 $\big\\{ \\{x_n\\} : \\{x_n\\} \overset{slim}{\sim} \\{a\\} \mbox{ and } \\{x_n\\} \in {\cal C}[X]\big\\}$ 은 수렴값이 $a$인 코시수열들의 모임을 의미한다. 즉 ${\cal X}[X]$는 각 수렴값이 같은 코시열을 묶은 클래스의 파티션으로 표현가능하다.  
