...
\end{cases}
\end{align}
그런데 우리가 관심있는 $-\frac{\partial^2}{\partial^2 \theta}E_{\theta_0} l(\theta;X_1)$
원하는 것은 (R4) 가정하에 $I(\theta)$와 같고 $I(\theta)$는 (R0)~(R5) 하에서 positive definite matrix 이므로 증명이 끝난다. 

- 결국 (R0)~(R5)를 가정하면 최대우도추정량이 일치추정량임을 MCE를 사용해서 보일 수 있다. 