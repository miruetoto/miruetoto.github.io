...
\begin{align}
\frac{1}{n} \sum_{i=1}^{n} \log f(X_i;\theta)\quad \overset{p_{\theta_0} }{\operatorname \rightarrow} \quad E \log f(X_1;\theta).
\end{align}
...
\hat{\theta}^{MLE} \overset{p}{\rightarrow} \theta
\end{align}
핵심은 극한과 argmax함수의 교환인데 일반적으로 이를 다루는것이 MCE이다. MCE의 조건을 다시 살펴보면 아래와 같다. (이때 $D_0(\theta):=E \log f(X_1;\theta)$.) <br/><br/>
