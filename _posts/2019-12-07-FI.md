...
- 그 다음에는  **(R3) for all realization of $(X_1,\dots,X_n) \in \Omega\times \dots \Omega$, the second derivative of log-likelihood function exist and is continuous, i.e., $\frac{\partial^2}{\partial^2 \theta}\frac{1}{n}\sum_{i=1}^{n}l(\theta;x_i)$ exist and is continuous** 임을 가정하자. 참고로
\[
l(\theta,x_i)\neq l(\theta,X_i)
\] 
이므로 위의 조건은 $\frac{\partial^2}{\partial^2 \theta}D_n(\theta)$와 다름을 명심하자. (R3)은 (3)을 imply 한다(모든 realization에 대하여 성립하면 그 realization들의 평균에 대하여서도 성립할것이므로).  

- 남은것은 (2)와 (4)이다. 그런데 ***expectation of $l(\theta;X_1)$***이 $\theta$에 대하여 strictly convex이고 (R3)이 성립하면 (4)가 성립한다는 것이 Pollard 1991에 의해서 증명되었다.  또한 ***loglikelihood***가 strictly convex임을 증명하면 (2)는 저절로 증명되는 셈이다. 따라서 이제 목표는 명확해졌다. 
