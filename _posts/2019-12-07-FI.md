...
- 그 다음에는  **(R3) $\frac{\partial^2}{\partial^2 \theta}D_n(\theta)=\frac{\partial^2}{\partial^2 \theta}\frac{1}{n}\sum_{i=1}^{n}l(\theta;X_i)$ 가 (존재하며) 연속함수** 임을 가정하자. 참고로 이걸 가정하면 (3)은 자동으로 성립한다. 
