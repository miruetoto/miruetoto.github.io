...
\end{align}
우리는 위의 조건에서 (1)이 성립함을 보이기 위해 $(R0)$, $(R1)$를 사용하였다. (2)와 (3)에서 사용되는 compact조건을 해결하기 위해서 
\begin{align}
(R2) \mbox{ parameter space $\Theta$ is open in $\mathbb{R}^k$} 
\end{aling}
를 가정한다. 이를 가정하면 임의의 $\theta$를 포함하는 openball이 항상 존재하게 되어서 (2)와 (3)의 compact조건을 해결할 수 있다. 이제 (2)를 위해서 
\begin{align}
(R3) \mbox{ parameter space $\Theta$ is open in $\mathbb{R}^k$} 
\end{aling}
