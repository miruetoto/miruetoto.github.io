...
\begin{align}
\mathbb{E}\big(R \big)&=r_{00}\frac{e^c}{1+e^c}\frac{e^b}}{1+e^b}+r_{01}\frac{e^c}{1+e^c}\frac{e^{w+b}}{1+e^{w+b}} \\\\ \\
&+r_{10}\frac{1}{1+e^c}\frac{e^b}{1+e^b}+r_{11}\frac{1}{1+e^c}\frac{e^{w+b}}{1+e^{w+b}}
\end{align}
...
\begin{align}
\begin{cases}
\frac{e^{w+b}}{1+e^{w+b}} \approx 1 \\\\ \\
\frac{e^{b}}{1+e^{b}} \approx 1 
\end{cases}
\end{align}
이 달성된다. (즉 두번째 레이어에서 linear를 선택할 확률이 100%에 가까워 진다는 의미임) 따라서 이경우는 어차피 $w$ 값자체가 $0$ 이든 $3$ 이든 $b$만 $-\infty$ 에 가깝게 만들면 장땡이
