...
**(1)** $r_1:=r\big(A_1=-1,A_2=-1\big):\quad$ 에이전트(=컨트롤러)가 $(\mbox{linear},\mbox{linear})$ 를 활성화함수로 선택했을때 보상. <br/>
**(2)** $r_2:=r\big(A_1=-1,A_2=1\big):\quad$: 에이전트(=컨트롤러)가 $(\mbox{linear},\mbox{sigmoid})$ 를 활성화함수로 선택했을때 보상.  <br/>
**(3)** $r_3:=r\big(A_1=1,A_2=-1\big):\quad$ 에이전트(=컨트롤러)가 $(\mbox{sigmoid},\mbox{linear})$ 를 활성화함수로 선택했을때 보상.  <br/>
**(4)** $r_4:=r\big(A_1=1,A_2=1\big):\quad$ 에이전트(=컨트롤러)가 $(\mbox{sigmoid},\mbox{sigmoid})$ 를 활성화함수로 선택했을때 보상.  <br/> <br/> 
...
<br/>
**2000-에폭** 이후에 구해지는 값은 $r_1=0.0535$, $r_2=91.1902$, $r_3=0.3961$, $r_4=34.4771$ 이다. 그냥 이 값들이 수렴값이라고 믿자. (언제까지 돌리고 있을순 없잖음?) 
...
\end{align}
여기에서 $\mathbb{P} \big(A_1 = a_1 \big)$ 와 $\mathbb{P} ~ \big(A_2 = a_2 ~ \big\| ~ A_1= a_1 \big)$는 아래와 같이 모델링 할 수 있다. 
\begin{align}
\begin{cases} 
\mathbb{P} \big(A_1 = a_1 \big) = \frac{e^{a_1c}}{\sum_{a_1 \in {\cal A}}e^{a_1c}} \\\\ \\
\mathbb{P} ~ \big(A_2 = 1 ~ \big\| ~ A_1=i \big)=\frac{e^{wi+b}}{1+ e^{wi+b}}
\end{cases}
\end{align}
...
- 노파심에 조금 더 파라메터를 해석하면 $c$는 첫번째 레이어에서 linear 를 선택할지 sigmoid 를 선택할지를 결정하는 파라메터이고, $b$는 두번째 레이어에서 linear 를 선택할지 sigmoid 를 선택할지 결정하는 파라메터이다. 그리고 $w$는 첫번째 레이어의 선택과 두번째 레이어의 선택 사이의 시컨셜한 관계를 파악하는 파라메터이다. 이 예제 같은 경우는 그냥 첫번째 레이어에서 멀 선택했든지 상관 없이 두번째 레이어의 선택은 linear 이어야 한다. 왜냐고? $r_{00}=0.0535$, $r_{01}=91.1902$, $r_{10}=0.3961$, $r_{11}=34.4771$ 을 잘 살펴보라. 일단 두번째 레이어에서 linear를 선택하지 않고 sigmoid를 선택하는 순간 loss가 기본적으로 $30$ 이 넘어간다. 따라서 이 예제의 경우는 ***레이어 간 activation function의 선택사이의 시컨셜한 관계*** 어쩌고를 파악할 이유 자체가 없다. 그냥 다른 레이어들이 멀 선택하든 각각 독립적으로 linear 를 선택하는게 최선이다. 그래서 최적의 $w$ 값은 $0$이다. 물론 이 예제의 경우 애초에 $w$값보다 $b$값을 압도적으로 $-\infty$에 가까운 값으로 만들면 $w$ 값에 크게 상관없이
\begin{align}
\begin{cases}
\frac{1}{1+e^{w+b}} \approx 1 \\\\ \\
\frac{1}{1+e^{b}} \approx 1 
\end{cases}
\end{align}
이 달성된다. (즉 두번째 레이어에서 linear를 선택할 확률이 100%에 가까워 진다는 의미임) 따라서 이경우는 어차피 $w$ 값자체가 $0$ 이든 $3$ 이든 $b$만 $-\infty$ 에 가깝게 만들면 장땡이다. 
...
\frac{\partial }{\partial b }J(\theta_c)= \mbox{blah~blah~blah~blah~}
\end{align}
를 계산하면 된다. 그런데 막상 미분하려고 보니까 좀 귀찮은거 같아서 코드로 구현하였다. 어차피 이해를 위해서는 미분텀의 closed form이 필요한게 아니고 특정 $\theta_c=(c*,w*,b*)' $ 에서 각각의 편미분 값 자체가 필요한 거라서 계산하지 않았다. 아래는 각각 $J_{\theta_c}$와 임의의 $f$에서 그래디언트를 구하는 함수를 구현한 것이다. <br/>
<center><img src="https://github.com/miruetoto/miruetoto.github.io/blob/master/img/NAS/my_fig2.png?raw=true" width="100%" height="100%"></center>
