...
따라서 ${\cal R}=\\{r_{11},r_{12},r_{21},r_{22}\\}$ 이다. ~~사실 이 예제의 경우 (2)와 (3)의 아키텍처는 같은 아키텍처이므로 에이전트가 선택할 수 있는 아키텍처도 3개이고 따라서 사실상 **신**이 주는 보상도 3개 뿐이다.. 예제를 만들고 보니까 생각났다.. 그래서 잘 만든 예제는 아니다. 그런데 그냥 4개라고 치자.. 어차피 예제일뿐이고 이후의 논리를 전개하는데 그것이 중요한 요소는 아니다..~~
