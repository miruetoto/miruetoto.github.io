...
**(1) $r_{11}$:**  $(a_1,a_2)=(1,1)=(\mbox{linear},\mbox{linear})$ 인 경우의 보상. <br/>
**(2) $r_{12}$:**  $(a_1,a_2)=(1,2)=(\mbox{linear},\mbox{sigmoid})$ 인 경우의 보상. <br/>
**(3) $r_{21}$:**  $(a_1,a_2)=(2,1)=(\mbox{sigmoid},\mbox{linear})$ 인 경우의 보상. <br/>
**(4) $r_{22}$:**  $(a_1,a_2)=(2,2)=(\mbox{sigmoid},\mbox{sigmoid})$ 인 경우의 보상. <br/> <br/> 
따라서 ${\cal R}=\\{r_{11},r_{12},r_{21},r_{22}\\}$ 이다. 

- 이 예제에 한정하여 $r_{11},\dots,r_{22}$를 구해보면 아래와 같다. <br/>
...
<br/>
**2000-에폭** 이후에 구해지는 값은 $r_{11}=0.0535$, $r_{12}=91.1902$, $r_{21}=0.3961$, $r_{22}=34.4771$ 이다. 그냥 이 값들이 수렴값이라고 믿자. (언제까지 돌리고 있을순 없잖음?) 

- 컨트롤러의 액션에 따라 보상은 당연히 달라지게 된다. 그것은 아래와 같이 표현가능하다. 
\begin{align}
\mathbb{E}\big(R \big) = \sum_{j=1}^{2}\sum_{i=1}^{2} r_{ij}~ \mathbb{P} ~ \big(A_2 = j ~ \big\| ~ a_1=i \big) \mathbb{P} \big(A_1 = i \big)
\end{align}
여기에서 $\mathbb{P} \big(A_1 = i \big)$ 와 $\mathbb{P} ~ \big(A_2 = j ~ \big\| ~ a_1=i \big)$는 아래와 같이 모델링 할 수 있다. 
\begin{align}
\begin{cases} 
\mathbb{P} \big(A_1 = 1 \big) = \frac{e^c}{1+e^c} \\\\ \\
\mathbb{P} ~ \big(A_2 = 1 ~ \big\| ~ a_1=i \big)=\frac{e^{wa_1+b}}{\sum_{a_1 \in {\cal A}} e^{wi+b}}
\end{cases}
\end{align}
이제 $p=\mathbb{P}\big(A_1 = 1 \big)$ 이라고 두면 $\mathbb{E}\big(R \big)$ 은 아래와 같이 표현가능하다. 
\begin{align}
\mathbb{E}\big(R \big)&=r_{11}\frac{e^c}{1+e^c}\frac{e^{w+b}}{1+e^{wp+b}}+r_{12}\frac{e^c}{1+e^c}\frac{e^{w+b}}{1+e^{w+b}} \\\\ \\
&+r_{21}\Big(1-\frac{e^c}{1+e^c}\Big)\frac{e^{2w+b}}{1+e^{2w+b}}+r_{22}\Big(1-\frac{e^c}{1+e^c}\Big)\frac{e^{2w+b}}{1+e^{2w+b}}
\end{align}
당연히 쉽게 눈치채겠지만 $p\approx 1$일 수록 그리고 $b$가 클수록 최적값이다. $w$의 값은 크게 의미없다. 어차피 $p\approx 1$이면 첫번째 레이어에서 linear 를 선택할테고, $b$가 크면 두번째 레이어에서 sigmoid 를 선택할 것이다. 

- 노파심에 조금 더 파라메터를 해석하면 $p$는 첫번째 레이어에서 linear 를 선택할지 sigmoid 를 선택할지를 결정하는 파라메터이고, $b$는 두번째 레이어에서 linear 를 선택할지 sigmoid 를 선택할지 결정하는 파라메터이다. 그리고 $w$는 첫번째 레이어의 선택과 두번째 레이어의 선택 사이의 시컨셜한 관계를 파악하는 파라메터이다. 이 예제 같은 경우는 그냥 첫번째 레이어에서 멀 선택했든지 상관 없이 두번째 레이어의 선택은 linear 이어야 한다. 왜냐고? $r_{11}=0.0535$, $r_{12}=91.1902$, $r_{21}=0.3961$, $r_{22}=34.4771$ 을 잘 보라. 일단 두번째 레이어에서 linear를 선택하지 않고 sigmoid를 선택하는 순간 loss가 기본적으로 $30$ 이 넘어간다. 따라서 이 예제의 경우는 ***레이어 간 activation function의 선택사이의 시컨셜한 관계*** 어쩌고를 파악할 이유 자체가 없다. 그냥 다른 레이어들이 멀 선택하든 각각 독립적으로 linear 를 선택하는게 최선이다. 그래서 최적의 $w$ 값은 $0$이다. 물론 이 예제의 경우 애초에 $p \in [0,1]$ 사이고 $b \in \mathbb{R}$ 인 상황이라서 $w$값보다 $b$값을 압도적으로 크게 만들면 
\begin{align}
\frac{e^{wp+b}}{1+e^{wp+b}} \approx 1 
\end{align}
이 달성된다. (즉 두번째 레이어에서 linear를 선택할 확률이 100%에 가까워 진다는 의미임) 따라서 어차피 $w$ 값자체가 $0$ 이든 $3$ 이든 $b$만 크게 만들면 장땡이므로 의미가 없다. 

- 논문의 표기법을 따라서 $\mathbb{E}(R)=J(\theta_c)$ 라고 하자. 여기에서 $\theta_c=(p,w,b)'$이다. **컨트롤러**의 목적은 $J(\theta_c)$를 최소화하는 것이다. (원래 강화학습에서는 리워드를 $J$로 정의해서 $J$를 최대화하는 파라메터를 찾는데 나는 여기서 손실을 $J$로 정의했으므로 $J$를 최소화하는 파라메터를 찾아야 한다. 사실 이거는 정의하기 나름이라 $J$를 최소화 하든 최대화 하든 별로 안중요하다. 논리적으로 맞게 구하기만 하면 된다.) 이제 $J(\theta_c)$를 최소화 하기 위한 미분을 계산해야 한다. 즉 
\begin{align}
\frac{\partial }{\partial p }J(\theta_c)= \mbox{blah~blah~blah~blah~} \\\\ \\
\frac{\partial }{\partial w }J(\theta_c)= \mbox{blah~blah~blah~blah~} \\\\ 
\frac{\partial }{\partial c }J(\theta_c)= \mbox{blah~blah~blah~blah~}
\end{align}
를 계산하면 된다. 그런데 막상 미분하려고 보니까 좀 귀찮은거 같아서 코드로 구현하였다. 어차피 이해를 위해서는 미분텀의 closed form이 필요한게 아니고 특정 $\theta_c=(p*,w*,b*)' $ 에서 각각의 편미분 값 자체가 필요한 거라서 계산하지 않았다. 아래는 각각 $J_{\theta_c}$와 임의의 $f$에서 그래디언트를 구하는 함수를 구현한 것이다. <br/>
<center><img src="https://github.com/miruetoto/miruetoto.github.io/blob/master/img/NAS/my_fig2.png?raw=true" width="100%" height="100%"></center>
