...
\begin{align}
\mathbb{E}(R): {\cal A} \times \dots {\cal A} \rightarrow \mathbb{R} 
\end{align}
인 $\mathbb{E}(R)$만 옵티마이징하면 된

- 강화학습의 묘미 혹은 강점은 아래와 같다. <br/><br/>
**(1)** 에이전트가 **매순간** action을 취하고 엔바이론먼트는 **매순간** 에이전트의 action 에 대한 피드백을 준다. (새로운 상태, 혹은 보상) <br/>
**(2)** 엔바이론먼트가 **매순간** 보상을 주지 않고 **지연된** 보상을 주어도 잘 동작한다. 즉 컨트롤러는 엔바이러먼트의 보상에 대한 단기 혹은 장기계획을 모두 세울 수 있다. 참고로 일반적인 옵티마이징 방법들은 (특히 그래디언트 어쩌구 류) 당장의 보상을 최소화하는 **단기계획**만 세울 수 있다.) <br/>
**(3)** 엔바이론먼트가 보상을 주는 메커니즘 자체를 몰라도 동작한다. 
