...
\begin{align}
\begin{cases}
p_1 = \mathbb{P}_1 ~ \big(A_1 = \mbox{"linear"} \big) \\\\ 
p_2 = \mathbb{P}_2 ~ \big(A_2 = \mbox{"linear"} ~ \big\| ~ a_1 \big) 
\end{cases}
\end{align}
이다. 여기에서 $\mathbb{P}_i : \sigma(A_i) \rightarrow [0,1]$ 인 **probability measure** 이다. 편의상 $(p_1,p_2)$ 를 $\boldsymbol\theta_c$ 라고 하자. 즉 $\boldsymbol\theta_c=(p_1,p_2)'$ 이다. 

- 이제 **에이전트** 와 **엔바이러먼트(혹은 신)** 을 정의하자. 여기에서 **에이전트**는 적절한 $h$와 $g$를 고르는 주체이고 **신**은 에이전트의 선택이 적절한지 적절하지 않은지 보상을 주는 존재이다. 
