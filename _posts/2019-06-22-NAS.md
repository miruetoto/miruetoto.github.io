...
- 이 예제에 한정하여 $r_11,\dots,r_22$를 구해보면 아래와 같다. 
...
- 노파심에 조금 더 파라메터를 해석하면 $p$는 첫번째 레이어에서 linear 를 선택할지 sigmoid 를 선택할지를 결정하는 파라메터이고, $b$는 두번째 레이어에서 linear 를 선택할지 sigmoid 를 선택할지 결정하는 파라메터이다. 그리고 $w$는 첫번째 레이어의 선택과 두번째 레이어의 선택 사이의 시컨셜한 관계를 파악하는 파라메터이다. 이 예제 같은 경우는 그냥 첫번째 레이어에서 멀 선택했든지 상관 없이 두번째 레이어의 선택은 linear 이어야 한다. 왜냐고? $r_{11}=0.0535$, $r_{12}=91.1902$, $r_{21}=0.3961$, $r_{22}=34.4771$ 을 잘 보면 일단 두번째 레이어에서 linear를 선택하지 않고 sigmoid를 선택하는 순간 loss가 기본 34,91 이런식으로 나오기 때문이다. 따라서 이 예제의 경우는 ***각 레이의 선택사이의 시컨셜한 관계*** 어쩌고를 파악할 이유 자체가 없다. 그래서 최적의 $w$ 값은 $0$이다. 물론 이 예제의 경우 애초에 $p \in [0,1]$ 사이고 $b \in \mathbb{R}$ 인 상황이라서 $w$값보다 $b$값을 압도적으로 크게 만들면 
\begin{align}
frac{e^{wp+b}}{1+e^{wp+b}} \approx 1 
\end{align}
이 달성된다. (즉 두번째 레이어에서 linear를 선택할 확률이 100%에 가까워 진다는 의미임) 따라서 어차피 $w$ 값자체가 의미가 없다. 
...
\end{align}
를 계산하면 된다. 그런데 막상 미분하려고 보니까 좀 귀찮은거 같아서 코드로 구현하였다. 어차피 이해를 위해서는 미분텀의 closed form이 필요한게 아니고 특정 $\theta_c=(p*,w*,b*)'$ 에서 각각의 편미분 값 자체가 필요한 거라서 계산하지 않았다. 
