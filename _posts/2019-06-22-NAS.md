...
\begin{align}
\mathbb{P} ~ \big(A_2 = j ~ \big\| ~ a_1=i \big)=\mbox{sigmoid}\big(wa_1+b \big)= \frac{e^{wa_1+b}}{1+e^{wa_1+b}}
\end{align}
이제 $p=\mathbb{P}\big(A_1 = 1 \big)$ 이라고 두면 $\mathbb{E}\big(R \big)$ 은 아래와 같이 표현가능하다. 
\begin{align}
\mathbb{E}\big(R \big)&=r_{11}p\frac{e^{wp+b}}{1+e^{wp+b}}+r_{12}p\frac{e^{w(1-p)+b}}{1+e^{w(1-p)+b}} \\\\ \\
&+r_{21}(1-p)\frac{e^{wp+b}}{1+e^{wp+b}}+r_{22}(1-p)\frac{e^{w(1-p)b}}{1+e^{w(1-p)+b}}
\end{align}
