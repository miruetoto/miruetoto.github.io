...
- 그래프형태의 자료에 $(V,E,f(V))$에서의 hst를 $(V,E,\boldsymbol{h}(V))$라고 정의하자. 각각의 $i=1,\dots,n$에 대하여 $\boldsymbol{h}(v_i)$ 는 크기가 $({\cal T}+1) \times 1$인 벡터이다. 각각의 $i,j \in \\{1,\dots,n\\}$ 에 대하여 노드 $v_i$, $v_j$ 간의 snow-dist는 아래와 같이 정의한다. 
\begin{align}
...
\end{align}
이것은 각각 $\boldsymbol{h}(v_1),\dots,\boldsymbol{h}(v_n)$ 을 차분해서 얻을 수 있는 길이가 ${\cal T} \times 1$ 벡터이다. 

- 각 노드에 내리는 눈의 양을 $\epsilon(v_i)$ 라고 하자. ${\cal N}(i,\tau)$ 를 아래와 같이 정의하자. 
\begin{align}
{\cal N}^+(i,\tau) := \\{j:(i,j)\in E , ~ h^{\tau}(v_i) \leq h^{\tau}(v_j) \\} 
\end{align}
그러면 각 노드에 쌓인 눈의 양을 아래와 같이 정의할 수 있다. 
\begin{align}
h^{\tau}(v_i)=\sum_{j \in {\cal N}^+(i,\tau-1)} \frac{1}{\|{\cal N}(i) \| }\epsilon(v_j) + h^{\tau-1}(v_i)
\end{align}
