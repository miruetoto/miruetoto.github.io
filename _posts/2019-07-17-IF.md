...
- ${\boldsymbol X}$를 timeseries 라고 하자. 즉 ${\boldsymbol X}$는 $(\Omega,{\cal F})$ 에서 $(\mathbb{R}^{\mathbb{R}},{\cal R}^{\mathbb{R}})$으로 가는 메저러블맵핑이다. $X_1,\dots,X_n$ 을 관찰하였다고 하고 이들은 시계열 처럼 서로 먼가 연관이 있는 확률변수라고 하자. 
