...
***신경망의 종류***
- 앞먹임신경망과 순환신경망: DMLP(=DNN)나 CNN과 같은것을 전방신경망이라 하며 RNN이나 LSTM과 같은것을 순환신경망이라고 한다. 
...
\begin{itemize}
	\item 항등함수: $z[i,1]=u[i,1]$. 
	\item 로지스틱함수: $z[i,1]=\frac{1}{1+e^{-u[i,1]}}$. 
	\item 소프트맥스함수: $z[i,1]=\frac{e^{-u[i,1]}}{sum(e^{-u[i,]})}$
\end{itemize}
