...
- (하나의 observation에 대한) 히든레이어의 노드수는 사용자가 임의로 설정할 수 있다. 일반적으로 $p$보다 높은 갯수를 설정한다. 위의 그림의 경우 $p=3$이므로 히든레이어의 노드수는 $3$보다 큰 $4$로 설정하였다. 히든레이어의 경우 각 노드는 입력과 출력이 있다. 편의상 노드의 인풋은 $u$로 노드의 아웃풋은 $w$로 설정한다. 위의 그림이 $i$번째 observation에 대한 아키텍처라면 첫번째 히든레이어의 각 노드에 들어갈 인풋값은 
\begin{align}
u[i,1],\dots,u[i,4]
\end{align}
와 같이 표시할 수 있다. 많은 사람이 아는것처럼 히든레이어 노드의 인풋값은 이전레이어의 아웃풋을 선형결합한 경우이다. 따라서 아래와 같이 표현가능하다. 
\begin{align}
\begin{cases}
