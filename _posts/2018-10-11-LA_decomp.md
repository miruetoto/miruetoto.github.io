...
\end{align}
를 만족하는 벡터 $\boldsymbol v$는 오직 ${\boldsymbol v}={\boldsymbol 0}$인 경우일 때 뿐이란 것을 의미한다. 그런데 고유값의 정의상 $det({\bf A}-\lambda^{\star} {\bf I})=0$이 된다. 따라서 행렬 ${\bf A}-\lambda^{\star} {\bf I}$은 *sing*-매트릭스가 된다. 따라서 ${\bf A}-\lambda^{\star} {\bf I}$의 rows는 일차독립이 아니다. 따라서  $({\bf A}-\lambda^{\star} {\bf I})\bf v=0$을 만족하는 $\bf v \neq 0$ 인 벡터가 적어도 하나는 존재한다. 
