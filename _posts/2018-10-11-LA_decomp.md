...
\end{align}
여기에서 ${\bf P}=cbind(P_1,\dots,P_n)$이고 ${\bf D}=diag(\lambda_1,\dots,\lambda_n)$이다. 그리고 ${\bf A}={\bf P}{\bf D}{\bf P}'$를 그대로 풀면 아래와 같이 쓸 수 있다. 
\begin{align}
{\bf A}&=cbind(P_1, \dots, P_n) diag(\lambda_1, \dots, \lambda_n) rbind(P_1', \dots, P_n') \\\\ \\\\
&=cbind(\lambda_1 P_1, \dots, \lambda_n P_n) rbind(P_1', \dots, P_n') \\\\ \\
&=\sum_{i=1}^{n} \lambda_i P_i P_i'
\end{align}
특히 
\begin{align}
{\bf A}={\bf P}{\bf D}{\bf P}'=\sum_{i=1}^{n} \lambda_i P_i P_i'
\end{align}
와 같은 표현이나 
\begin{align}
{\bf A}{\bf P}={\bf P}{\bf D}
...
**(1)** $cbind(\lambda_1 P_1, \dots, \lambda_n P_n) = cbind(P_1, \dots, P_n)diag(\lambda_1,\dots,\lambda_n)$. <br/>
**(2)** $cbind(\lambda_1 P_1, \dots, \lambda_n P_n) rbind(P_1', \dots, P_n')=\sum_{i=1}^{n} \lambda_i P_i P_i'$. 
