...
\begin{align}
{\bf A} & cbind(Q_1, \dots, Q_n)=cbind({\bf A}Q_1, \dots, {\bf A}Q_n)=cbind(\lambda_1 Q_1, \dots, \lambda_n Q_n) \\\\ 
&=cbind(Q_1, \dots, Q_n)diag(\lambda_1,\dots,\lambda_n)
\end{align}
따라서 *real-symm*-매트릭스 ${\bf A}_ {n \times n}$는 아래와 같이 표현 가능하다. 
\begin{align}
{\bf A}{\bf Q}={\bf Q}{\bf \Lambda} ~~ or ~~ {\bf A}={\bf Q}{\bf \Lambda}{\bf Q}'
\end{align}
여기에서 ${\bf Q}=cbind(Q_1,\dots,Q_n)$이고 ${\bf \Lambda}=diag(\lambda_1,\dots,\lambda_n)$이다. 그리고 ${\bf A}={\bf Q}{\bf \Lambda}{\bf Q}'$를 그대로 풀면 아래와 같이 쓸 수 있다. 
\begin{align}
{\bf A}&=cbind(Q_1, \dots, Q_n) diag(\lambda_1, \dots, \lambda_n) rbind(Q_1', \dots, Q_n') \\\\ \\\\
&=cbind(\lambda_1 Q_1, \dots, \lambda_n Q_n) rbind(Q_1', \dots, Q_n') \\\\ \\
