...
일단 ${\bf A}_ {n \times n}$는 항상 $n$개의 고유값을 가진다. 그중에서 하나의 고유값 $\lambda^{\star} $를 fix했다고 하자. 고유벡터가 없다는 말은 
\begin{align}
\left({\bf A}-\lambda^{\star} {\bf I}\right){\boldsymbol v}={\boldsymbol 0}
\end{align}
를 만족하는 벡터 $\boldsymbol v$는 오직 ${\boldsymbol v}={\boldsymbol 0}$인 경우일 때 뿐이란 것을 의미한다. 그런데 고유값의 정의상 $det({\bf A}-\lambda^{\star} {\bf I})=0$이 된다. 따라서 행렬 ${\bf A}-\lambda^{\star} {\bf I}$은 *sing*-매트릭스가 된다. 따라서 ${\bf A}-\lambda^{\star} {\bf I}$의 rows는 일차독립이 아니다. 따라서  $({\bf A}-\lambda^{\star} {\bf I}){\boldsymbol  v}={\boldsymbol 0}$을 만족하는 $\bf v \neq 0$ 인 벡터가 적어도 하나는 존재한다. 
...
- 지금까지 정리한 사항을 잘 추론하면 아래와 같은 사실들을 정리할 수 있다. 생각보다 당연한듯, 당연하지 않은 사실들이므로 자주 읽으면서 외우는것이 좋다. <br/>
...
\begin{align}
{\bf A} & cbind(P_1, \dots, P_n)=cbind({\bf A}P_1, \dots, {\bf A}P_n)=cbind(\lambda_1 P_1, \dots, \lambda_n P_n) \\\\ 
&=cbind(P_1, \dots, P_n)diag(\lambda_1,\dots,\lambda_n)
\end{align}
따라서 *real-symm*-매트릭스 ${\bf A}_ {n \times n}$는 아래와 같이 표현 가능하다. 
\begin{align}
{\bf A}{\bf P}={\bf P}{\bf \Lambda} ~~ or ~~ {\bf A}={\bf P}{\bf \Lambda}{\bf Q}'
\end{align}
여기에서 ${\bf P}=[{\boldsymbol p}_ 1 ,\dots,{\boldsymbol p}_ n]$이고 ${\bf \Lambda}=diag(\lambda_1,\dots,\lambda_n)$이다. 그리고 ${\bf A}={\bf P}{\bf \Lambda}{\bf P}'$를 그대로 풀면 아래와 같이 쓸 수 있다. 
\begin{align}
{\bf A}&=[{\boldsymbol p}_ 1 ,\dots,{\boldsymbol p}_ n] diag(\lambda_1, \dots, \lambda_n) rbind({\boldsymbol p}_ 1', \dots, {\boldsymbol p}_ n') \\\\ \\\\
&=[\lambda_1{\boldsymbol p}_ 1 ,\dots,\lambda_n{\boldsymbol p}_ n] rbind({\boldsymbol p}_ 1', \dots, {\boldsymbol p}_ n') \\\\ \\
&=\sum_{i=1}^{n} \lambda_i {\boldsymbol p}_ i {\boldsymbol p}_ i'
\end{align}
특히 
\begin{align}
{\bf A}={\bf P}{\bf \Lambda}{\bf P}'=\sum_{i=1}^{n} \lambda_i {\boldsymbol p}_ i {\boldsymbol p}_ i'
\end{align}
와 같은 표현이나 
\begin{align}
{\bf A}{\bf P}={\bf P}{\bf \Lambda}
\end{align}
와 같은 표현은 자주나오므로 익숙해지는 것이 좋다. 또한 위의 과정에서 아래와 같이 매트릭스 계산하는 방식도 눈여겨볼만 하다. <br/>
**(1)** $cbind(\lambda_1 {\boldsymbol p}_ 1, \dots, \lambda_n {\boldsymbol p}_ n) = cbind({\boldsymbol p}_ 1, \dots, {\boldsymbol p}_ n)diag(\lambda_1,\dots,\lambda_n)$. <br/>
**(2)** $cbind(\lambda_1 {\boldsymbol p}_ 1, \dots, \lambda_n {\boldsymbol p}_ n) rbind({\boldsymbol p}_ 1', \dots, {\boldsymbol p}_ i')=\sum_{i=1}^{n} \lambda_i {\boldsymbol p}_ i {\boldsymbol p}_ i'$. 
...
{\bf x}^{H}{\bf A}{\bf x}>0
\end{align}
이는 ${\bf A}=Q\Lambda Q^{H}$꼴로 변형하고 위의 식의 대입하면 쉽게 증명할 수 있다. 반대로 어떠한 정사각행렬 ${\bf A}$가 모든 *non-zero vector* ${\bf x}$에 대하여 위의 식을 만족하면 이 행렬 ${\bf A}$는 1) *에르미트*-매트릭스이며 2) 모든 고유치가 양수인 행렬이 된다(이것도 쉽게 증명된다). 즉 아래조건은 *(에르미트)-pd*-매트릭스의 정의처럼 사용가능하다. 
\begin{align}
...
- 만약에 행렬 ${\bf A}$가 ***psd-symm*** 인 경우 아래가 성립한다. 고유치분해와 SVD는 같다. 즉 아래가 성립한다. 
\begin{align}
