...
### 연구의 필요성
- 본 연구단은 비정형화된 자료를 다루는 새로운 통계방법을 연구한다. 여기에서 비정형화된 자료란 벡터, 행렬 혹은 텐서의 형태로 자료를 정리하여 분석하기에 적합하지 않은 자료를 의미한다. 우리는 특히 비정화된 자료중에서도 그래프형태의 도메인을 가지는 자료를 분석하는 것에 관심이 있다. 그래프형태의 도메인을 가지는 자료는 아래와 같은 자료를 의미한다. 
\begin{align}
f:({\cal V},{\cal E}) \rightarrow \mathbb{R}
\end{align}
여기에서 $V$는 set of vertice이며, $E$는 set of edge를 나타낸다. 그렇다면 그래프형태의 자료에 관심을 가져야만 하는 이유는 무엇인가?
**(1)** 그래프자료에 대한 분석기법은 파급력을 가지고 있다. 그래프자료에 대한 분석기법을 개발하면 그 분석방법을 timeseries, image data, spatio temporary data 등 다양한 형태를 가진 데이터로 확장할 수 있다. 이러한 확장성은 그래프자료가 위의 데이터 형태를 모두 아우르는 가장 일반적인 자료표현형태이기 때문이다. 
**(2)** 그래프자료에 대한 분석은 매우 도전적인 과제이다. 그래프자료는 
- 
