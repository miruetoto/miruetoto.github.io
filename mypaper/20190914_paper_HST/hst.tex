...
\[
sdist(v_i , v_j) :=d_{ij}= \|{\boldsymbol h}_i- {\boldsymbol h}_j\|_2.
\]
...
\begin{rmk}
Suppose $f(v_1),\dots,f(v_n)$ is $i.i.d$ random sample and $(V,E,W)$ be the regular graph. Let ${\cal H}\boldsymbol{f}$ be a heavysnow transform of ${\boldsymbol f}$. Since $f(v_i)$ is $i.i.d.$ random variable ${\boldsymbol h}_i$ can be thought as $i.i.d.$ random vector (\textcolor{red}{need to prove, using `regular graph'}) such that 
\[
