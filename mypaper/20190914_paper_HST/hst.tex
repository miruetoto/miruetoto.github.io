...
\begin{thm} 
Let $X:{\cal V} \to \mathbb {R}$ as measurable mapping connecting vertex $\nu$ to $v$. Define graph signal $f(v) \in {\cal M}$ and its heavy-snow transform ${\cal H}({\boldsymbol f};\tau)$. Let $({\cal S}^{(\tau)},\rho^{(\tau)})$ be the another metric space defined in snowyground where ${\cal S}^{(\tau)}:=\{{\boldsymbol h}^{(\tau)}(v): v\in {\cal M}\}$. Cleary, $\rho^{(0)}$ is metric in ${\cal M}$. Further $\rho^{(\infty)}$ is metric in ${\cal V}$. 
\end{thm}
