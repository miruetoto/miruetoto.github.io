...
\begin{dfn}
Let $V:=\{v_i:i=1,2,\dots,n\}$ be the given index set or realization of $X$. Let ${\boldsymbol f}$ be the graph signal on $(V,E)$. Let ${\cal H}({\boldsymbol f};\tau)$ be heavy-snow transform for ${\boldsymbol f}$. For each $\ell =1,2,\dots,\tau$ the $\ell$-th \emph{snow distance} between $v_i$ and $v_j$ is defined by 
\[
\rho^{(\ell)}(v_i , v_j) := \|{\boldsymbol h}^{(\ell)}_i- {\boldsymbol h}^{(\ell)}_j\|_2.
\]
where ${\boldsymbol h}^{(\ell)}_i=\big(h^{(0)}(v_i),\dots,h^{(\ell)}(v_i)\big)$. 
\end{dfn}

\begin{thm} 
Let $V:=\{v_i:i=1,2,\dots,n\}$ be the given index set or realization of $X$. Define graph signal $f(v) \in {\cal M}$ and its heavy-snow transform ${\cal H}({\boldsymbol f};\tau)$. Let $({\cal S}^{(\ell)},\rho^{(\ell)})$ be the another metric space defined in snowyground where ${\cal S}^{(\ell)}:=\{{\boldsymbol h}^{(\ell)}(v): v\in V\}$. Cleary, $\rho^{(0)}$ is metric in ${\cal M}$. Further $\rho^{(\infty)}$ is metric in ${\cal V}$. 
\end{thm}
