...
\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{Fig/Fig1.png} % requires the graphicx package
...
%Heavy-snow transformation can be applied to a data structure that can be defined by a pair of nodes and edges in graph theory. 
...
%In Definition \ref{snow:df_hst}, $X_i$ can be interpreted as ground and $Y_{i,\tau}$ can be interpreted as snowy ground. $\epsilon_i$ represent the falling snow with amount $b$ and $\eta_i$ is snowdrift which are caused by wind and shape of the land. The way which the snow can move in is defined as ${\cal N}_i$ and movement of snow or wind direction can be specified by $p_{ij}$. 
\fi
\begin{dfn}
